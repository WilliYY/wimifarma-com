...
- Em 2026-05-28, o Miauby interno e o Miauby WhatsApp passaram a compartilhar memoria curta por `site/miauw/agent-memory.php` e pela tabela MySQL `miauw_channel_events`. O chat interno grava entradas/saidas sanitizadas, o worker WhatsApp grava cada resposta enviada sem bloquear a fila, e `site/miauw/agent-context.php` exporta `channel_memory` junto do treino/perfil/tools. Essa memoria usa hash/mascara do contato, texto resumido e metadados limpos; nao guardar telefone cru, payload bruto, audio, midia ou segredo.
