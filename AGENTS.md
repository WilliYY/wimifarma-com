...
- Banco core compartilhado: Postgres `wimifarma_core`, com dados persistidos em `core-data/postgres`; guarda `core_users`, `core_audit_logs` e `core_login_rate_limits`, sincronizados de `wf_users` por `apps/core-auth`. Cotacao usa esse core como login unico, sem dependencia MySQL. Gestao e Pedidos usam esse core como login principal por `*_AUTH_PROVIDER=core`, mantendo fallback MySQL por `*_AUTH_MYSQL_FALLBACK_ENABLED=true` durante a janela de corte.
