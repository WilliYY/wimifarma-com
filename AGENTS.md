...
- Em 2026-05-17, o Miauby iniciou a Fase 19 com `MIAUW_AGENT_VERSION=2.0-fase19`: o chat principal ganhou botao `Falar` para audio Realtime/WebRTC com `gpt-realtime`, criado pelo PHP via `/v1/realtime/calls` para nao expor chave no navegador. `MIAUW_AUDIO_ENABLED=true` libera a interface, `MIAUW_REALTIME_MODEL` e `MIAUW_REALTIME_VOICE` controlam modelo/voz, o microfone so liga por clique, audio nao e gravado/transcrito para historico e escrita operacional por voz continua bloqueada; o servico `wimifarma-miauw-agent` passou para `SERVICE_VERSION=0.13.0` e `PHASE=fase19-realtime-audio-control`.
