...
- Em 2026-05-17, o Miauby ajustou a Fase 19 com `MIAUW_AGENT_VERSION=2.0-fase19`: o chat principal e o widget global usam o botao `Falar` em fluxo estilo WhatsApp, gravando audio temporario no navegador, transcrevendo no PHP com `MIAUW_TRANSCRIPTION_MODEL=gpt-4o-transcribe` e colocando o texto no campo para o usuario revisar, `Enviar` ou `Cancelar`. `MIAUW_AUDIO_ENABLED=true` libera a interface, o microfone so liga por clique, o audio nao e armazenado no banco, escrita operacional por voz continua bloqueada e acoes fortes seguem exigindo confirmacao humana; o servico `wimifarma-miauw-agent` passou para `SERVICE_VERSION=0.14.0` e `PHASE=fase19-record-transcribe-confirm`.
