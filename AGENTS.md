...
- Ainda em 2026-05-20, cards em `Aguardando chegada` e `Confirmados` de Pedidos passaram a minimizar/expandir ao clicar na area de resumo do card, sem botao `+/-`, com estado salvo no navegador. No modo reduzido, `Aguardando chegada` mantem a acao `Confirmar chegada` visivel e `Confirmados` mantem `Pago` visivel. O vencimento do boleto e a data de pagamento parcial na interface de Pedidos passaram a ser somente data, sem horario.
