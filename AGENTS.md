...
- Cotacao V2 em Node.js 22 + Express + Socket.IO, com Postgres 17 e Redis 7. Em 2026-05-31, iniciou a migracao para TypeScript em fases: Fase 0 inventariou baseline, Fase 1 adicionou `tsconfig.json`, `npm run typecheck` e devDependencies TypeScript, Fase 2 criou contratos tipados em `apps/cotacao/src/contracts/` para env/sessao/dominio/APIs/Socket.IO, e Fase 3 comecou helpers TS sombra em `apps/cotacao/src/utils/normalizers.ts` e `apps/cotacao/src/utils/styles.ts`. Essas fases nao alteram runtime, frontend, rotas, Dockerfile ou deploy; `npm start` segue `node src/server.js`.
