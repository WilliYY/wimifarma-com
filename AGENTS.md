...
- O card `Cashback` abre `/cashback/`, servido oficialmente por `apps/cashback` via proxy Apache, mantendo o frontend visual de `site/cashback` por assets montados. A fonte oficial e o Postgres `wimifarma_cashback`; em 2026-05-29 foram validados clientes, atendentes, compras, creditos, resgates, itens, settings, mensagens, saldos e sequencias contra `wf_*`, e as flags `CASHBACK_LEGACY_MYSQL_IMPORT_ENABLED`, `CASHBACK_LEGACY_MYSQL_MIRROR_ENABLED` e `CASHBACK_LEGACY_MYSQL_LOGS_ENABLED` passaram a default `false`. MySQL do Cashback fica apenas como referencia historica/rollback manual, sem conexao ativa do app por padrao.
...
- Em 2026-05-29, o Financeiro foi cortado para `apps/financeiro` como rota oficial `/financeiro/` via proxy Apache para `wimifarma-financeiro-app:3800/financeiro`, preservando o frontend visual de `site/financeiro` por assets montados, login `core_users` por `FINANCEIRO_AUTH_PROVIDER=core`, sessao `WFFINANCEIRO`, Postgres `wimifarma_financeiro` como fonte de verdade e endpoints internos tokenizados para Miauby/WhatsApp. A paridade com MySQL foi validada em contagens, somatorios em centavos, categorias, amostras por dia, CSV, rotas autenticadas e dry-run n8n; por isso `FINANCEIRO_LEGACY_MYSQL_IMPORT_ENABLED` e `FINANCEIRO_LEGACY_MYSQL_MIRROR_ENABLED` ficam `false` por padrao e o Compose nao injeta mais credenciais MySQL no app. MySQL `financeiro_*` deve voltar somente em rollback manual com variaveis/credenciais reintroduzidas.
...
- Em 2026-05-27, confirmacoes do WhatsApp via Evolution passaram a usar texto simples por padrao, porque `sendButtons` pode retornar sucesso na Baileys mas nao renderizar no WhatsApp normal/linked device. Meta Cloud API pode continuar usando botoes interativos; Evolution so deve usar botoes se `MIAUW_WHATSAPP_EVOLUTION_INTERACTIVE_CONFIRMATIONS=true` for ligado apos teste real. O texto de confirmacao nao deve expor codigo curto ao usuario; `SIM`/`NAO` ou clique em botao resolve pela pendencia ativa do remetente autorizado, mesmo quando `MIAUW_WHATSAPP_REQUIRE_PREFIX=true`.
...
- A rota `/cashback/` e servida por proxy interno do Apache para `wimifarma-cashback-app:4000/cashback`; `site/cashback` fica como legado/fallback historico e fonte visual dos assets, mas nao e a fonte oficial da tela apos o corte.
...
- A rota `/financeiro/` e servida por proxy interno do Apache para `wimifarma-financeiro-app:3800/financeiro`; `site/financeiro` fica como legado/fallback historico e fonte visual dos assets, mas nao e a fonte oficial da tela apos o corte.
...
- Banco core compartilhado: Postgres `wimifarma_core`, com dados persistidos em `core-data/postgres`; guarda `core_users`, `core_audit_logs`, `core_login_rate_limits`, `core_user_module_permissions`, `core_user_xp_links`, `core_user_whatsapp_links`, `core_user_audit_events` e sessoes `usuarios_sessions`. `apps/core-auth` sincroniza `wf_users` para `core_users`; `apps/usuarios` cria novos logins core com `legacy_mysql_id` negativo para nao conflitar com ids antigos.
- Banco do Cashback: Postgres `wimifarma_cashback`, com dados persistidos em `cashback-data/postgres`; guarda `cashback_attendants`, `cashback_clients`, `cashback_purchases`, `cashback_credits`, `cashback_redemptions`, `cashback_redemption_items`, `cashback_settings`, `cashback_whatsapp_messages`, `cashback_audit_events`, `cashback_migration_runs` e sessoes `cashback_sessions`. MySQL `wf_*` relacionado ao Cashback nao e mais dependencia ativa por padrao desde 2026-05-29; o caminho de importacao/espelho/log so deve voltar em rollback manual com variaveis e credenciais MySQL explicitamente reintroduzidas.
- Banco do XP: Postgres `wimifarma_xp`, com dados persistidos em `xp-data/postgres`; guarda `xp_employees`, `xp_sales`, `xp_settings`, `xp_audit_events` e sessoes `xp_sessions`. O MySQL `wf_xp_employees`, `wf_xp_sales` e `wf_xp_settings` fica apenas como legado/importacao/espelho temporario de rollback quando as flags `XP_LEGACY_MYSQL_*` estiverem ligadas. O ADM tambem existe como perfil protegido em `system_key='adm'`, pode ter nome/foto editados, pode receber XP, nao pode ser excluido e fica sincronizado com a foto da moldura ADM.
...
- Banco do Financeiro: Postgres `wimifarma_financeiro`, com dados persistidos em `financeiro-data/postgres`; guarda `financeiro_closings`, `financeiro_entries`, `financeiro_sangrias`, `financeiro_card_entries`, `financeiro_pix_entries`, `financeiro_settings`, `financeiro_audit_events`, `financeiro_migration_runs`, `financeiro_internal_idempotency` e `financeiro_sessions`. MySQL `financeiro_*` fica como referencia historica/rollback manual; `FINANCEIRO_LEGACY_MYSQL_*` fica desligado por padrao e exige variaveis/credenciais MySQL explicitas para voltar.
- Banco da Cotacao V2: Postgres `wimifarma_cotacao`, com dados persistidos em `cotacao-data/postgres`.
- Banco da Gestao/Pedidos: Postgres `wimifarma_gestao`, com dados persistidos em `gestao-data/postgres`; `core_users` no Postgres core e o login oficial por padrao. O MySQL `wimifarma_app` fica apenas como rollback opt-in de autenticacao, espelho temporario `wf_logs` e importacao legado.
- Banco da Tarefa: Postgres `wimifarma_tarefa`, com dados persistidos em `tarefa-data/postgres`; guarda `tarefa_tasks`, `tarefa_audit_events` e `tarefa_sessions`. `tarefa_tasks` pode ter `assigned_core_user_id` para tarefa privada por usuario. `TAREFA_AUTH_PROVIDER=core` usa `core_users` como login oficial por padrao; rollback de autenticacao exige voltar `TAREFA_AUTH_PROVIDER=mysql`. `TAREFA_LEGACY_MYSQL_IMPORT_ENABLED`, `TAREFA_LEGACY_MYSQL_MIRROR_ENABLED` e `TAREFA_LEGACY_MYSQL_LOGS_ENABLED` controlam a janela de legado/rollback de dados com MySQL.
...
- Ainda em 2026-05-28, Cotacao V2, Gestao e Pedidos ganharam auth pelo core Postgres. Em 2026-05-29, a Cotacao removeu o fallback MySQL, a dependencia `mysql2`, o `depends_on` de `wimifarma-com-db` e as variaveis MySQL do servico; seu login depende apenas de `core_users` em `wimifarma_core`. Gestao e Pedidos usam `core_users` por default e fallback MySQL apenas quando `*_AUTH_MYSQL_FALLBACK_ENABLED=true` for ligado explicitamente no ambiente.
- Ainda em 2026-05-28, Tarefa foi migrado para `apps/tarefa` em Node.js 22 + TypeScript + Postgres, mantendo HTML/CSS/JS equivalentes ao modulo antigo. Em 2026-05-29, o login passou a usar `core_users` por default com `TAREFA_AUTH_PROVIDER=core`, preservando a sessao propria `WFTAREFA`; rollback de autenticacao e voltar `TAREFA_AUTH_PROVIDER=mysql`. O importador de `wf_tarefas` para `tarefa_tasks` e idempotente, e a janela de legado MySQL pode ser desligada por `TAREFA_LEGACY_MYSQL_IMPORT_ENABLED=false`, `TAREFA_LEGACY_MYSQL_MIRROR_ENABLED=false` e `TAREFA_LEGACY_MYSQL_LOGS_ENABLED=false` apos validacao de dados.
...
- O login de Códigos segue o mesmo padrao visual vinho/rosa dos outros logins internos; a autenticacao oficial usa `core_users` por `CODIGOS_AUTH_PROVIDER=core`, com rollback por `CODIGOS_AUTH_PROVIDER=mysql`.
