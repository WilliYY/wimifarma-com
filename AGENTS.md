...
- Em 2026-05-27, o Miauby WhatsApp ganhou leitura opcional de comprovante Pix por imagem: `MIAUW_WHATSAPP_PIX_RECEIPT_IMAGE_ENABLED=true` baixa imagem autorizada apenas no worker, extrai campos com Gemini, valida `MIAUW_WHATSAPP_PIX_RECEIPT_CNPJ` (padrao `07676534000181`) e prepara lancamento `Pix CNPJ` no Financeiro com confirmacao `Sim`/`Nao`. A imagem bruta nao e persistida; o Postgres guarda somente metadados sanitizados, resultado de extracao e pendencia. Grupos continuam bloqueados por padrao e o contato precisa ter card `Financeiro` liberado.
