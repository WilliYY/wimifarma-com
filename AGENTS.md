...
- Banco core compartilhado: Postgres `wimifarma_core`, com dados persistidos em `core-data/postgres`; guarda `core_users`, `core_audit_logs` e `core_login_rate_limits`, sincronizados de `wf_users` por `apps/core-auth`. Cotacao, Gestao e Pedidos usam esse core como login principal por `*_AUTH_PROVIDER=core`, mantendo fallback MySQL por `*_AUTH_MYSQL_FALLBACK_ENABLED=true` durante a janela de corte.
...
- Ainda em 2026-05-28, Cotacao V2, Gestao e Pedidos ganharam `COTACAO_AUTH_PROVIDER`, `GESTAO_AUTH_PROVIDER` e `PEDIDOS_AUTH_PROVIDER`. O default versionado passou a ser `core`, com fallback MySQL ligado por `*_AUTH_MYSQL_FALLBACK_ENABLED=true` para preservar operacao se o core estiver indisponivel ou uma senha divergir durante a janela de corte. A sombra `*_CORE_AUTH_SHADOW_ENABLED=true` continua disponivel apenas quando o provider volta para `mysql`.
...
- Desde o corte de autenticacao em 2026-05-28, o login da Cotacao V2 usa `core_users` por `COTACAO_AUTH_PROVIDER=core`, com fallback temporario em `wf_users` quando `COTACAO_AUTH_MYSQL_FALLBACK_ENABLED=true`.
