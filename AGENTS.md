...
- O card `Cashback` abre `/cashback/`, servido oficialmente por `apps/cashback` via proxy Apache, mantendo o frontend visual de `site/cashback` por assets montados. A fonte oficial passa a ser Postgres `wimifarma_cashback`; MySQL `wf_clientes`, `wf_atendentes`, `wf_compras`, `wf_cashback_creditos`, `wf_resgates`, `wf_resgate_itens`, `wf_settings`, `wf_whatsapp_mensagens` e `wf_logs` ficam como importacao/espelho temporario de rollback por `CASHBACK_LEGACY_MYSQL_*`.
