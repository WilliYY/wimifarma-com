...
- Cotacao V2 em Node.js 22 + Express + Socket.IO, com Postgres 17 e Redis 7. Em 2026-05-31, iniciou a migracao para TypeScript em fases: Fase 0 inventariou baseline, Fase 1 adicionou `tsconfig.json`, `npm run typecheck` e devDependencies TypeScript, Fase 2 criou contratos tipados em `apps/cotacao/src/contracts/` para env/sessao/dominio/APIs/Socket.IO, Fase 3 comecou helpers TS sombra em `apps/cotacao/src/utils/normalizers.ts`, `styles.ts`, `winner.ts` e `sheets.ts`, e Fase 3.5 adicionou `npm run build:ts`/`tsconfig.build.json` para compilar em `apps/cotacao/dist/` ignorado. Essas fases nao alteram runtime, frontend, rotas, Dockerfile ou deploy; `npm start` segue `node src/server.js`.
- Em 2026-06-01, a Cotacao V2 ganhou atalho `F4` apenas na tela da planilha para repetir a ultima acao util e segura no contexto atual. A estrutura repetivel aceita valor nao vazio, colagem sem celulas vazias, aplicar cor e limpar cor; nao registrar/executar via `F4` acoes destrutivas ou definitivas como excluir valor, apagar distribuidora, cancelar, finalizar, enviar, importar/exportar Google Sheets ou restaurar backup.
...
- Tarefa em Node.js 22 + TypeScript + Express, separado em `/tarefa/`, com sessao propria, Postgres 17 dedicado para tarefas/auditoria/lembretes Miauby, tarefas privadas por usuario e importacao idempotente de `wf_tarefas`.
...
- Em 2026-05-31, o VPS validou a sombra do Miauby apos migracao idempotente: `/miauby/api/internal/readiness?sample=10` respondeu `ok=true`, Postgres ok, paridade 12/12 e zero divergencia de contagem/checksum/amostra, mantendo `write_enabled=false`, `route_cutover_enabled=false` e `public_proxy_enabled=false`.
- Em 2026-05-31, a Fase 4 do Miauby foi ativada no VPS com `MIAUW_ENGINE=node_shadow`, `MIAUW_AGENT_ENGINE_ALLOWED_USERS=adm` e `MIAUW_AGENT_SHADOW_ON_SEND=false`: apenas o usuario `adm` dispara comparacao Node sombra em envios reais, a resposta oficial continua PHP, usuarios comuns nao ganham latencia extra e o rollback imediato e voltar `MIAUW_ENGINE=php` e recriar `wimifarma-com-web`.
...
- Home publica da raiz `/` servida por `site/home.php` via `site/.htaccess` durante a estabilizacao da migracao; antes dos cards existe um login inicial simples com sessao propria `WFHOME`, CSRF, cookie `HttpOnly`/`SameSite=Lax`, credencial temporaria padrao `adm`/`adm` e override opcional por `WIMIFARMA_HOME_LOGIN_USER`/`WIMIFARMA_HOME_LOGIN_PASSWORD`. Em 2026-05-30, a home passou a emitir `WFHOME_SSO`, cookie assinado por `WIMIFARMA_HOME_SSO_SECRET` ou `WP_AUTH_KEY` forte, para que Cashback, Cotacao, Gestao, Pedidos, Tarefa, XP, Codigos, Financeiro, Usuarios e Miauby PHP criem suas proprias sessoes sem pedir senha novamente, sempre revalidando `core_users` ativo e a regra de role/permissao de cada modulo. Em 2026-05-31, as rotas protegidas e telas antigas de login dos modulos passaram a redirecionar visitantes sem sessao nem `WFHOME_SSO` para `/`, centralizando a entrada na home/login; por isso producao deve manter segredo forte para o SSO. A tela de login usa a mesma logo GIF animada da home autenticada, texto `Apenas funcionarios`, anel animado, happy cat zanzando, estrelas cadentes leves apenas no topo ciano, footer liquido com variacao cromatica lenta e bolhas integradas ao bloco, logo, bloco de atendimento e botao do WhatsApp com icone solido, centralizado em telas estreitas e sem a lista antiga de modulos. Em 2026-05-30 e 2026-05-31, o footer do login foi reduzido por espacamento/altura, preservando a animacao das bolhas; os tres arcos do anel do login foram ampliados novamente de forma leve em telas com espaco pelo tamanho do container, sem alterar a animacao nem o formulario; os textos do footer foram alinhados em colunas com limites de largura para leitura mais regular. Em 2026-06-01, a tela de login ganhou em telas com espaco um card de video MP4 em loop (`assets/video/login-redirecionado.mp4`) com borda arredondada e link para `https://wimifarma.com.br`, sem alterar o fluxo de login; em telas estreitas o card fica oculto para preservar layout e performance. Ainda em 2026-06-01, a troca multicolorida pesada e os circulos decorativos internos do footer do login foram desativados para evitar piscada/repaint; a animacao liquida original foi restaurada com 128 bolhas e filtro SVG, mantendo `contain: layout` para preservar o visual sem recortar o efeito, e a variacao de cores voltou como ciclo lento de `background-color` aplicado ao bloco e as bolhas, sem mexer na geometria do efeito. Ainda em 2026-06-01, o topo do login passou a usar 28 estrelas cadentes aleatorias em CSS, sem fundo preto, atravessando a largura da area superior atras do formulario/video e com reducao automatica em telas pequenas. Ainda em 2026-06-01, o footer do login ganhou contador anonimo por cookie `WFHOME_VISITOR`, exibindo visitantes unicos e acessos registrados, gravando apenas totais em `site/wp-content/uploads/wimifarma-runtime/home-counter.json` protegido por `.htaccess` gerado; se o arquivo zerado encontrar navegador com cookie antigo, a primeira visualizacao humana tambem inicializa 1 visitante para nao ficar preso em zero. A home autenticada usa fundo visual em video em tela inteira com as cores originais, sem overlay branco de clareamento, cards inferiores elevados para abrir espaco futuro, logo animada propria sem fundo em `site/wp-content/themes/wimifarma-cashback-theme/assets/img/logo-wimifarma-home-animated.gif`, botao `Sair` e GIFs decorativos com o mesmo padrao de movimento dos logins.
...
- O card `Tarefas` abre `/tarefa/`, servido oficialmente por `apps/tarefa` via proxy Apache, preservando a mesma interface visual do modulo antigo. A fonte oficial de tarefas e `tarefa_tasks` no Postgres `wimifarma_tarefa`; tarefas comuns ficam com `assigned_core_user_id IS NULL` e aparecem para todos, enquanto tarefas privadas usam `assigned_core_user_id` e aparecem somente para o login delegado. Desde 2026-06-01, o ADM/admin tambem ve todas as tarefas na tela `/tarefa/`, pode escolher o usuario de destino ao criar/editar e pode agendar lembrete Miauby por WhatsApp em `tarefa_reminders`; o bridge envia apenas para contato vinculado ao usuario com card `tarefas` liberado, e tentativas/resultados ficam auditados. Desde 2026-05-30, Tarefa nao possui `mysql2`, importador, espelho de logs, fallback `wf_users`, `TAREFA_AUTH_PROVIDER` nem flags `TAREFA_LEGACY_MYSQL_*`; `wf_tarefas` fica apenas como referencia historica/rollback por restauracao de versao anterior e backup.
...
- Em 2026-05-27, o Miauby WhatsApp ganhou leitura opcional de comprovante Pix por midia: `MIAUW_WHATSAPP_PIX_RECEIPT_IMAGE_ENABLED=true` baixa foto, print, imagem encaminhada ou PDF/documento autorizado apenas no worker, extrai campos com Gemini, valida o destino por CNPJ/chave Pix `MIAUW_WHATSAPP_PIX_RECEIPT_CNPJ` (padrao `07676534000181`) ou nome correlato em `MIAUW_WHATSAPP_PIX_RECEIPT_DESTINATION_ALIASES`, e prepara lancamento `Pix CNPJ` no Financeiro com confirmacao `Sim`/`Nao`. A midia bruta nao e persistida; o Postgres guarda somente metadados sanitizados, resultado de extracao e pendencia. Erros transitorios de OCR tentam retry antes de responder falha ao usuario. Grupos continuam bloqueados por padrao e o contato precisa ter card `Financeiro` liberado antes de chamar OCR/Gemini. Em 2026-05-29, a leitura passou a usar legenda/nome do arquivo como pista, reforcar a extracao de valor pago contra saldo/tarifa/limite e aceitar fallback deterministico de valor/data/hora a partir do texto OCR compacto. Em 2026-06-01, a validacao ficou mais conservadora: CNPJ de destino extraido diferente do configurado bloqueia o lancamento, CNPJ achado apenas no texto bruto so vale quando estiver em contexto de destino/recebedor, a extracao pode carregar ID Pix/E2E e confianca por campo, e falhas/recusas passam a registrar diagnostico sanitizado sem `raw_text` completo. Ainda em 2026-06-01, a etapa de velocidade passou a filtrar antes do OCR midias/legendas claramente nao Pix ou extensoes sem suporte, salvar hash salgado do arquivo quando o transporte fornece e evitar nova leitura/confirmacao quando o mesmo arquivo ou ID Pix/E2E ja foi tratado nos ultimos 90 dias; quando a imagem nao parece comprovante Pix, responde `Isso ai é um comprovante pix?` sem tentar lancar nada. A correcao manual aceita `pix cnpj valor - nome - obs opcional` e tambem tolera o typo `pix cpnj`; se data/hora nao vierem, usa o momento atual.
...
- Em 2026-05-29, o Miauby WhatsApp ganhou a rotina n8n `Chegada de pedidos`: `POST /miauw/whatsapp/internal/pedidos-arrival-check` deve ser chamado pelo n8n todo dia as 17h com `notify=always`, consulta Pedidos por endpoint interno tokenizado, envia a lista de `Aguardando chegada` apenas para contatos reais com card `Pedidos` e aparece no painel `/miauw/whatsapp/` com botao para ativar/desativar. Em 2026-06-01, essa tabela passou a usar o valor total do pedido, nao o saldo restante, para nao mostrar R$ 0,00 em pedidos ja pagos mas ainda nao recebidos; a mensagem automatica nao deve incluir o rodape pedindo `cimed chegou`/`nenhum chegou`, embora essas respostas continuem aceitas. Perguntas como `miauby em pedidos o que falta chegar` respondem localmente com a mesma tabela quando o contato tem card `Pedidos`. Respostas como `cimed chegou` sao tratadas deterministicamente pelo bridge, exigem card `Pedidos`, chamam `POST /pedidos/api/internal/confirm-arrival` e movem somente a chegada para `Confirmados`/`Historico`; pagamento continua no fluxo normal de Pedidos.
- Em 2026-05-29, o Miauby WhatsApp ganhou a rotina n8n `Fechamento de caixa`: `POST /miauw/whatsapp/internal/financeiro-cash-closing-reminder` deve ser chamado pelo n8n todo dia as 18h com `notify=always`, consulta `GET /financeiro/api/internal/cash-closing-status` e envia lembrete com frase variada apenas para contatos reais com card `Financeiro` quando houver caixa em aberto, em conferencia ou sem registro no dia consultado. Desde 2026-06-01, a mensagem tambem informa quais dias estao em aberto para finalizar, limitada aos ultimos 10 dias ate a data consultada para nao puxar meses antigos nem datas futuras fora da rotina, e o card da rotina no painel `/miauw/whatsapp/` mostra o status vivo do Financeiro quando disponivel. Se o Financeiro nao retornar nenhum dia em aberto nessa janela e o dia consultado estiver `fechado`, `divergente` ou `sem_movimento`, o bridge nao envia nada. A rotina aparece no painel com botao para ativar/desativar.
...
- O workflow versionado `ops/n8n/workflows/financeiro-fechamento-caixa-18h.json` agenda o lembrete diario de fechamento do caixa as 18h e chama o endpoint interno do Miauby WhatsApp. Ele usa `WIMIFARMA_INTERNAL_BASE_URL` e `MIAUW_GUARDIAN_TOKEN` no ambiente do n8n; nunca gravar token dentro do JSON. Em n8n 2.22, manter `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` na stack n8n, pois esses workflows usam `$env` para URL/token; se bloquear, os crons rodam mas falham com `access to env vars denied`.
...
- Banco da Tarefa: Postgres `wimifarma_tarefa`, com dados persistidos em `tarefa-data/postgres`; guarda `tarefa_tasks`, `tarefa_reminders`, `tarefa_audit_events` e `tarefa_sessions`. `tarefa_tasks` pode ter `assigned_core_user_id` para tarefa privada por usuario; `tarefa_reminders` registra horario, status, tentativas e resultado do envio Miauby WhatsApp. O app usa `core_users` como login unico e nao possui mais dependencia MySQL desde 2026-05-30; rollback MySQL exige restaurar versao anterior e backup validado.
