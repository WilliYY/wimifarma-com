...
- Em 2026-05-29, os logins dos modulos internos foram alinhados para usar o core Postgres por padrao. Pedidos removeu fallback MySQL e usa somente `core_users`; Miauby PHP usa `WIMIFARMA_INTERNAL_AUTH_PROVIDER=core` via `pdo_pgsql` no container web. Em 2026-05-30, Cashback, Gestao e Tarefa passaram a login unico em `core_users`, sem `CASHBACK_AUTH_PROVIDER`, `GESTAO_AUTH_PROVIDER`, `TAREFA_AUTH_PROVIDER` nem fallback MySQL no codigo. Rollback de autenticacao ainda existe por `.env` apenas onde documentado, mas deve ser opt-in e temporario. Frontend, assets e happy cat dos logins nao foram alterados.
