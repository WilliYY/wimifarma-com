...
- Em 2026-05-29, os logins dos modulos internos foram alinhados para usar o core Postgres por padrao. Gestao/Pedidos usam `core_users` com fallback MySQL desligado por default (`*_AUTH_MYSQL_FALLBACK_ENABLED=false`); Tarefa usa `TAREFA_AUTH_PROVIDER=core`; Cashback e Miauby PHP usam `WIMIFARMA_INTERNAL_AUTH_PROVIDER=core` via `pdo_pgsql` no container web. Rollback de autenticacao ainda existe por `.env`, mas deve ser opt-in e temporario. Frontend, assets e happy cat dos logins nao foram alterados.
...
- Modulos internos PHP puro:
  - `site/cashback`, com dados de clientes/compras/resgates ainda em MySQL, mas login interno no core Postgres por `WIMIFARMA_INTERNAL_AUTH_PROVIDER=core`.
  - `site/financeiro`, legado/fonte de assets; a rota oficial `/financeiro/` usa Node/Postgres.
