...
- Cores de categorias comuns devem vir de `cotacao_regras_formatacao`; nao recriar classes fixas no CSS/JS nem filtro de cor por palavra-chave. As palavras historicas `geral`, `urgente` e `encomenda` nao devem ter gatilho automatico por texto na categoria: regras legadas ativas para `urgente`/`encomenda` sao desativadas por `cotacao_disable_legacy_category_trigger_rules()` e a regra legada de `geral` por `cotacao_disable_default_category_trigger_rules()`.
- Categoria vazia nao deve virar `geral` automaticamente durante edicao. Em linhas existentes, saves de categoria tambem nao devem alterar `ordem`; a ordem so deve mudar em criacao/reordenacao explicita.
