...
- Em 2026-05-29, os logins dos modulos internos modernos foram alinhados para usar o core Postgres por padrao: Gestao e Pedidos continuam com `GESTAO_AUTH_PROVIDER=core`/`PEDIDOS_AUTH_PROVIDER=core`, mas o fallback MySQL deixou de ligar por default (`*_AUTH_MYSQL_FALLBACK_ENABLED=false`); Tarefa passou a usar `TAREFA_AUTH_PROVIDER=core` por default. Rollback de autenticacao ainda existe por `.env`, mas deve ser opt-in e temporario. Frontend, assets e happy cat dos logins nao foram alterados.
