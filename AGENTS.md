...
- A Evolution API do Miauby WhatsApp tem template operacional em `ops/evolution/` e, no VPS, deve rodar separada em `/home/ubuntu/projetos/wimifarma-evolution-api`, com containers `wimifarma-evolution-api`, `wimifarma-evolution-manager`, `wimifarma-evolution-postgres` e `wimifarma-evolution-redis`. A API fica publicada apenas em `127.0.0.1:8080` e conectada internamente na rede `wimifarma-com-network`; o bridge deve usar `EVOLUTION_API_BASE_URL=http://wimifarma-evolution-api:8080`.
