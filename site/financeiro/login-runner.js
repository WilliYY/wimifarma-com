(function () {
    ...
    function initLoginRunners() {
        ...
        var states = nodes.map(function (node, index) {
            return {
                node: node,
                x: clamp(window.innerWidth * (0.16 + (index * 0.18)), 18, Math.max(18, window.innerWidth - 160)),
                y: clamp(window.innerHeight * (0.58 - (index * 0.2)), 72, Math.max(72, window.innerHeight - 160)),
