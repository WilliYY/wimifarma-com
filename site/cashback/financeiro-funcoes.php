...
function financeiro_schema_statements(): array
{
    return array(
        "CREATE TABLE IF NOT EXISTS financeiro_fechamentos (
            ...
            status ENUM('aberto', 'conferencia', 'fechado', 'divergente') NOT NULL DEFAULT 'aberto',
...
function financeiro_ensure_existing_columns(): void
{
    $columns = array(
        'financeiro_fechamentos' => array(
            ...
            'status' => "status ENUM('aberto', 'conferencia', 'fechado', 'divergente') NOT NULL DEFAULT 'aberto' AFTER responsavel_id",
            ...
            'ajustes' => 'ajustes DECIMAL(10,2) NOT NULL DEFAULT 0.00 AFTER abertura_sistema',
...
function financeiro_fetch_by_date(string $date): ?array
{
    ...
    $stmt = db()->prepare(
        'SELECT f.*, a.nome AS responsavel_nome, u.username AS fechado_por_nome
...
{
    $stmt = db()->prepare('SELECT * FROM financeiro_fechamentos WHERE id = ? LIMIT 1');
...
function financeiro_recalculate(int $closingId): array
{
    ...
    $cartao = $sums['maquininhas_total_qtd'] > 0 ? $sums['cartao_total'] : (float) $closing['cartao_total'];
    $pixBanco = $sums['pix_total_qtd'] > 0 ? $sums['pix_banco_total'] : (float) $closing['pix_banco_total'];
    $pixMaquininhaAuto = $sums['pix_maquininha_maquininha'] + $sums['pix_maquininha_pix'];
    $pixMaquininha = ($sums['maquininhas_total_qtd'] + $sums['pix_total_qtd']) > 0 ? $pixMaquininhaAuto : (float) $closing['pix_maquininha_total'];
    $sangria = $sums['sangrias_total_qtd'] > 0 ? $sums['sangria_total'] : (float) $closing['sangria_total'];
    ...
    $stmt = db()->prepare(
        'UPDATE financeiro_fechamentos
         SET cartao_total = ?, pix_banco_total = ?, pix_maquininha_total = ?, pix_correto_total = ?,
             sangria_total = ?, total_conferido = ?, sobra_falta = ?
         WHERE id = ?'
    );
    $stmt->execute(array($cartao, $pixBanco, $pixMaquininha, $pixCorreto, $sangria, $total, $diff, $closingId));
...
function financeiro_update_manual_closing(int $closingId, array $data): array
{
    ...
    $stmt = db()->prepare(
        "UPDATE financeiro_fechamentos
         SET responsavel_id = ?, status = 'conferencia', caixa_fisico = ?, cartao_total = ?,
             pix_banco_total = ?, pix_maquininha_total = ?, pix_correto_manual = ?,
             pix_correto_justificativa = ?, sangria_total = ?, retirada_caixa = ?,
             abertura_sistema = ?, ajustes = ?, justificativa = ?, observacao = ?
         WHERE id = ?"
    );
    $stmt->execute(array(
        $data['responsavel_id'],
    ...
    financeiro_assert_justification((float) $after['sobra_falta'], (string) $after['justificativa']);
...
function financeiro_month_closings(int $month, int $year): array
{
    ...
    $stmt = db()->prepare(
        'SELECT f.*, a.nome AS responsavel_nome
         FROM financeiro_fechamentos f
         LEFT JOIN wf_atendentes a ON a.id = f.responsavel_id
