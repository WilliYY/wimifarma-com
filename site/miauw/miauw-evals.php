...
miauw_eval_add('agent_status_fase21', static function (): void {
    ...
    miauw_eval_assert(strpos((string) ($status['version'] ?? ''), '2.0-fase21') === 0, 'Versao do agente deve apontar Fase 21.');
    ...
    miauw_eval_assert_same('miauby-voice-playback-profile-2026-05-17', (string) ($status['audio_version'] ?? ''), 'Versao de audio seguro mudou.');
    ...
    miauw_eval_assert(in_array('treino_aprovado_compilado', (array) ($status['features'] ?? array()), true), 'Fase 17 precisa anunciar treino compilado.');
    miauw_eval_assert(in_array('resposta_local_por_treino', (array) ($status['features'] ?? array()), true), 'Fase 17 precisa anunciar resposta local por treino.');
    miauw_eval_assert(in_array('evals_treino_relevancia', (array) ($status['features'] ?? array()), true), 'Fase 17 precisa anunciar evals de relevancia do treino.');
    ...
    miauw_eval_assert(in_array('audio_bolha_player_chat', (array) ($status['features'] ?? array()), true), 'Fase 21 precisa anunciar bolha de audio no chat.');
    miauw_eval_assert(in_array('audio_resposta_falada', (array) ($status['features'] ?? array()), true), 'Fase 21 precisa anunciar resposta falada.');
    miauw_eval_assert(in_array('audio_curto_bloqueado', (array) ($status['features'] ?? array()), true), 'Fase 21 precisa bloquear audio curto.');
    ...
    miauw_eval_assert(in_array('audio_playback_blob_liberado', (array) ($status['features'] ?? array()), true), 'Fase 21 precisa liberar playback blob no CSP.');
    miauw_eval_assert(in_array('seletor_voz_diagnostico', (array) ($status['features'] ?? array()), true), 'Fase 21 precisa anunciar seletor de voz.');
    miauw_eval_assert(in_array('perfil_voz_tts_forte', (array) ($status['features'] ?? array()), true), 'Fase 21 precisa anunciar perfil forte de TTS.');
    ...
    miauw_eval_assert_same('gpt-4o-mini-tts', (string) ($status['speech_model'] ?? ''), 'Modelo de voz falada padrao mudou.');
    miauw_eval_assert((string) ($status['speech_voice'] ?? '') !== '', 'Voz de resposta falada nao pode ficar vazia.');
    miauw_eval_assert(in_array('cedar', (array) ($status['speech_voice_options'] ?? array()), true), 'Opcoes de voz precisam incluir cedar.');
...
miauw_eval_add('fase21_contrato_voz_audio_node', static function (): void {
    $contract = miauw_agent_next_phase_contract();

    miauw_eval_assert_same('fase21', (string) ($contract['fase_atual'] ?? ''), 'Contrato da proxima fase deve partir da fase 21.');
    ...
    miauw_eval_assert(!empty($contract['pronto_agora']['perfis_voz_tom']), 'Fase 18 precisa marcar perfis de voz/tom prontos.');
    ...
    miauw_eval_assert(!empty($contract['pronto_agora']['audio_bolha_player_chat']), 'Fase 21 precisa marcar bolha de audio pronta.');
    miauw_eval_assert(!empty($contract['pronto_agora']['audio_resposta_falada']), 'Fase 21 precisa marcar resposta falada pronta.');
    miauw_eval_assert(!empty($contract['pronto_agora']['audio_curto_bloqueado']), 'Fase 21 precisa marcar bloqueio de audio curto.');
    miauw_eval_assert(!empty($contract['pronto_agora']['audio_sem_armazenamento']), 'Fase 19 precisa manter audio sem armazenamento.');
    miauw_eval_assert(!empty($contract['pronto_agora']['audio_playback_blob_liberado']), 'Fase 21 precisa marcar playback blob no contrato.');
    miauw_eval_assert(!empty($contract['pronto_agora']['seletor_voz_diagnostico']), 'Fase 21 precisa marcar seletor de voz no contrato.');
    miauw_eval_assert(!empty($contract['pronto_agora']['perfil_voz_tts_forte']), 'Fase 21 precisa marcar perfil TTS forte no contrato.');
    ...
    miauw_eval_assert(!empty($contract['pronto_agora']['perfil_treino_compilado']), 'Fase 17 precisa marcar perfil de treino compilado.');
    miauw_eval_assert(!empty($contract['pronto_agora']['resposta_local_por_treino']), 'Fase 17 precisa marcar resposta local por treino.');
...
miauw_eval_add('fase15_roteador_estilo_casual', static function (): void {
    ...
    miauw_eval_assert(is_array($context['voice_profile'] ?? null), 'Contexto de estilo precisa carregar perfil de voz.');
    miauw_eval_assert_same('miauby_padrao', (string) ($context['voice_profile']['profile_id'] ?? ''), 'Contexto de voz precisa manter perfil padrao.');
...
miauw_eval_add('fase21_perfil_voz_audio_seguro', static function (): void {
    ...
    $speechVoices = miauw_agent_speech_voices();
    $speechVoice = miauw_agent_speech_voice_contract();
    $voice = miauw_agent_voice_profile_contract();
    $audio = miauw_agent_audio_contract();
    $speechInstructions = miauw_agent_speech_instructions(array('id' => 1, 'username' => 'adm'));
    $contextText = miauw_agent_style_context_text('qual sua api?', 1);

    miauw_eval_assert(isset($profiles['miauby_padrao'], $profiles['miauby_curto'], $profiles['miauby_operacional']), 'Perfis de voz essenciais precisam existir.');
    miauw_eval_assert(isset($speechVoices['marin'], $speechVoices['cedar'], $speechVoices['ash'], $speechVoices['coral'], $speechVoices['verse']), 'Vozes TTS essenciais precisam existir.');
    miauw_eval_assert(in_array((string) ($speechVoice['id'] ?? ''), array_keys($speechVoices), true), 'Voz TTS atual precisa estar na lista fechada.');
    miauw_eval_assert_same('miauby-voice-profile-2026-05-17', (string) ($voice['version'] ?? ''), 'Perfil de voz precisa ser versionado.');
    ...
    miauw_eval_assert_same('miauby-voice-playback-profile-2026-05-17', (string) ($audio['version'] ?? ''), 'Contrato de audio precisa ser versionado.');
    ...
    miauw_eval_assert(in_array((string) ($audio['mode'] ?? ''), array('text_only', 'record_transcribe_voice_reply_confirmed'), true), 'Modo de audio precisa ser seguro.');
    ...
    miauw_eval_assert_same((bool) ($audio['enabled'] ?? false), (bool) ($audio['playback_enabled'] ?? false), 'Playback so pode ligar quando audio estiver habilitado.');
    miauw_eval_assert_same((bool) ($audio['enabled'] ?? false), (bool) ($audio['tts_enabled'] ?? false), 'TTS so pode ligar quando audio estiver habilitado.');
    miauw_eval_assert_same((bool) ($audio['enabled'] ?? false), (bool) ($audio['voice_reply_enabled'] ?? false), 'Resposta falada so pode ligar quando audio estiver habilitado.');
    miauw_eval_assert_same('gpt-4o-transcribe', (string) ($audio['model'] ?? ''), 'Modelo de transcricao padrao mudou.');
    miauw_eval_assert_same('gpt-4o-mini-tts', (string) ($audio['speech_model'] ?? ''), 'Modelo de fala padrao mudou.');
    miauw_eval_assert((string) ($audio['voice'] ?? '') !== '', 'Voz de fala nao pode ficar vazia.');
    miauw_eval_assert_same('blob_url', (string) ($audio['playback_transport'] ?? ''), 'Playback deve usar blob URL temporaria.');
    miauw_eval_assert(count((array) ($audio['voice_options'] ?? array())) >= 5, 'Contrato de audio precisa listar opcoes de voz.');
...
miauw_eval_add('fase21_audio_curto_bloqueia_chute', static function (): void {
    $blocked = false;
    try {
        miauw_agent_validate_transcribed_audio_text('A empresa farmaceutica Wimifarma esta oferecendo um novo servico de cotacao para clientes com cashback e encomenda.', 1000);
    } catch (InvalidArgumentException $error) {
        $blocked = true;
    }
...
miauw_eval_add('fase17_treinador_compilado', static function (): void {
    ...
    try {
        ...
        miauw_eval_assert(is_array($context['training_profile'] ?? null), 'Contexto deve expor training_profile como array.');
        miauw_eval_assert_contains('tema amplo ou fora da operacao', $contextJson, 'Perfil de treino deve compilar regra para tema amplo.');

        miauw_add_message($conversationId, 1, 'user', 'qual login e senha?');
        $assistantSensitiveId = miauw_add_message($conversationId, null, 'assistant', 'Nao exponho senha.');
        $sensitiveFeedback = miauw_training_create_feedback(
            $conversationId,
            1,
            $assistantSensitiveId,
            'ajuste',
            'sensivel',
            'Oxe. Login e senha eu nao invento e nem exponho. Se travou acesso, chama suporte interno com usuario e tela.',
            'informacao interna',
            'miauby direto',
            false
        );
        miauw_training_review_item(
            (int) $sensitiveFeedback['id'],
            'aprovado',
            1,
            'Oxe. Login e senha eu nao invento e nem exponho. Se travou acesso, chama suporte interno com usuario e tela.',
            'informacao interna',
        ...
        $local = miauw_training_try_local_reply('qual login e senha?');
        miauw_eval_assert(is_array($local), 'Treino aprovado exato precisa virar resposta local.');
        miauw_eval_assert_same('miauw-training-router', (string) ($local['model'] ?? ''), 'Resposta local por treino deve declarar roteador correto.');
        miauw_eval_assert_contains('nao invento', (string) ($local['text'] ?? ''), 'Resposta local por treino perdeu texto aprovado.');
    } finally {
...
miauw_eval_add('fase21_tool_contract_export_seguro', static function (): void {
    ...
    miauw_eval_assert_same('fase21-voice-playback-profile-selector', (string) ($contracts['phase'] ?? ''), 'Contrato de tools deve apontar Fase 21.');
...
miauw_eval_add('intent_gestao_conta_confirmada', static function (): void {
    ...
    $valueAndName = miauw_skill_gestao_command_from_message('gestao - 50 - Will');
    miauw_eval_assert(is_array($valueAndName), 'Comando valor-nome da Gestao nao foi detectado.');
    miauw_eval_assert_same('Will', (string) ($valueAndName['titulo'] ?? ''), 'Comando valor-nome errou titulo.');
    miauw_eval_assert(abs((float) ($valueAndName['valor'] ?? 0) - 50.0) < 0.001, 'Comando valor-nome errou valor.');
    miauw_eval_assert_same('geral', (string) ($valueAndName['categoria'] ?? ''), 'Comando valor-nome precisa cair na categoria geral.');

    $nameAndValue = miauw_skill_gestao_command_from_message('gestao - Will - 50');
    miauw_eval_assert(is_array($nameAndValue), 'Comando nome-valor da Gestao nao foi detectado.');
    miauw_eval_assert_same('Will', (string) ($nameAndValue['titulo'] ?? ''), 'Comando nome-valor errou titulo.');
    miauw_eval_assert(abs((float) ($nameAndValue['valor'] ?? 0) - 50.0) < 0.001, 'Comando nome-valor errou valor.');
    miauw_eval_assert_same('geral', (string) ($nameAndValue['categoria'] ?? ''), 'Comando nome-valor precisa cair na categoria geral.');

    $compactNameValue = miauw_skill_gestao_command_from_message('gestao Will 50');
    miauw_eval_assert(is_array($compactNameValue), 'Comando compacto nome-valor da Gestao nao foi detectado.');
    miauw_eval_assert_same('Will', (string) ($compactNameValue['titulo'] ?? ''), 'Comando compacto nome-valor errou titulo.');
    miauw_eval_assert_same('geral', (string) ($compactNameValue['categoria'] ?? ''), 'Comando compacto nome-valor precisa cair na categoria geral.');

    $categoryFirst = miauw_skill_gestao_command_from_message('gestao - geral - Will - 50');
    miauw_eval_assert(is_array($categoryFirst), 'Comando categoria-nome-valor da Gestao nao foi detectado.');
    miauw_eval_assert_same('Will', (string) ($categoryFirst['titulo'] ?? ''), 'Comando categoria-nome-valor errou titulo.');
    miauw_eval_assert_same('geral', (string) ($categoryFirst['categoria'] ?? ''), 'Comando categoria-nome-valor errou categoria.');
...
miauw_eval_add('intent_gestao_pendente_nao_contamina_novo_prompt', static function (): void {
    ...
    $reply = miauw_try_controlled_action('gestao - 50 - Will', 1, '', true);
...
miauw_eval_add('falteiro_comando_compartilhado_case_insensitive', static function (): void {
    ...
    miauw_eval_assert(miauw_skill_falteiro_command_candidate('Miauby losartana 50 zerou'), 'Estoque zerado deveria ser candidato.');
    miauw_eval_assert(miauw_skill_falteiro_command_candidate('Miauby omeprazol 20 estoque baixo'), 'Estoque baixo deveria ser candidato.');
