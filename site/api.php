...
try {
    ...
    if ($action === 'save_row') {
        ...
        if (!cotacao_api_row_has_content($row, $prices)) {
            cotacao_json(array('ok' => true, 'empty' => true, 'message' => 'Linha vazia.'));
        }
    ...
    if ($action === 'delete_row') {
        ...
        cotacao_json(array('ok' => true, 'message' => 'Linha excluida.'));
    }

    if ($action === 'add_category') {
        $category = cotacao_add_category((int) $block['id'], (string) ($_POST['nome'] ?? ''));
        cotacao_json(array('ok' => true, 'category' => $category, 'categories' => cotacao_categories((int) $block['id'])));
    }

    if ($action === 'delete_category') {
        cotacao_delete_category((int) $block['id'], (string) ($_POST['nome'] ?? ''));
        cotacao_json(array('ok' => true, 'categories' => cotacao_categories((int) $block['id'])));
    }

    if ($action === 'add_supplier') {
        $supplier = cotacao_add_supplier((int) $block['id'], (string) ($_POST['nome'] ?? ''));
        cotacao_json(array('ok' => true, 'supplier' => $supplier));
    }
    ...
    if ($action === 'delete_supplier') {
        ...
        cotacao_json(array('ok' => true, 'message' => 'Coluna removida.'));
    }
