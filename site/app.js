document.addEventListener('DOMContentLoaded', function () {
    ...
    function saveRow(row) {
        ...
        if (!rowHasContent(payload)) {
            return;
        }

        if (!payload.produto.trim()) {
            setStatus('Aguardando produto para salvar', 'waiting');
            return;
        }
    ...
    function scheduleRowSave(row, delay) {
        if (!row) {
            return;
        }
    ...
    function deleteContextRow(row) {
        ...
        api('delete_row', { id: itemId }).then(function () {
            if (rows.length <= 1) {
                clearRow(row);
                setStatus('Linha limpa', 'saved');
                return;
    ...
    function clearFormattingForSelectedCells(options) {
        ...
        var changes = cells.map(function (cell) {
            ...
            return entry.before.color !== '' || Object.keys(entry.before.style || {}).length > 0 || entry.cell.classList.contains('is-conditional-format');
        });
    ...
    function defaultFontSizeForCell(cell) {
        ...
        if (cell && cell.classList.contains('winner-col')) {
            return 18;
        }
    ...
    function fillCellsFromPointer(clientX, clientY) {
        ...
        if (row > bounds.maxRow && col >= bounds.minCol && col <= bounds.maxCol) {
            direction = 'down';
            cells = cellsInRange(bounds.maxRow + 1, row, bounds.minCol, bounds.maxCol);
        } else if (row < bounds.minRow && col >= bounds.minCol && col <= bounds.maxCol) {
            direction = 'up';
            cells = cellsInRange(row, bounds.minRow - 1, bounds.minCol, bounds.maxCol);
        } else if (col > bounds.maxCol && row >= bounds.minRow && row <= bounds.maxRow) {
            direction = 'right';
            cells = cellsInRange(bounds.minRow, bounds.maxRow, bounds.maxCol + 1, col);
        } else if (col < bounds.minCol && row >= bounds.minRow && row <= bounds.maxRow) {
            direction = 'left';
    ...
    function updateContextMenuActions() {
        ...
        var hideRowActions = !!contextHeaderCell;
        ...
        if (addSupplierButton) {
            addSupplierButton.hidden = !!contextHeaderCell && !contextSupplierHeader;
        }
        if (deleteButton) {
            deleteButton.hidden = !supplierId;
    ...
    function applyGridFilters(options) {
        ...
        grid.querySelectorAll('tbody tr').forEach(function (row) {
            ...
            } else if (category || productColor || row.querySelector('.product-input')?.value.trim()) {
                visibleWithData += 1;
            }
        });
    ...
    function updateProductColorOptions() {
        ...
        if (productColorFilterValue && !colors.some(function (entry) {
            return entry.color === productColorFilterValue;
        })) {
    ...
    document.addEventListener('focusout', function (event) {
        if (event.target.matches('.sheet-input.is-editing')) {
    ...
    document.addEventListener('keydown', function (event) {
        ...
        if (event.key === 'F2') {
            if (enterEditCell(activeCell, true)) {
                event.preventDefault();
            }
            return;
        }
    ...
    document.addEventListener('contextmenu', function (event) {
        ...
        if (!selectedCells().length) {
            var firstCell = grid.querySelector('tbody td.sheet-cell');
            if (firstCell) {
                selectCell(firstCell, false);
            }
        }
    ...
    document.querySelectorAll('[data-add-rows]').forEach(function (button) {
        button.addEventListener('click', function () {
            addRows(Number(button.dataset.addRows || 10));
        });
    });
    ...
    document.querySelector('[data-delete-selected]')?.addEventListener('click', function () {
        ...
        Promise.all(selected.map(function (row) {
            ...
            return api('delete_row', { id: itemId }).then(function () {
                clearRow(row);
            });
        })).then(function () {
