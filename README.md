...
- Miauby Whatsapp para acompanhar canal, webhook, fila, outbox, Evolution API, Meta Cloud API e automacoes n8n de smoke/watchdog por endpoint interno tokenizado.
...
- O core compartilhado de autenticacao fica em Postgres `wimifarma_core`: `apps/core-auth` sincroniza `wf_users` para `core_users`, preservando id legado, hash, role e status. Cotacao, Gestao e Pedidos usam esse core como login principal, com fallback MySQL temporario para preservar rollback.
...
- O login da Cotacao usa `core_users` por `COTACAO_AUTH_PROVIDER=core`, com fallback `wf_users` por `COTACAO_AUTH_MYSQL_FALLBACK_ENABLED=true`; os dados novos da planilha ficam em Postgres no volume ignorado `cotacao-data/`.
...
- Pedidos fica oficialmente em `apps/pedidos`, usa Node.js/TypeScript/Express, sessao propria `WFPEDIDOS` e rota/proxy separados em `/pedidos/`. Ele usa `core_users` por `PEDIDOS_AUTH_PROVIDER=core`, com fallback `wf_users`, tabelas operacionais `pedidos_orders` e `pedidos_confirmed_orders` no Postgres da Gestao para manter a integracao financeira com `Boleto`, permite editar fornecedor/valores/vencimentos por parcela com auditoria e arquiva pedidos da tela sem apagar dados financeiros.
- Tarefa fica oficialmente em `apps/tarefa`, usa Node.js/TypeScript/Express, sessao propria `WFTAREFA`, Postgres dedicado `wimifarma_tarefa` e rota/proxy separados em `/tarefa/`. A tela foi preservada visualmente; o servico pode autenticar pelo core Postgres com `TAREFA_AUTH_PROVIDER=core`, mantendo rollback por `.env` para `mysql`, e a janela legada de importacao/espelho/log MySQL pode ser desligada por flags proprias.
