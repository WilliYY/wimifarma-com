...
- A Cotacao V2 fica em `apps/cotacao`, usa Node.js/Express/Socket.IO, Postgres e Redis, e e publicada por proxy interno do Apache em `/cotacao/`. Em 2026-05-31, a migracao para TypeScript entrou na Fase 1: `tsconfig.json`, `npm run typecheck` e devDependencies TypeScript foram adicionados sem alterar runtime, frontend, rotas, Dockerfile ou deploy; `npm start` continua `node src/server.js`.
