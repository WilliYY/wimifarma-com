...
- Desde 2026-07-24, o resultado concluido de `Gastar/Usar CashBack` aproveita melhor a largura: mostra numero da operacao e destino da impressao, organiza cliente, codigo, telefone, cashback gerado e validade por prioridade visual, mantem as acoes no rodape e deixa a previa termica ao lado. O formulario tambem oferece `Concluir e imprimir`, que confirma a operacao antes de acionar uma unica vez a mesma rota inteligente do botao `Imprimir`; atualizar a pagina nao repete a solicitacao. A mudanca e responsiva e nao altera saldo, XP, codigo ou validade.
...
- Desde 2026-07-24, `Gastar/Usar CashBack` usa somente o calculo automatico sobre o valor efetivamente cobrado e nao aceita mais `Cashback Manual`; o registro simples de compra preserva essa opcao administrativa e os creditos historicos nao sao alterados. O detalhe do cliente mostra dias restantes por credito e permite excluir do saldo apenas cashback gerado ainda intacto, por cancelamento logico/auditoria. Cada uso real de cashback gera +500 XP para o usuario logado vinculado ao XP, com idempotencia por resgate e sem bloquear a compra se o XP estiver indisponivel.
...
- Desde 2026-07-21, o card ADM `Wimi Impressora` disponibiliza um unico instalador Windows para o computador USB da Bematech MP-4200 TH. O agente instala servico com inicio automatico, pareia sem digitacao de codigo, recebe a fila central por HTTPS e atualiza sozinho com verificacao SHA-256. Nos comprovantes, o unico botao `Imprimir` escolhe a fila Wimi quando ha agente online ou abre a impressao local quando nao ha; falhas incertas nao caem automaticamente no navegador, para nao duplicar papel. Desde 2026-07-24, o rodape termico foi ampliado para leitura no balcao: validade, WhatsApp, endereco, atendente e data/hora usam fontes maiores e em negrito. O agente `1.0.8` tambem imprime o codigo permanente, destaca o nome salvo e amplia o valor de `Cashback gerado` nos comprovantes de clientes identificados; o papel mostra somente esse valor como informacao financeira. O agente usa a versao do proprio assembly, cultura explicita e logs precompilados, atualiza com troca atomica/rollback e compila com analisadores sem warnings. Arquitetura, seguranca, build e operacao ficam em `docs/31-wimi-impressora.md`.
