...
- A Encomenda pelo Miauby tambem e contextual: separa a identidade completa do produto de cliente, telefone, quantidade do pedido, endereco, retirada/entrega, data, horario, prioridade, referencia e observacao; exige confirmacao e grava de forma idempotente somente `Produto` + `Categoria` na Cotacao.
...
- Em 2026-08-23, o prefixo falado do audio do WhatsApp ganhou normalizacao fonetica conservadora: transcricoes iniciadas por `Miau bi`, `Miau bis`, `Miaubi` ou `Miauw` sao tratadas como `Miauby`, inclusive quando o nome vem repetido, sem liberar `miau` isolado nem audio sem ativacao quando o ambiente exige prefixo. Texto digitado continua com a regra exata; cards, permissoes e confirmacoes nao mudaram.
