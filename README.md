...
- O backend WhatsApp possui endpoints internos para n8n/pos-deploy (`/miauw/whatsapp/internal/smoke-check` e `/miauw/whatsapp/internal/watchdog`) e recuperacao de outbox: pendencias recentes podem ser reenviadas automaticamente, enquanto pendencias antigas viram `dead` para nao disparar mensagem fora de contexto depois de queda ou deploy. O painel nao conta `dead/stale_pending_expired` no card `Problemas`, pois isso e expiracao segura de pendencia velha, nao erro atual; tambem nao lista logs `info` de automacao bem-sucedida, avisos de outbox recuperado nem tentativas transitorias de fila que ja terminaram com resposta enviada.
