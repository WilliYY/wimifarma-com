...
- A logo oficial foi atualizada em 2026-05-21 como SVG horizontal e esta sincronizada nos assets compartilhados de Cashback/Codigos/Gestao/Pedidos, Financeiro, Tarefa, Miauw e Cotacao V2. Em 2026-05-24, a home publica passou a usar `logo-wimifarma-home-animated.gif` como variacao animada sem fundo da marca, substituida em 2026-06-08 pelo SVG animado atual.
- Desde 2026-06-08, a Home usa `logo-wimifarma-home-animated.svg` como variacao animada sem fundo da marca, aplicada no login central, no rodape do login e no header autenticado; o asset versionado remove a camada branca e os pixels brancos internos do SVG fornecido, usa `mix-blend-mode: multiply` e microanimacao CSS no elemento para evitar retangulo branco e manter movimento legivel no ciclo mobile/desktop, com cache-bust em `site/home.php`.
...
- Desde 2026-06-08, o Pix CNPJ por midia tenta OCR com Gemini primeiro e usa fallback OpenAI quando houver cota/spending cap, 429, timeout, 5xx ou resposta vazia do provedor primario. O container `wimifarma-miauw-whatsapp` recebe `MIAUW_OPENAI_API_KEY`/`MIAUW_OPENAI_MODEL` e tambem aceita sobrescritas `MIAUW_WHATSAPP_OPENAI_*`; a midia segue somente em memoria e a validacao de destino, valor, pagador, confirmacao e Financeiro permanece igual. O painel mostra o provedor `gemini/openai-fallback` e o modelo de fallback para diagnostico.
