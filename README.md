...
Desde 2026-08-16, o mesmo servico expoe `POST /miauw/agent/interpret`, protegido pelo token interno e sem efeitos colaterais. Miauby Interno e Miauby WhatsApp consultam essa camada antes dos parsers operacionais para identificar intencao, entidades e parametros na frase inteira, sem depender da ordem ou de maiusculas/minusculas. Um resultado `resolved` fornece uma mensagem canonica compativel com os parsers existentes; `ambiguous` pede somente a desambiguacao e bloqueia qualquer escrita; `none` ou indisponibilidade preserva o formato legado. Os timeouts podem ser ajustados por `MIAUW_AGENT_INTERPRETER_TIMEOUT_MS` no PHP e `MIAUW_WHATSAPP_AGENT_INTERPRET_TIMEOUT_MS` no bridge.
