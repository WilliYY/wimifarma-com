...
- A Cotacao V2 removeu os botoes visiveis de adicionar linhas e colar planilha: inserir/remover linhas fica no menu de contexto e colagem do Sheets usa `Ctrl+V`. A tela tambem possui desfazer/refazer, selecao multipla, filtros por icone em `CATEGORIA` e `Ganhador`, backup/restore do Postgres e import/export Google Sheets controlado por variaveis de ambiente; o diagnostico operacional continua disponivel por API, mas saiu do menu principal da equipe.
...
- A operacao diaria tambem cobre `Ctrl+C` em selecao de celulas, `Ctrl+Z` para desfazer filtros/busca local, colagem com normalizacao de texto/numeros, formato condicional que pinta apenas a celula da coluna-alvo com texto preto, paleta de cores flutuante pelo topo ou menu de contexto, manutencao da linha visivel durante edicao sob filtro e heartbeat/recarregamento leve apos inatividade.
...
- O widget do Miauby voltou a carregar dentro da Cotacao V2 e a tela de login foi compactada para ocupar menos a tela.
- Pendencias/cuidados atuais da Cotacao V2: Google Sheets ainda precisa de credenciais reais no `.env` do VPS; restore/import sao acoes fortes e devem ser usados com backup/revisao; o `fill handle` visual existe, mas arrastar para preencher igual Sheets ainda pode evoluir.
