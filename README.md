...
- Miauby tambem entende comandos controlados da Gestao: `gestao` aponta para `/gestao/`, e `gestao - titulo - 500 - categoria` ou `gestao - 500 - titulo - categoria` prepara uma conta a pagar com confirmacao humana antes de gravar pelo endpoint interno tokenizado da Gestao. Se um comando incompleto pedir correcao, uma nova mensagem iniciada por `gestao` substitui a pendencia anterior em vez de juntar prompts antigos.
