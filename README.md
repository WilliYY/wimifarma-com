...
- O canal WhatsApp do Miauby iniciou como backend dedicado em `apps/miauw-whatsapp`, usando Node.js 22 + TypeScript e Postgres 17 proprio. O servico publica `/miauw/whatsapp/` por proxy Apache, recebe webhooks da Evolution API ou da Meta Cloud API oficial, usa allowlist, prefixo opcional, fila duravel, dedupe, painel operacional com login opcional por `.env`, favicon proprio do Miauby e outbox. O repositorio mantem default seguro `MIAUW_WHATSAPP_ENABLED=false`; em producao o canal pode ser ativado por `.env` quando tokens/cifragem estiverem configurados. A stack separada da Evolution API tem template em `ops/evolution/` e roda no VPS em `/home/ubuntu/projetos/wimifarma-evolution-api`; para Meta, usar `MIAUW_WHATSAPP_PROVIDER=meta`.
