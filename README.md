...
- Em 2026-06-02, a migracao do Miauby interno avancou sem corte: `site/miauw` pode consultar o contexto canônico do `apps/miauby` em sombra por `MIAUBY_CONTEXT_SHADOW_ENABLED`, gravando apenas trace sanitizado com versoes/contagens. A resposta oficial continua PHP, e `write_enabled`, `route_cutover_enabled` e `public_proxy_enabled` continuam falsos por seguranca.
...
- O backend WhatsApp possui endpoints internos para n8n/pos-deploy (`/miauw/whatsapp/internal/smoke-check` e `/miauw/whatsapp/internal/watchdog`) e recuperacao de outbox: pendencias recentes podem ser reenviadas automaticamente, enquanto pendencias antigas viram `dead` para nao disparar mensagem fora de contexto depois de queda ou deploy. O painel nao conta `dead/stale_pending_expired` no card `Problemas`, pois isso e expiracao segura de pendencia velha, nao erro atual; tambem nao lista como erro aberto avisos de outbox recuperado nem tentativas transitorias de fila que ja terminaram com resposta enviada.
- O n8n tambem pode chamar `POST /miauw/whatsapp/internal/pedidos-arrival-check` todo dia as 17h. O bridge consulta Pedidos, envia a tabela de `Aguardando chegada` para contatos autorizados com card `Pedidos`, usando o valor total do pedido, previsao de chegada quando houver e a data/hora persistida em `pedidos_orders.created_at`, ordenando os mais antigos primeiro; o painel mostra a rotina `Chegada de pedidos` com botao de pausar/ativar. Perguntas como `miauby em pedidos o que falta chegar` retornam a mesma tabela. Respostas como `cimed chegou` confirmam apenas a chegada pelo endpoint interno de Pedidos e deixam o boleto em `Confirmados` para pagar.
...
Para iniciar ou atualizar a migracao sombra do Miauby interno, definir `MIAUBY_POSTGRES_PASSWORD` por ambiente, subir `wimifarma-com-db` e `wimifarma-miauby-db`, e rodar `sh scripts/miauby-shadow-migrate.sh migrate` seguido de `sh scripts/miauby-shadow-migrate.sh validate`. Esse script prefere executar o migrador dentro de `wimifarma-miauby-app` quando ele ja esta rodando e usa `docker compose run --rm --no-deps` apenas como fallback, evitando recriar `wimifarma-com-db` em ambientes com containers antigos/labels de Compose diferentes. O migrador de `apps/miauby` copia tabelas `miauw_*` para tabelas `miauby_*` com `legacy_mysql_id`, checksum e payload sanitizado. `MIAUBY_INTERNAL_TOKEN` protege os endpoints internos do `wimifarma-miauby-app`; quando vazio, o servico pode reutilizar `MIAUW_GUARDIAN_TOKEN` ou `MIAUW_AGENT_INTERNAL_TOKEN`. O app responde apenas na rede Docker em `wimifarma-miauby-app:4100`, sem proxy Apache/publico, e expoe `/miauby/health`, `/miauby/api/internal/status`, `/miauby/api/internal/parity`, `/miauby/api/internal/readiness`, `/miauby/api/internal/context`, `/miauby/api/internal/cutover` e `/miauby/api/internal/canonical-context` para validar paridade, ler amostras sanitizadas e montar contexto/persona/tools read-only antes de qualquer troca oficial. O alias interno `/miauby/api/internal/context-pack` aponta para o mesmo pacote canonico. O smoke automatizado fica em `scripts/miauby-shadow-smoke.sh`.
