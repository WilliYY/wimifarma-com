...
- Desde 2026-05-27, o bridge WhatsApp pode ler imagem de comprovante Pix autorizado quando `MIAUW_WHATSAPP_PIX_RECEIPT_IMAGE_ENABLED=true`. Ele baixa a imagem somente em memoria, extrai dados com Gemini, confere o CNPJ destino `MIAUW_WHATSAPP_PIX_RECEIPT_CNPJ`, prepara um lancamento `Pix CNPJ` no Financeiro e exige confirmacao `Sim`/`Nao`; o banco nao guarda imagem bruta nem URL/token de midia.
