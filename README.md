...
- A Cotacao V2 fica em `apps/cotacao`, usa Node.js/Express/Socket.IO, Postgres e Redis, e e publicada por proxy interno do Apache em `/cotacao/`. Em 2026-05-31, a migracao para TypeScript iniciou a Fase 3 com helpers TS sombra em `apps/cotacao/src/utils/normalizers.ts`, `styles.ts` e `winner.ts`, depois de `tsconfig.json`, `npm run typecheck`, devDependencies TypeScript e contratos em `apps/cotacao/src/contracts/`; nada disso altera runtime, frontend, rotas, Dockerfile ou deploy, e `npm start` continua `node src/server.js`.
