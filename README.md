...
- Desde 2026-07-21, o card ADM `Wimi Impressora` disponibiliza um unico instalador Windows para o computador USB da Bematech MP-4200 TH. O agente instala servico com inicio automatico, pareia sem digitacao de codigo, recebe a fila central por HTTPS e atualiza sozinho com verificacao SHA-256. Nos comprovantes, o unico botao `Imprimir` escolhe a fila Wimi quando ha agente online ou abre a impressao local quando nao ha; falhas incertas nao caem automaticamente no navegador, para nao duplicar papel. Desde 2026-07-24, o rodape termico foi ampliado para leitura no balcao: validade, WhatsApp, endereco, atendente e data/hora usam fontes maiores e em negrito. O agente `1.0.7` tambem imprime o codigo permanente e destaca o nome salvo nos comprovantes de clientes identificados; o papel mostra somente `Cashback gerado` como informacao financeira. O agente usa a versao do proprio assembly, cultura explicita e logs precompilados, atualiza com troca atomica/rollback e compila com analisadores sem warnings. Arquitetura, seguranca, build e operacao ficam em `docs/31-wimi-impressora.md`.
