...
- Entrega para cadastro rapido com responsavel selecionavel, previa termica ao vivo, comprovante com marca fixa, comissao por responsavel, pagamento resumido e historico auditado;
- Indicacao para campanhas com cupons termicos, comissao financeira do indicador e +300 XP por uso confirmado;
...
- Desde 2026-08-22, Miauby Interno e Miauby WhatsApp registram produtos no Falteiro da Cotacao por intencao natural de falta, estoque baixo, compra ou reposicao, sem depender de ordem, caixa ou sintaxe fixa. Uma mensagem pode conter ate 50 itens separados por virgula, ponto e virgula, quebra de linha, marcadores, numeracao e conectores naturais; no WhatsApp, a extracao, a fila e a memoria conversacional preservam essas quebras ate o parser. A intencao e herdada, enquanto produto, categoria e contexto sao interpretados por item. A segmentacao conservadora preserva kits e nomes compostos, usa produtos conhecidos apenas como ancora segura e mantem contexto local ou coletivo no item correto. `Produto` preserva identidade, dosagem, apresentacao, embalagem e marca obrigatoria; `Categoria` combina o rotulo oficial mais especifico com estoque, quantidade de compra, prazo, preco, preferencia, demanda e observacao. Negacao da acao bloqueia, restricao negativa e preservada, demanda geral vai ao Falteiro e reserva individual segue para Encomenda. A gravacao reserva, numa unica transacao, somente as primeiras linhas integralmente vazias, sem qualquer valor em EAN, produto, quantidade, categoria ou fornecedor; nunca sobrescreve linhas parcialmente preenchidas e nao conclui parcialmente.
- A Encomenda pelo Miauby tambem e contextual: separa a identidade completa do produto de cliente, telefone, quantidade do pedido, endereco, retirada/entrega, data, horario, prioridade, referencia e observacao; exige confirmacao e grava de forma idempotente somente `Produto` + `Categoria` na primeira linha integralmente vazia da Cotacao, sem criar linha nova no fim nem sobrescrever linha parcialmente preenchida. Expressoes como `valor 57` ficam na observacao/categoria e nao contaminam o nome do cliente.
- Desde 2026-08-24, perguntas simples de Encomenda, inclusive `quais encomenda tem?` e `o que tem de encomenda e tals?`, entram na leitura deterministica antes da interpretacao semantica e podem listar sem o prefixo digitado. Essa excecao e somente de leitura, exige contato permitido e card `Cotacao`; criacao, alteracao e demais comandos digitados continuam exigindo `Miauby`. Vicios de fala `e tal`/`e tals` nao viram filtro de produto. `sim`/`nao` sem confirmacao forte pendente continuam a conversa, sem executar escrita antiga. A memoria curta e o resumo persistente duram 24 horas renovaveis (`86400` segundos), enquanto a validade de uma acao pendente permanece em 5 minutos (`300` segundos).
...
- Desde 2026-08-27, a Cotacao revalida usuario ativo e permissao no core em cada requisicao e periodicamente no Socket.IO; o healthcheck depende de Postgres, Redis e core, mutacoes de linha da interface ficam atomicas com seus eventos e lotes de celula usam ordem deterministica de locks. O VPS gera dump Postgres diario validado, com SHA-256 e retencao local de 30 dias em `cotacao-data/automatic-backups`; a copia externa continua pendente.
...
- Desde 2026-06-09, o menu de filtro da Cotacao suporta listas grandes de categoria/cor com corpo rolavel e botoes de aplicar/limpar sempre visiveis, sem alterar filtros locais, undo ou dados salvos.
...
- A Gestao fica oficialmente em `apps/gestao`, usa Node.js/TypeScript/Express com Postgres dedicado `wimifarma_gestao`, e e publicada por proxy interno do Apache em `/gestao/`; o login usa somente `core_users`, sem fallback `wf_users`, sem espelho `wf_logs`, sem `mysql2` e sem variaveis `MYSQL_*` no servico. A ponte Miauby -> Gestao tambem usa somente endpoints internos tokenizados e auditoria Postgres, sem `wf_logs` no contrato da tool `criar_conta_gestao`. A tela principal usa linhas compactas em `Categoria / Nome / Valor / Pagar`, abre detalhes ao clicar na linha inteira, coloca contas com repeticao ativa primeiro e mostra simbolo de repeticao nelas. Desde 2026-06-06, o `Pagar` compacto da lista principal registra o pagamento pelo fluxo existente sem abrir confirmacao nativa do navegador. O painel `Mensal` fica ao lado da lista principal, com as contas de repeticao ativa reordenaveis por arrastar; contas marcadas como `Repetir sempre mes que vem` ficam priorizadas no topo, continuam no painel mesmo pagas e garantem a proxima copia ao quitar sem duplicar pagamento, ajustando vencimentos de fim de mes para o ultimo dia valido quando necessario. Ainda em 2026-06-06, os blocos `Nova conta`, `Contas abertas`, `Mensal`, `Pedidos` e `Categorias` ganharam separacao visual por acento/cor para leitura mais rapida, sem mudar escrita, banco ou integracoes. Desde 2026-06-08, o formulario `Nova conta` usa grid mais respiravel para competencia/status/vencimento e itens, evitando sobreposicao visual sem alterar entradas, POSTs, CSRF, banco ou integracoes. Desde 2026-07-13, o cadastro manual nao exibe mais campo nem previa de categoria: a categoria e inferida automaticamente pelo `Nome ou titulo`; no mesmo card, a competencia tambem saiu da tela, o vencimento entra como numero de dias ate vencer, o status inicial virou botoes `Pendente`/`Pago` e a repeticao mostra previa apenas quando ativa, mantendo a competencia gravada pelo mes selecionado e persistindo a data final calculada. Desde 2026-06-11, no desktop, `Nova conta`, `Contas abertas`, `Mensal`, `Pedidos` e `Categorias` ficam em uma faixa horizontal livre e o proprio navegador oferece rolagem lateral quando nao couber; no mobile os blocos continuam empilhados. Desde 2026-06-12, o antigo bloco `Notas` saiu da Gestao e virou o modulo `/notas/`. Observacoes preservam quebras de linha no card aberto.
- Ainda em 2026-06-11, o bloco `Pedidos` dentro da Gestao nao usa mais rolagem interna nem limite de altura, deixando todos os pedidos aparecerem no fluxo da pagina. O painel `Categorias` mostra resumo de abertas/fechadas com valores e linhas por categoria, mantendo filtros, acoes em lote, POSTs, CSRF, banco e auditoria.
- Ainda em 2026-06-11, a busca da Gestao foi ampliada para texto livre combinavel, com normalizacao de espacos, acentos e caixa; ela considera nome, fornecedor/distribuidor, responsavel, categoria, tipo/origem, valor, vencimento/data, observacao, parcelas, status e Pedidos vinculados, mostrando uma linha-resumo por resultado sem alterar dados.
- O Balcao do Cashback mostra `Gastar/Usar Cashback` em blocos de cliente, compra atual, resumo financeiro e acao para facilitar leitura, mantendo o mesmo POST `save_redeem`, CSRF, calculo automatico de uso/novo cashback e selecao server-side de um usuario Wimifarma elegivel.
- Desde 2026-07-25, `Gastar/Usar CashBack` possui uma unica acao `Usar CashBack e imprimir`: a operacao confirma a transacao antes de abrir uma unica vez a impressao local. O usuario responsavel e obrigatorio, preseleciona o login atual, pode ser trocado entre atendentes elegiveis e recebe +250 XP somente quando houve cashback realmente usado. Atualizar a pagina nao repete compra, resgate, credito, XP ou solicitacao automatica de impressao.
...
- Desde 2026-07-24, `Gastar/Usar CashBack` usa somente o calculo automatico sobre o valor efetivamente cobrado e nao aceita mais `Cashback Manual`; o registro simples de compra preserva essa opcao administrativa e os creditos historicos nao sao alterados. O detalhe do cliente mostra dias restantes por credito e permite excluir do saldo apenas cashback gerado ainda intacto, por cancelamento logico/auditoria. Desde 2026-07-25, cada uso real de cashback gera +250 XP para o usuario responsavel escolhido, com idempotencia por resgate; o usuario da sessao permanece como ator da auditoria e falha do XP nao bloqueia a compra.
- Desde 2026-07-21, o inicio do Balcao oferece `Cashback rapido`: valor gasto, previa do percentual padrao, atendente que imprime e emissao de codigo aleatorio com validade de seis meses. Desde 2026-08-12, codigos novos possuem cinco digitos na faixa exclusiva `00000` a `49999`; codigos historicos de quatro digitos e de cinco digitos fora dessa faixa continuam aceitos enquanto validos. Uma combinacao usada ou cancelada permanece reservada ate sua data original de vencimento e so pode ser sorteada novamente depois dos seis meses, sob lock transacional. O codigo nao precisa de cliente na emissao, mas exige cadastro/selecao no uso; o primeiro uso pode ocorrer junto ao `Novo cliente`, e os seguintes usam `Gastar/Usar Cashback`. O resgate integral preserva a regra 4x e gera o proximo codigo sobre o valor cobrado depois do desconto. O recibo compacto para rolo de 80 mm usa a logo oficial em PNG preparada em preto sobre branco, mostra a data real de vencimento e abre o dialogo da impressora do computador; a web nao seleciona silenciosamente a Bematech MP-4200 TH.
- Desde 2026-07-29, `Novo cliente` e `Gastar/Usar CashBack` aceitam ate 10 codigos de Cashback rapido na mesma compra pelo botao `+ Adicionar`. O servidor soma os beneficios, aplica a regra 4x sobre o total, trava e consome todos os vouchers na mesma transacao e gera uma unica compra, um unico resgate, um unico sucessor e um unico comprovante; se qualquer codigo falhar, nenhum deles e usado. Formularios antigos que enviam somente um `codigo_cashback` continuam compativeis.
- Desde 2026-07-24, emitir `Cashback rapido` gera +250 XP para o usuario escolhido em `Usuario que imprime`, desde que ele esteja vinculado ao XP. O lancamento usa `xp_sales.source='cashback_quick_voucher_issue'` e o ID do voucher em `source_entity_id`, impedindo pontuacao duplicada em reenvio do formulario; falha ou ausencia de vinculo XP nao desfaz o codigo ja emitido.
- O Balcao exibe os 20 vouchers rapidos mais recentes com situacao do codigo, valores, atendente, validade, impressao, cliente e XP. Desde 2026-07-28, o historico tambem mostra um resumo por usuario com a quantidade total de emissoes rapidas diretas cadastradas, estados, cashback emitido e ultimo cadastro; sucessores gerados durante resgates nao contam nesse placar. O login mestre `adm` pode cancelar somente codigos ativos; o cancelamento e logico, preserva auditoria e reserva o codigo ate a validade original. Os 250 XP vinculados tambem sao cancelados logicamente, com acao de correcao disponivel caso o banco XP esteja temporariamente indisponivel.
- Desde 2026-07-28, `Dados do cashback rapido` e `Historico do cashback rapido` sao blocos separados: os dados por usuario ficam abertos por padrao e o historico dos codigos inicia recolhido, abrindo automaticamente depois de acoes administrativas que precisam de conferencia.
- Desde 2026-07-24, quando um `Cashback rapido` e confirmado, o Balcao aproveita a area ao lado do recibo com um resumo compacto de status, destino da impressao, codigo, valor, validade e emissor, deixando `Imprimir` e `Gerar outro` mais evidentes. O formulario de proxima emissao tambem ganhou hierarquia visual e adaptacao para tablet/celular, sem alterar geracao, impressao, CSRF, XP, codigo ou validade.
- Desde 2026-07-25, o card e a estacao `Wimi Impressora` foram retirados. Os comprovantes de Cashback usam somente a impressao local do navegador, mantendo o layout termico de 80 mm e a auditoria de solicitacao. Dispositivos e trabalhos antigos permanecem no Postgres apenas como historico; dispositivos ativos sao revogados e filas incompletas sao canceladas para nao imprimir depois.
- Desde 2026-06-12, a disposicao visual do bloco `Gastar/Usar Cashback` ficou mais alinhada: campos da compra e cards de resumo usam grade elastica por largura real do bloco, mantendo responsividade e todos os nomes de campos/POSTs/calculos intactos.
...
- Desde 2026-07-24, o `Novo cliente` usa largura operacional maior no desktop, campos distribuidos em quatro colunas para identificacao e tres para compra inicial quando houver espaco, secoes planas com bordas de 8 px, resumo financeiro de maior contraste e acao principal em largura total. Tablet e celular reduzem para duas ou uma coluna sem alterar inputs, CSRF, `save_client`, calculos ou banco.
- Desde 2026-07-24, o ID do Postgres de `cashback_clients` e exibido como `Codigo do cliente`: ele e unico, permanente e pode ser digitado com ou sem `#` na busca e no seletor de `Gastar/Usar CashBack`. Quando o cadastro em `Novo cliente` inclui compra inicial com cashback gerado, o resultado aparece no proprio card com recibo e botao `Imprimir`; o papel identificado leva nome, telefone e codigo do cliente. O recibo anonimo de `Cashback rapido` permanece sem codigo de cliente.
- Desde 2026-07-25, `Novo cliente` possui um unico botao `Cadastrar e imprimir`. O atendente responsavel pode ser escolhido entre usuarios Wimifarma elegiveis e e validado novamente pelo servidor; a conta escolhida recebe +250 XP depois que o cliente e confirmado, com idempotencia por `cashback_clients.id`. Se a compra inicial gerar cashback, o mesmo clique abre o comprovante para impressao local; sem cashback, somente o cadastro e o XP sao concluidos. Falha no XP ou na abertura do dialogo nao reverte nem duplica o cliente.
- Desde 2026-07-28, esse cadastro pode ser concluido com apenas nome ou apenas telefone. O navegador e o backend exigem pelo menos um identificador; quando o nome nao e informado, o registro recebe o nome operacional `Cliente <telefone>` para manter busca, recibos e o contrato `cashback_clients.name NOT NULL`.
- A tela `Mensagens` do Cashback mostra cards de WhatsApp pendentes com campanha, cliente, telefone/detalhe, mensagem completa e acoes; desde 2026-06-07, indicadores, abas, secoes e cards ficaram mais compactos para caber mais fila por tela. Desde 2026-06-08, os cards tambem usam acento por campanha, metadados de criacao/data e largura operacional maior, mantendo o mesmo backend. Abrir WhatsApp, copiar texto ou excluir atualiza o status salvo e remove o card da fila visual, sem apagar o historico. Desde 2026-06-16, ao abrir a tela, mensagens pendentes de compra, aniversario e expiracao com `due_date` ja passado saem da fila ativa como `expirado_da_fila`, preservando historico e dedupe.
...
- Em 2026-06-02, o widget global do Miauby refinou o enquadramento visual usando `site/miauw/miauby-avatar.jpeg`, derivado quadrado de `site/miauw/miauby-novo.jpeg`: o cabecalho e as mensagens mostram o avatar maior/proporcional por CSS central em `site/miauw/widget.css`, sem alterar chat, audio, sessao ou backend. Em 2026-06-10, o botao flutuante deixou de ser circular e passou a usar o video `site/miauw/miauby-widget-loop.mp4` em loop dentro de uma caixa quadrada arredondada, mantendo o label `Miauby`, falas da Home, clique para abrir, painel, audio, sessao e backend; cache-bust atual `20260610-miauby-video`.
...
- Desde 2026-06-09, a Home usa `logo-wimifarma-home-animated.svg` como variacao animada em loop, sem fundo, aplicada no login central, no rodape do login e no header autenticado. A animacao enviada pelo usuario foi preservada e corrigida com filtro SVG interno para transformar os PNGs recortados de preto para branco e remover o fundo branco como transparencia, mantendo preload `image/svg+xml`, prioridade/cache-bust em `site/home.php`, proporcao `1024/400` e sem `mix-blend-mode`. Ainda em 2026-06-09, o SVG foi otimizado sem recompressao: o PNG interno repetido passou a existir uma vez em `logo-raster-source`, reaproveitado por `<use>` nos mesmos recortes para reduzir peso sem mudar detalhes, qualidade ou loop.
- Desde 2026-07-15, o tema WordPress `wimifarma-cashback-theme` inclui a vitrine publica `Melhor oferta`, com shortcode `[wimifarma_melhor_oferta]`, 15 cards em grade 5x3 no desktop e posicionamento automatico na home WordPress entre o bloco de video/hero e o banner quando o conteudo da pagina vier do banco.
...
- A home publica mostra no maximo cinco cards por linha no desktop, na ordem `Cashback`, `Cotacao`, `Pedidos`, `Financeiro`, `Bloco de notas/lembretes`, `Tarefas`, `Codigos`, `Calendario`, `XP`, `Gestao`, `Miauby`, `Miauby Whatsapp`, `Login / Senha` e `Usuarios`; `Usuarios` fica por ultimo quando todos estiverem visiveis. O card `Miauby` usa a rota canonica `/miauby/` e o card `Miauby Whatsapp` usa `/miauby/whatsapp/`, mantendo `/miauw/*` como compatibilidade. `Pedidos` mostra badge com o total ainda em `Aguardando chegada`, o card `XP` usa moldura propria como borda/cantos por `border-image`, sem cortar a arte nem cobrir o texto, enquanto os demais cards seguem em grade compacta. Quando a home tem `WFHOME_SSO` valido ou o navegador tem sessao ativa no XP/Usuarios, e o login esta vinculado em `core_user_xp_links`, a home mostra no desktop um mini-card do XP acima da grade, lendo os totais atuais de `xp_sales` e atualizando por polling curto; a grade ajusta colunas conforme a quantidade de cards visiveis pelo filtro de permissoes. No mobile os cards ficam em duas colunas para caber mais acessos por tela.
- Desde 2026-06-03, o login da Home autentica em `core_users` ativo, normalizando o login para `username_normalized` em minusculo e conferindo `password_hash` bcrypt. Login com caixa diferente (`thiago`, `Thiago`, `THIAGO`) entra no mesmo usuario canonico; desde 2026-09-01, a credencial paralela `WIMIFARMA_HOME_LOGIN_USER`/`WIMIFARMA_HOME_LOGIN_PASSWORD` e o fallback fixo `adm`/`adm` foram removidos.
- O modulo `Cashback` fica oficialmente em `apps/cashback`, usa Node.js/TypeScript/Express com Postgres `wimifarma_cashback`, sessao propria `WFCASHBACK`, login unico por `core_users` e proxy Apache em `/cashback/`. A tela preserva o CSS/JS/assets de `site/cashback`, enquanto PHP direto nessa pasta fica bloqueado por `.htaccess`; desde 2026-06-04, os PHPs antigos de tela/API/diagnostico que consultavam `wf_*` ficam arquivados em `site/_legacy-disabled/2026-06-04/cashback-php/`, e `site/cashback/functions.php` e apenas bootstrap compartilhado minimo para o Miauby. Desde 2026-05-30, a paridade com `wf_*` foi validada e o app nao possui mais `mysql2`, importador, espelho, logs ou fallback de autenticacao MySQL. Cada novo credito de cashback fica vinculado a compra que o gerou e expira 45 dias apos `cashback_purchases.purchased_at::date`, sem zerar outros creditos validos do cliente. Desde 2026-06-03, compra/resgate do Cashback resolve o responsavel pelo usuario logado da sessao core, vinculando `cashback_attendants.core_user_id` e preenchendo/travando o campo Atendente para nao registrar operacao como `Sem atendente` ou em nome de outro usuario comum. Desde 2026-06-06, o Balcao mostra os ultimos clientes alterados, 5 visiveis por padrao e `Mostrar mais` em blocos de 5; a busca do Balcao e do autocomplete filtra clientes ativos e normaliza telefone/ID para evitar resultados gerais quando a busca textual nao tem numeros. Desde 2026-06-16, cards do Balcao, resumo lateral, cliente selecionado, busca viva e seletor mostram o saldo ativo dividido por data de vencimento, por exemplo `R$ 2,00 em 18/06/2026 (faltam 2 dias)`, lendo apenas creditos ativos, com saldo restante, nao cancelados e ainda nao vencidos. O cadastro rapido `Novo cliente` no Balcao usa blocos visuais separados para dados do cliente e compra inicial opcional, sem alterar campos, calculo, CSRF ou POST. A fila `Cashback vencendo em ate N dias` mostra somente creditos ativos ainda nao vencidos, separados por data de vencimento, e creditos vencidos seguem o status `expirado` existente sem apagar historico. A fila `Clientes com saldo e sem compra recente` mantem uma pendencia de recompra por cliente e ultima compra por ate 14 dias, arquivando com `expirado_da_fila` quando envelhece; `Excluir da fila` usa `cancelada` e tambem impede retorno imediato, sem apagar cliente, saldo ou historico. Desde 2026-06-16, mensagens pendentes de compra, aniversario e expiracao com data passada tambem saem da fila ativa como `expirado_da_fila` ao abrir `Mensagens`, preservando historico e dedupe. Desde 2026-06-06, o atalho visual `Diagnostico` saiu do topo do Cashback; a rota tecnica permanece acessivel por URL para manutencao. Ainda em 2026-06-06, a navegacao superior do Cashback destaca a pagina/secao ativa, e `Configuracao e Relatorio` ganhou layout compacto com atalhos internos, cards menores, lista de atendentes com rolagem propria e microanimacoes CSS, sem alterar escrita, relatorios CSV ou permissao. Rollback agora exige restaurar commit/imagem anterior e backup, nao trocar `.env`.
- Desde 2026-07-21, a equipe operacional do Cashback e derivada de contas humanas reais do modulo Usuarios: novo atendente exige uma conta `core_users` ativa com acesso ao Cashback, o perfil institucional `farmacia` fica fora da equipe, nome/login sao sincronizados pelo core, a conta atualmente logada fica destacada e atendentes legados sem `core_user_id` deixam de aparecer nos seletores e na gestao diaria sem perder o historico financeiro.
- Desde 2026-07-28, `Configuracao e Relatorio` usa melhor a largura operacional: o aviso de equipe totalmente vinculada vira uma faixa compacta, cards de atendentes ficam mais legiveis, indicadores ganham agrupamento visual e os downloads CSV aparecem em cards maiores, sem alterar formularios, permissoes, filtros, exportacoes ou auditoria.
- Desde 2026-07-21, o atalho superior do Cashback usa o nome curto `Gastar/Usar CashBack`. Depois de concluir essa operacao, o Balcao apresenta um resumo de sucesso e um comprovante termico da compra confirmada com logo, nome, telefone, compra, cashback usado, valor pago, novo cashback, validade ou novo codigo rapido, atendente e data. `Imprimir comprovante` isola somente o papel de 76 mm no dialogo do navegador e audita a solicitacao por um ID de compra autorizado na sessao, sem gerar nova compra, credito ou resgate.
- Desde 2026-07-21, a regra anterior de 45 dias descrita acima foi substituida: cashback comum, manual, voucher rapido e sucessores vencem seis meses corridos apos a geracao. Vouchers rapidos ainda ativos com prazo antigo sao ajustados para `issued_at + 6 meses`; vouchers usados/cancelados e creditos comuns historicos permanecem como gravados. Todos os comprovantes usam espacos verticais menores, mostram a data real de vencimento e nao imprimem a frase de instrucao para informar o codigo.
- Ainda em 2026-07-21, os cards operacionais do Balcao ganharam contraste e cores funcionais consistentes: cliente/cadastro em vermelho, compra em azul, cashback/saldo em verde e validade em amarelo, sem alterar formulários, seletores JS, POSTs, CSRF, calculos ou banco.
- Ainda em 2026-07-21, ao abrir `Gastar/Usar CashBack`, o Balcao usa a largura operacional disponivel e remove somente o resumo lateral duplicado dessa secao. Campos e indicadores ficam em grade responsiva de ate cinco colunas, a validade de seis meses aparece no resumo financeiro e o codigo rapido usa autocomplete de codigo temporario para evitar sugestoes de endereco, preservando formulario, calculos e transacao.
- Ainda em 2026-07-21, a secao inicial `Balcao` tambem usa a largura operacional disponivel e oculta o resumo lateral vazio/duplicado enquanto `#busca` esta ativo. `Cashback rapido`, pesquisa e cards de clientes ganharam cores funcionais mais claras; em desktop largo aparecem dois clientes por linha, em notebook uma linha ampla por cliente e no celular tudo empilha, sem alterar geracao de voucher, busca, saldo, selecao ou historico.
...
- O modulo `XP` fica oficialmente em `apps/xp`, usa Node.js/TypeScript/Express com Postgres `wimifarma_xp`, sessao propria `WFXP`, login unico por `core_users` e proxy Apache em `/xp/`. A tela preserva CSS/JS/assets/uploads de `site/xp`; o PHP antigo foi arquivado em `site/_legacy-disabled/2026-05-29/xp-php/`. Desde 2026-05-30, o app nao possui `mysql2`, importador, espelho, logs, fallback `wf_users`, `XP_AUTH_PROVIDER` nem flags `XP_LEGACY_MYSQL_*`; `wf_xp_*` fica apenas como referencia historica/backup. Desde 2026-06-11, `xp_sales` tem `source`/`source_entity_id` para lancamentos idempotentes de integracoes internas, incluindo uso de cashback no Balcao, atualmente +250 XP por resgate, e, desde 2026-06-20, +1.000 XP pelo fechamento real de caixa.
- O modulo `Financeiro` fica oficialmente em `apps/financeiro`, usa Node.js/TypeScript/Express com Postgres `wimifarma_financeiro`, sessao propria `WFFINANCEIRO`, login unico por `core_users` e proxy Apache em `/financeiro/`. A tela preserva o CSS/JS/assets de `site/financeiro`, e `financeiro-funcoes.php` continua apenas como helper minimo de data por include do Miauby; PHP direto nessa pasta fica bloqueado por `.htaccess`. Desde 2026-06-04, o PHP completo antigo do Financeiro fica arquivado em `site/_legacy-disabled/2026-06-04/financeiro-php/`. Desde 2026-05-30, a paridade com `financeiro_*` no MySQL foi validada e o app nao possui mais `mysql2`, importador, espelho, fallback `wf_users`, `FINANCEIRO_AUTH_PROVIDER` nem flags `FINANCEIRO_LEGACY_MYSQL_*`. Desde 2026-06-20, o botao `Fechar dia +1.000 XP` pontua o funcionario XP vinculado ao usuario logado somente na primeira conclusao daquele `financeiro_closings.id`; reabrir/fechar nao duplica, sem movimento nao pontua e falha do XP nao desfaz o caixa.
- O modulo `Bloco de notas/lembretes` fica oficialmente em `apps/notas`, usa Node.js/TypeScript/Express com Postgres `wimifarma_notas`, sessao propria `WFNOTAS`, login unico por `core_users`, permissao individual `notas` e proxy Apache em `/notas/`. Ele substitui o bloco `Notas` que ficava dentro da Gestao: na primeira subida importa idempotentemente `gestao_notepad_notes`, mas novas escritas ficam em `notas_notes`. As notas sao individuais por `created_by`/`core_users.id`: cada usuario ve, edita, apaga e reordena somente os proprios lembretes, sem tela ADM global para ler texto alheio. A tela usa notas em estilo papel, ate 5 por fileira no desktop, altura dinamica conforme texto, edicao, exclusao logica e reordenacao por arrasto; desde 2026-06-13, o arrasto espera movimento real antes de pegar a nota e o rodape compacta `Salvar`/`Apagar` na mesma linha. O Miauby conhece a existencia/rota e pode consultar apenas `/notas/api/internal/summary`, sem expor o conteudo das notas.
- O modulo `Entrega` fica oficialmente em `apps/entrega`, usa Node.js/TypeScript/Express com Postgres `wimifarma_entrega`, sessao `WFENTREGA`, SSO da Home e proxy Apache em `/entrega/`. O card e as rotas operacionais sao exclusivos do login mestre exato `adm`; acesso direto de qualquer outra conta recebe HTTP 403, enquanto `/entrega/health` continua publico para monitoramento. Cada criacao idempotente gera exatamente uma comissao de R$ 1,00 e +400 XP para o responsavel ativo escolhido por `adm`, usando `delivery_creation + deliveries.id` no XP oficial sem duplicar. Edicao/reimpressao preservam identidade, comissao e XP; cancelamento anterior ao pagamento estorna comissao e XP. Somente `adm` cancela antes da baixa e paga lotes mensais idempotentes com relatorio termico resumido.
- O modulo `Indicacao` fica em `apps/comissao`, usa Node.js/TypeScript/Express com Postgres `wimifarma_comissao`, sessao `WFCOMISSAO`, SSO da Home e proxy Apache em `/comissao/`. Toda conta ativa pode consultar e confirmar cupons; cada uso confirmado gera uma comissao financeira para um indicador externo e +300 XP idempotentes para o usuario logado. Somente o login mestre `adm` ou perfil `admin` cria novos indicadores e ofertas; `adm`, `admin` ou `gerente` administram os registros existentes, pagam comissoes parciais ou integrais e cancelam utilizacoes com estorno logico de comissao e XP. Novos cupons recebem codigo automatico de cinco digitos na faixa `50000` a `99999`, separada do Cashback (`00000` a `49999`), conferida tambem contra todos os codigos historicos de cinco digitos do Cashback, e validade padrao de tres meses. A impressao termica mostra oferta, codigo e validade, nunca a comissao nem o bloco do indicador.
- O modulo `Gestao` foi elevado para Node.js + TypeScript + Postgres: login restrito a `adm`, `admin` ou `gerente`, contas a pagar manuais em `gestao_accounts`, categoria livre com resumo lateral normalizado, lista operacional compacta, painel `Mensal` para contas com repeticao ativa e ordem manual salva, busca por nome/valor/categoria/datas com limite inicial de 10 e `Mostrar mais`, itens flexiveis em `gestao_account_items`, pagamentos parciais datados em `gestao_account_payments`, vencimento opcional calculado por dias no cadastro manual e editavel por data nos detalhes, status reversivel, extrato por conta com saldo/progresso, pagamento parcial por qualquer lancamento aberto, cancelamento/reabertura de lancamento sem apagar historico, exclusao da tela apenas por arquivamento de contas canceladas, reabertura de contas pagas, renomeacao por icone de lapis, repeticao do mes seguinte em ciclo liga/desliga sem copiar pagamentos, recorrencia permanente `Repetir sempre mes que vem` em `repeat_forever`, observacao editavel/minimizavel com quebras de linha preservadas, detalhes abertos ao clicar na linha compacta e pagamentos/historico minimizaveis, com auditoria em `gestao_audit_events` e `core_audit_logs`. Desde 2026-06-01, contas pendentes de competencias anteriores aparecem na visao do mes selecionado ate serem pagas/canceladas, preservando a competencia original; pagamentos datados no mes seguem compondo o total pago daquele mes. Ainda em 2026-06-01, contas com repeticao ativa aparecem primeiro na lista, recebem simbolo de repeticao e nao usam mais botao separado `Abrir`. Desde 2026-06-03, contas com `repeat_forever` ficam no topo do painel `Mensal` e, quando pagas, garantem a copia pendente do proximo mes sem duplicar financeiro. Desde 2026-06-05, contas ligadas a Pedidos aparecem em um bloco visual `Pedidos`, saindo da lista geral para nao misturar boletos de fornecedores com contas manuais, mas mantendo categoria `Boleto`, pagamentos, totais, status, busca/filtro e auditoria no mesmo fluxo. Desde 2026-06-11, os blocos da tela principal ficam lado a lado em faixa horizontal livre no desktop, com rolagem lateral do navegador, e mobile continua empilhado; desde 2026-06-12, o antigo bloco `Notas` saiu da Gestao e virou `/notas/`; vencimento sem data, pagamentos, historico, observacao e ajustes seguem como opcoes recolhidas depois dos lancamentos.
...
- Desde 2026-06-12, `encomenda` gera contexto operacional para o Miauby Whats por resumo diario: todos os dias as 17h, no fuso `America/Sao_Paulo`, o bridge consulta `GET /cotacao/api/internal/encomendas` e envia uma unica lista das encomendas ativas para contatos autorizados com card `Cotacao`. A extracao reconhece variacoes seguras como `encomenda`, `encomendar`, `encomendado` e `enc.`, separa observacao antes/depois da palavra sem inventar dados e preserva o texto original da linha para auditoria. A antiga rotina individual do dia seguinte as 16h ficou legado desativado; `cotacao_v2_encomenda_reminders` continua preservada como historico/data de deteccao, sem apagar dados antigos nem alterar valores da cotacao.
- Desde 2026-08-22, Miauby Interno e WhatsApp criam Encomenda por linguagem natural em qualquer ordem, incluindo pedido, reserva, separacao e guarda para cliente. A identidade do item fica em `Produto`; todo o restante util fica em `Categoria`, iniciada por `Encomenda`. Cliente e telefone sao opcionais; produto e confirmacao sao obrigatorios. A Cotacao revalida ator ativo/permissao e usa `source + request_id` para impedir duplicacao, sem substituir o cliente pelo usuario que executou. Consultas de encomendas continuam somente leitura.
...
- Desde 2026-08-28, nomes de produto iguais ou quase iguais, como `dipirona 500 mg`, `dipirona 500` e `dipirona`, recebem verde-neon claro temporario na grade. Desde 2026-08-31, dosagens explicitamente diferentes deixam de ser tratadas como duplicidade (`sinvastatina 20`/`40`, `desvenlafaxina 50`/`100`), enquanto grafias equivalentes da mesma dosagem (`500 mg`, `500mg`, `500`, `0,5 g`) e quantidades de embalagem diferentes continuam sendo entendidas corretamente. A indicacao e somente visual, recalculada em tempo real e nao grava cor, regra ou alteracao adicional no banco.
- Apagar totalmente um nome antes preenchido em `PRODUTO` tambem limpa a cor manual daquela celula e volta ao fundo padrao; apenas reescrever o nome preserva a cor. A mudanca e atomica, sincronizada entre abas e pode ser desfeita com `Ctrl+Z`.
...
- Desde 2026-08-11, o card de confirmacao aguarda a resposta da API e termina visualmente como confirmado, cancelado ou falho, sem ficar preso em `Confirmando...`. Sangrias e lancamentos financeiros usam idempotencia unica por confirmacao, preservando retry seguro sem bloquear duas operacoes legitimas identicas.
...
- Em 2026-08-23, o TTS do Miauby Interno e do Miauby WhatsApp recebeu identidade vocal mais felina: cadencia perceptivel e finais levemente ronronados ficam restritos a falas leves, sem miados repetidos, caricatura ou clonagem. O Interno preserva `marin` e o WhatsApp preserva `Zephyr`; valores, nomes, codigos, alertas e confirmacoes suspendem o efeito para manter clareza absoluta.
...
- Desde 2026-06-05, quando `MIAUW_WHATSAPP_REQUIRE_PREFIX=true`, texto solto de numero autorizado sem `miauby` fica marcado como `prefix_missing_help_only`, nao chama Gemini/core/tools, nao executa parsers de PIX, sangria, pedidos ou tarefas, e nao cria pendencia operacional. Desde 2026-06-11, esse caminho responde somente `😺 Para acessar o Miauby é só digitar Miauby e seu comando! ✨`. A lista central de comandos ativos fica em `apps/miauw-whatsapp/src/command-help.ts` e continua sendo usada por `miauby menu` como catalogo oficial do que o Miauby Whats pode fazer; o prompt Gemini tambem orienta a nao inventar comando fora dessa tabela. O menu mostra somente fluxos Whats fechados e validados por card, como Cashback rapido, Financeiro, Pedidos, Cotacao, Tarefas e Miauby/n8n; Gestao, XP e Codigos ficam fora da tabela ate terem comando direto validado. No Cashback rapido, `miauby cashback 35` gera o voucher idempotente; emissao anonima nao da XP, identificada preserva a regra atual, e o WhatsApp orienta imprimir pelo Cashback ou Miauby Interno. A secao N8n do menu mostra apenas avisos que podem ser enviados para usuarios, com horario, card e destinatarios por usuario/display name, sem telefone nem status tecnico; rotinas internas ficam no painel/log.
...
- Desde 2026-08-16, o WhatsApp completa pedidos de relatorio em varias mensagens: `miauby relatorio`, o tipo e a data podem chegar separados. O bridge guarda somente o estado sanitizado por hash do telefone por 15 minutos, aceita a resposta esperada sem prefixo, rejeita datas impossiveis e encerra por consumo, cancelamento ou expiracao. Datas exatas usam `America/Sao_Paulo`; a ponte Node/PHP carrega `data=YYYY-MM-DD` ate a consulta real, Financeiro usa o endpoint diario e Cashback/Tarefas usam intervalo de um dia, sem apresentar resumo mensal como se fosse diario. O relatorio Geral diario exige acesso aos cards Financeiro, Cashback e Tarefas.
- Depois de uma pausa do provedor, o primeiro envio real bem-sucedido marca os alertas `provider_paused` antigos como resolvidos, preservando o historico e recuperando o diagnostico operacional.
...
- Em 2026-08-24, audio valido de contato autorizado no WhatsApp passou a funcionar como ativacao por si so: nao precisa falar `Miauby`, mesmo quando o ambiente exige prefixo para texto. Se a abertura for reconhecida como `Miauby`, `Miau bi`, `Miau bis` ou `Miauw`, ela e removida antes do roteador. Texto digitado continua com a regra exata; cards, permissoes, idempotencia e confirmacoes nao mudaram.
...
- Desde 2026-06-08, o Pix CNPJ por midia usa `gemini-2.5-flash-lite` como OCR padrao quando `MIAUW_WHATSAPP_PIX_RECEIPT_OCR_MODEL` nao for sobrescrito, limita tentativas por `MIAUW_WHATSAPP_PIX_RECEIPT_OCR_DAILY_LIMIT` e pausa chamadas Gemini por `MIAUW_WHATSAPP_GEMINI_SPEND_GUARD_PAUSE_MINUTES` quando detectar spending cap, cota, billing ou 429. Nesses casos o WhatsApp orienta o lancamento manual `miauby pix cnpj valor responsavel`, sem trocar automaticamente para outro provedor pago. O painel mostra modelo, tentativas 24h, limite diario e status do guard. O fallback OpenAI fica apenas para falha transitoria de OCR, como timeout, 5xx, alta demanda/indisponibilidade temporaria do Gemini ou resposta vazia, quando `MIAUW_OPENAI_API_KEY`/`MIAUW_WHATSAPP_OPENAI_API_KEY` estiver configurado. Desde 2026-06-25, uma leitura Gemini incompleta/insegura ou com JSON invalido/vazio tenta uma segunda leitura conservadora antes do manual/fallback, com prompt reforcado para print longo, PDF, texto pequeno, comprovante cortado, bancos/layouts diferentes, pagador/recebedor proximos, CNPJ sem mascara e diagnostico sanitizado; `high demand`/`try again later` do Gemini cai no fallback configurado sem esperar retries longos da fila.
...
|   |-- notas/               # Bloco de notas/lembretes Node.js/TypeScript/Postgres
...
|-- notas-data/              # volume Postgres do Bloco de notas ignorado pelo Git
...
NOTAS_POSTGRES_PASSWORD
NOTAS_SESSION_SECRET
NOTAS_INTERNAL_TOKEN
NOTAS_INTERNAL_BASE_URL
...
MIAUW_AGENT_INTERPRETER_TIMEOUT_MS
MIAUW_WHATSAPP_AGENT_INTERPRET_TIMEOUT_MS
...
Desde 2026-08-16, o mesmo servico expoe `POST /miauw/agent/interpret`, protegido pelo token interno e sem efeitos colaterais. Miauby Interno e Miauby WhatsApp consultam essa camada antes dos parsers operacionais para identificar intencao, entidades e parametros na frase inteira, sem depender da ordem ou de maiusculas/minusculas. Um resultado `resolved` fornece uma mensagem canonica compativel com os parsers existentes; `ambiguous` pede somente a desambiguacao; `blocked` impede que uma negacao explicita chegue ao parser legado; `none` ou indisponibilidade preserva o formato antigo. Perguntas informativas sobre escrita sao desambiguadas, typo de uma letra nos termos do comando e aceito apenas com ativacao explicita, singular/plural permanece exato e frases normalizadas usam cache. Os timeouts podem ser ajustados por `MIAUW_AGENT_INTERPRETER_TIMEOUT_MS` no PHP e `MIAUW_WHATSAPP_AGENT_INTERPRET_TIMEOUT_MS` no bridge.
...
Para Bloco de notas/lembretes, `apps/notas` e `wimifarma-notas-app:3970` sao a rota oficial `/notas/` via proxy Apache. Manter `NOTAS_POSTGRES_PASSWORD`, `NOTAS_SESSION_SECRET` e `NOTAS_INTERNAL_TOKEN` por ambiente; o token pode reutilizar `MIAUW_GUARDIAN_TOKEN` para o Miauby consultar somente o resumo seguro. O app usa `core_users`/`WFHOME_SSO`, permissao individual `notas`, Postgres `wimifarma_notas` e importa idempotentemente notas antigas de `gestao_notepad_notes` sem continuar gravando na Gestao. Desde 2026-06-15, a tela e as escritas filtram notas por `created_by` do usuario logado, mantendo cada lembrete individual por usuario. Desde 2026-06-13, a tela usa topo compacto sem rotulo duplicado, grade de 5 lembretes por linha no desktop, rodape menor nos cards e arrasto reforcado pelo botao de mover, tambem com setas quando focado, sem alterar CSRF, POSTs ou endpoint interno.
...
Desde 2026-09-01, a Home e os modulos ativos nao aceitam mais o fallback fixo `adm`/`adm`; a Home possui rate limiting persistente, SSO com segredo minimo de 32 caracteres e auditoria de login. Cookies Node usam Secure automatico em HTTPS, o upload do XP tem allowlist e limites estritos, e existe rotina validada de backup para todos os bancos. Consulte `docs/08-autenticacao-e-permissoes.md`, `docs/09-deploy-e-ambiente.md`, `docs/11-seguranca.md`, `docs/15-logs-e-auditoria.md` e `docs/16-testes.md` antes de alterar esses controles.
