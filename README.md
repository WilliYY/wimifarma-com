...
- Login / Senha para cofre interno simples de acessos da farmacia, com senha cifrada e auditoria;
...
Para Login / Senha, `apps/login-senha` e `wimifarma-login-senha-app:3950` sao a rota oficial `/login-senha/` via proxy Apache. Manter `LOGIN_SENHA_POSTGRES_PASSWORD`, `LOGIN_SENHA_SESSION_SECRET` e, em producao, `LOGIN_SENHA_VAULT_KEY` por ambiente. O banco dedicado `wimifarma_login_senha` guarda `login_senha_entries`, `login_senha_audit_events` e sessoes; senhas nunca devem aparecer em logs, console, erro ou documentacao. O modulo usa `core_users`/`WFHOME_SSO` e exige permissao explicita `login_senha` para usuario comum.
