...
- Comissao para campanhas de indicacao, cupons termicos, comissao financeira do indicador e +300 XP por uso confirmado;
...
- O modulo `Comissao` fica em `apps/comissao`, usa Node.js/TypeScript/Express com Postgres `wimifarma_comissao`, sessao `WFCOMISSAO`, SSO da Home e proxy Apache em `/comissao/`. Toda conta ativa pode consultar e confirmar cupons; cada uso confirmado gera uma comissao financeira para um indicador externo e +300 XP idempotentes para o usuario logado. Somente `adm`, `admin` ou `gerente` administram indicadores/cupons, pagam comissoes parciais ou integrais e cancelam utilizacoes com estorno logico de comissao e XP. A impressao termica mostra a oferta, codigo, indicador e validade, nunca a comissao.
