...
- Desde 2026-06-08, a Home usa `logo-wimifarma-home-animated.svg` como variacao animada sem fundo da marca, aplicada no login central, no rodape do login e no header autenticado. A referencia correta e a animacao do sinal `+` girando antes de formar a logo completa, com melhoria de nitidez/fluidez sem reinventar o movimento. O SVG carrega com preload `image/svg+xml`, prioridade/cache-bust em `site/home.php`, proporcao `1024/400` e sem `mix-blend-mode`.
