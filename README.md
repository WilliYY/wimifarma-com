...
- O n8n tambem pode chamar `POST /miauw/whatsapp/internal/financeiro-cash-closing-reminder` todo dia as 18h. O bridge consulta Financeiro por `GET /financeiro/api/internal/cash-closing-status`, recebe o status do dia e a lista de caixas em aberto, e so envia lembrete para contatos autorizados com card `Financeiro` quando houver dia pendente para finalizar. A mensagem informa o(s) dia(s) em aberto; se nao houver pendencia e o dia consultado estiver `fechado`, `divergente` ou `sem_movimento`, nao envia nada.
