...
- O core compartilhado de autenticacao fica em Postgres `wimifarma_core`: `apps/core-auth` sincroniza `wf_users` para `core_users`, preservando id legado, hash, role e status. `apps/usuarios` usa esse mesmo core para criar logins novos, permissoes por modulo, vinculo com XP, vinculo seguro com allowlist do Miauby WhatsApp, delegacao de tarefa privada e auditoria central. Cotacao, Pedidos e Cashback usam esse core como login unico, sem dependencia MySQL; Gestao, Tarefa, XP, Codigos, Financeiro e Usuarios usam esse core como login principal, com fallback MySQL apenas como rollback opt-in onde ainda existir.
