...
- Desde 2026-07-24, o card ADM `Wimi Impressora` ativa no Chrome do computador USB da Bematech MP-4200 TH uma estacao web sem EXE ou servico do Windows. A estacao recebe a fila central por HTTPS, usa cookie de dispositivo protegido, HMAC CSRF, CSP com nonce e Web Locks para impedir duas abas consumindo trabalhos. O atalho `.cmd` opcional apenas abre o Chrome com `--kiosk-printing`, sem segredo, elevacao, PowerShell ou tarefa agendada; atualizacoes chegam pelo servidor. Nos comprovantes, o unico botao `Imprimir` usa a fila quando a estacao esta online ou abre a impressao local quando nao esta. Falhas incertas nao caem automaticamente no navegador nem voltam para a fila, evitando papel duplicado. O papel identificado destaca codigo permanente, nome salvo e somente `Cashback gerado`; vouchers anonimos continuam sem cliente. Arquitetura, seguranca, operacao e testes ficam em `docs/31-wimi-impressora.md`.
