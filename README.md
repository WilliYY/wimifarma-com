...
- O modulo `site/codigos` guarda atalhos de comissao em `wf_codigos_comissao`, com blocos por prefixo de EAN, autosave de `Código`, `EAN` e `Preço`, botao `+` para criar novo bloco de EAN quando necessario, reordenacao por arrastar o numero da linha, criacao de novas linhas no rodape de cada grupo e exclusao logica de itens apagados.
