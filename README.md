...
- O modulo `Entrega` fica oficialmente em `apps/entrega`, usa Node.js/TypeScript/Express com Postgres `wimifarma_entrega`, sessao `WFENTREGA`, SSO da Home e proxy Apache em `/entrega/`. Toda conta ativa pode registrar e imprimir suas entregas; cada criacao idempotente gera exatamente uma comissao de R$ 1,00 e +400 XP para o responsavel escolhido, usando `delivery_creation + deliveries.id` no XP oficial sem duplicar. Edicao/reimpressao preservam identidade, comissao e XP; cancelamento anterior ao pagamento estorna comissao e XP. Somente `adm`, `admin` ou `gerente` podem cancelar antes da baixa ou pagar lotes mensais idempotentes com relatorio termico resumido.
