...
- `cashback_clients`: clientes importados de `wf_clientes`, com telefone, nascimento, status, atendente e `legacy_mysql_id`. A chave primaria `id` e tambem o `Codigo do cliente` permanente exibido na busca e nos comprovantes identificados; nao existe coluna ou sequencia paralela para esse codigo.
...
- `cashback_quick_vouchers`: vouchers rapidos independentes dos creditos comuns, com codigo textual, valor gasto, valor do beneficio, percentual em basis points, estado, validade de seis meses, atendente escolhido, usuario real da emissao, cliente opcional, cadeia pai/sucessor, compra/resgate vinculados e contadores de solicitacao de impressao. `canceled_at`, `canceled_by` e `canceled_reason` registram o cancelamento logico permitido somente ao login mestre `adm`; nao apagar fisicamente vouchers pelo Balcao. Novas emissoes usam cinco digitos; registros historicos de quatro digitos continuam validos. O indice de `code` e `expires_at` nao e globalmente unico porque uma combinacao de cinco digitos pode ser reutilizada depois do vencimento; um indice unico parcial ainda impede dois vouchers `ativo` com o mesmo codigo. A geracao serializa pelo advisory lock `QUICK_VOUCHER_ADVISORY_LOCK` e trata como reservada qualquer linha com o mesmo codigo e `expires_at >= CURRENT_DATE`, inclusive usada ou cancelada; somente depois dessa data a combinacao volta ao sorteio. Ao iniciar o app, vouchers ainda ativos sao normalizados para `issued_at + 6 meses`; vouchers usados/cancelados permanecem historicos. O Balcao le os 20 registros mais recentes e consulta o XP separadamente, sem deixar indisponibilidade do banco XP derrubar a operacao.
...
- Desde 2026-07-29, `cashback_quick_vouchers.redemption_id` nao e unico: ate dez vouchers podem apontar para o mesmo `cashback_redemptions.id` quando sao usados juntos. A inicializacao remove de forma idempotente a antiga constraint `cashback_quick_vouchers_redemption_id_key` e cria `idx_cashback_quick_vouchers_redemption`; os dados e relacionamentos existentes sao preservados.
...
Essa fase preserva `legacy_mysql_id`, `legacy_source_key`, campos auxiliares de usuario/conversa/status, checksum e `payload_sanitized` em `JSONB`. Quando uma tabela fonte nao tem `id`, o migrador usa um identificador sintetico estavel derivado de chave/nome ou da linha sanitizada. O migrador redige chaves, tokens, senhas, payload bruto, SQL bruto, stack trace, telefone e midia. Na leitura canonica 5A, `apps/miauby` resolve status persistido tanto da coluna `status` quanto de `revisao_status`/`ativo` dentro de `payload_sanitized`; treino entra apenas aprovado, memorias/padroes apenas aprovados, e conhecimentos apenas ativos/aprovados. Desde a Etapa 7A de 2026-06-04, `miauby_conversations` e `miauby_messages` tambem podem receber dual-write controlado pelo adaptador `/write-adapter/commit`, sempre depois da escrita PHP/MySQL e somente com flags especificas ligadas. O PHP continua fonte oficial de `/miauw/` ate existir paridade validada e corte documentado.
...
- `cotacao_v2_encomenda_reminders`: lembretes de encomenda detectados na Cotacao para envio interno pelo Miauby Whats, com cotacao/linha, texto original, produto, quantidade, categoria, valores da linha, data de deteccao, horario de aviso, destinatarios mascarados/modo, status, tentativas, erro, proxima tentativa e resultado do WhatsApp. Falha sem destinatario configurado fica como erro final sem novas tentativas; falha de transporte/configuracao usa `next_attempt_at` para retry atrasado. Depois de importacao Google Sheets ou restore de backup, a Cotacao reconcilia essa tabela para criar lembretes faltantes e cancelar pendentes de linhas removidas.
...
- `xp_sales.amount_cents` guarda venda em centavos inteiros, `xp_sales.xp_points` guarda o XP calculado no momento do lancamento e `xp_sales.note` guarda a observacao opcional exibida em `Ultimos lancamentos`. `source` e `source_entity_id` sao opcionais e possuem indice unico parcial para lancamentos idempotentes de integracoes internas, como +250 XP por emissao direta de cashback rapido (`cashback_quick_voucher_issue`, usando `cashback_quick_vouchers.id`), +500 XP por resgate de cashback (`cashback_redemption`) e +1.000 XP por fechamento real de caixa (`financeiro_cash_closing`, usando `financeiro_closings.id`). Quando um voucher rapido direto e cancelado, a venda XP correspondente recebe `deleted_at`/`deleted_by` e sai dos totais sem perder historico; se essa escrita falhar, o voucher continua cancelado e o ADM pode repetir somente a retirada do XP. A regra atual de venda manual e R$ 1.000,00 = 2.500 XP; o nivel 1 exige 30.000 XP para passar e os niveis seguintes usam progressao crescente por `xp_required_for_next_level()`. O schema do XP garante indice para leituras por venda ativa, funcionario e mes.
...
- Categoria da Gestao e texto livre no banco, mas o cadastro manual infere automaticamente a categoria pelo titulo da conta e nao exibe mais campo de categoria. A tela agrupa por chave normalizada apenas na aplicacao: remove acentos, ignora maiusculas/minusculas e junta espacos/pontuacao. O texto original da categoria continua preservado em `gestao_accounts.category`; `aluguel`, `Aluguel` e `ALUGUEL` aparecem juntos, enquanto categorias diferentes continuam separadas.
- Vencimento (`due_at`) e independente da competencia mensal e de `paid_at`; ele serve para ordenar urgencia e pode ser alterado ou removido sem recalcular o valor da conta. No cadastro manual da Gestao, o operador informa a quantidade de dias ate vencer e o app calcula a data final em `America/Sao_Paulo`, persistindo somente essa data em `gestao_accounts.due_at`.
- Repetir uma conta para o mes seguinte cria ou garante de forma idempotente novo registro em `gestao_accounts` com `status='pendente'`, nova `generated_at`, mesmos itens ativos em `gestao_account_items`, vencimento avancado em um mes quando houver `due_at`, nenhum pagamento em `gestao_account_payments` e `repeated_from_account_id` apontando para a origem; desligar o ciclo muda `repeat_next_month=false` sem apagar copia ja criada. Datas de vencimento em fim de mes devem cair no ultimo dia valido do mes seguinte, evitando pular um mes quando a origem for dia 29, 30 ou 31. A opcao `Repetir tantos meses` reutiliza a mesma regra em cadeia dentro da transacao, criando ate 24 competencias futuras sem copiar pagamentos nem status pago e sem novas colunas. A ordenacao manual do painel Mensal fica em `gestao_accounts.monthly_sort_order`, atualizada apenas por contas da competencia atual com `repeat_next_month=true`, sem alterar valor, status, itens ou pagamentos.
...
- O campo de dias de vencimento de Pedidos e apenas uma conveniencia de entrada: recebe inteiro positivo, calcula a data a partir do dia atual em `America/Sao_Paulo` e persiste somente a data final em `gestao_account_items.due_at`. A competencia mensal continua independente e nunca deve recalcular automaticamente vencimentos ja preenchidos.
...
- `pedidos_orders` controla a fila de pedidos feitos/aguardando chegada. `created_at` e a data/hora persistida do registro do pedido e nao deve ser alterada a cada aviso; ela e usada no resumo do Miauby WhatsApp para priorizar pedidos parados ha mais tempo. Confirmar chegada preenche `moved_to_confirmed_at` e cria/atualiza o registro correspondente em `pedidos_confirmed_orders`.
...
- `pedidos_orders` alimenta o badge do card `Pedidos` na home via `/pedidos/api/badge`, contando todos os pedidos ainda em `Aguardando chegada`; `expected_arrival_at` continua disponivel no JSON para previsao de chegada, e `created_at` identifica quando o pedido foi registrado. No formulario de criacao, a previsao e informada como numero de dias (`2` = hoje + 2 dias); o backend grava somente a data calculada em `expected_arrival_at`.
