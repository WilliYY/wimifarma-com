...
- `login_senha_entries` guarda Nome, Login / Usuario e senha cifrada por AES-256-GCM (`password_ciphertext`, `password_iv`, `password_tag`), com `scope='geral'` para `/login-senha/` e `scope='adm'` para `/login-senha-adm/`. Nao gravar senha em texto puro, log, auditoria, console, erro, payload generativo ou WhatsApp.
