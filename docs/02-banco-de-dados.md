...
- `wf_codigos_comissao` deve manter `codigo`, `ean` e `preco` editaveis por autosave; a separacao visual em `EAN 20` e `EAN 40` vem do prefixo do campo `ean`, sem tabela paralela. Apagar pela tela marca `ativo=0` e `apagado_em`, preservando o registro para auditoria basica.
