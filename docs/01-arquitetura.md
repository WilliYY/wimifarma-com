...
- Manter `/login-senha/` e `/login-senha-adm/` pelo proxy Apache para `apps/login-senha`; o modulo deve usar `wimifarma_login_senha` como fonte de verdade do cofre, `scope='geral'`/`scope='adm'` para separar areas, `core_users` para sessao/permissao, permissao explicita `login_senha` para usuarios comuns no cofre geral e gate `adm`/`admin`/`gerente` no cofre ADM, sem expor senha em logs, WhatsApp ou contexto generativo.
