...
- Cashback foi cortado para `apps/cashback` com Node.js + TypeScript e Postgres dedicado `wimifarma_cashback`. A tela visual foi preservada por assets de `site/cashback`; depois da validacao de 2026-05-29, `wf_*` do Cashback fica apenas como referencia historica/rollback manual e o Compose nao injeta credenciais MySQL no app.
...
- Reativar `FINANCEIRO_LEGACY_MYSQL_*` sem uma nova paridade pode confundir a fonte de verdade; em rollback, validar fechamentos, lancamentos, relatorio, CSV e integracao Pix CNPJ do Miauby WhatsApp antes de operar.
