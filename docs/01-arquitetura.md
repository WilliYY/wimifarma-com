...
- Manter `Tarefa` como modulo separado em `/tarefa/`, usando `apps/tarefa`, container `wimifarma-tarefa-app:3500`, sessao propria `WFTAREFA`, CSRF proprio e proxy Apache dedicado. `site/tarefa` fica apenas como legado de referencia/fonte visual, com PHP direto bloqueado por `.htaccess`.
...
- Manter `/login-senha/` pelo proxy Apache para `apps/login-senha`; `/login-senha-adm/` permanece somente como rota tecnica/legada para preservar dados antigos de `scope='adm'`, sem card na Home desde 2026-06-05 e sem apagar registros sem confirmacao. O modulo deve usar `wimifarma_login_senha` como fonte de verdade do cofre, `core_users` para sessao/permissao, permissao explicita `login_senha` para usuarios comuns no cofre geral e gate `adm`/`admin`/`gerente` se a rota tecnica ADM for acessada diretamente, sem expor senha em logs, WhatsApp ou contexto generativo.
...
- Login / Senha foi criado em `apps/login-senha` com Node.js + TypeScript, sessao `WFLOGINSENHA` e Postgres dedicado `wimifarma_login_senha`. O app guarda Nome, Login / Usuario e Senha cifrada em `login_senha_entries`, separa cofre comum e ADM por `scope`, registra auditoria em `login_senha_audit_events` e espelha resumo seguro em `core_audit_logs`.
