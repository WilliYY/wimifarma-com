...
- Manter `/login-senha/` pelo proxy Apache para `apps/login-senha`; `/login-senha-adm/` e o escopo restrito exibido como aba `Contas` no topo do modulo, sem card separado na Home, para logins/senhas especificos do sistema. O modulo deve usar `wimifarma_login_senha` como fonte de verdade do cofre, `core_users` para sessao/permissao, permissao explicita `login_senha` para usuarios comuns no cofre geral e gate `adm`/`admin`/`gerente` no escopo `Contas`, sem expor senha em logs, WhatsApp ou contexto generativo.
