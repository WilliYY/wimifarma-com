...
docker exec wimifarma-miauby-app sh -lc 'TOKEN="$MIAUW_AGENT_INTERNAL_TOKEN"; wget -qO- --header=x-miauby-internal-token:${TOKEN} "http://127.0.0.1:4100/miauby/api/internal/canonical-context?limit=3&message=teste%20miauby"'
...
Esse migrador copia `miauw_*` para `miauby_*` em Postgres sombra, com `legacy_mysql_id`, checksum e `payload_sanitized`. O script `scripts/miauby-shadow-migrate.sh` constroi e executa um `wimifarma-miauby-migrator` descartavel com `--no-deps`, mantendo o runtime `wimifarma-miauby-app` sem credenciais/conexao MySQL. O `wimifarma-miauby-app` expoe apenas API interna de leitura, sem proxy publico: `/miauby/health` e publico dentro da rede Docker; `/miauby/api/internal/status`, `/miauby/api/internal/parity?sample=5`, `/miauby/api/internal/readiness?sample=20`, `/miauby/api/internal/context?limit=3`, `/miauby/api/internal/canonical-context` e `/miauby/api/internal/context-pack` exigem `MIAUBY_INTERNAL_TOKEN`, `MIAUW_GUARDIAN_TOKEN` ou `MIAUW_AGENT_INTERNAL_TOKEN`. `readiness` consolida health e o ultimo `validate` salvo em `miauby_migration_runs`, `context` retorna apenas amostras sanitizadas, e `canonical-context` monta o read model 5A (`canonical_read_model.version=miauby-read-model-5a-2026-06-02`) com persona, treino aprovado, memorias/padroes aprovados, conhecimentos ativos/aprovados e contratos de tools, sempre read-only e sem `payload_sanitized` bruto. O smoke `scripts/miauby-shadow-smoke.sh` valida essas secoes e os guardas de `write_enabled=false`, `writes_enabled_in_node=false`, `route_cutover_enabled=false`, `public_proxy_enabled=false`, `openai_called=false` e `tools_executed=false`. A flag `MIAUBY_CONTEXT_SHADOW_ENABLED` permite que o PHP consulte esse pacote em sombra durante a comparacao do agente, gravando trace sanitizado sem mudar a resposta oficial. O corte canonico seguro de 2026-05-31 publica `/miauby/` como redirect para `/miauw/` e aliases `/miauby/agent/` e `/miauby/whatsapp/` para os servicos Node atuais; isso nao troca widget, nao corta o PHP e nao deve gravar token, telefone cru, SQL bruto, stack trace, audio ou midia.
