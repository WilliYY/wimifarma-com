...
- Wimi Impressora acrescenta auditoria de download sem ticket bruto, pareamento, criacao de fila, teste, revogacao, reimpressao e conclusao `printed`/`failed`/`uncertain`. O evento `printed` significa envio aceito pelo spooler informado pelo agente, nao comprovacao fisica de papel. Erros sao limitados e nao podem carregar token, payload HTTP bruto ou caminho de configuracao local.
