...
- Cashback registra login/falha/logout, criacao/edicao de cliente, compra, resgate, mensagens, atendentes, configuracoes e manutencao em `cashback_audit_events`; vouchers rapidos acrescentam `cashback_rapido_emitido`, `cashback_rapido_usado`, `cashback_rapido_impressao_solicitada` e `cashback_rapido_limite_tentativas`. O comprovante da operacao comum acrescenta `cashback_comprovante_impressao_solicitada` com entidade `compra`, somente para ID autorizado na sessao. Os eventos mantem valor/relacoes essenciais na propria tabela transacional e nao afirmam que o papel foi impresso. Desde 2026-05-30 nao espelha mais logs em `wf_logs` e nao possui caminho `mysql2`. Historico antigo de importacoes permanece em `cashback_migration_runs`.
- Wimi Impressora Web acrescenta auditoria de ativacao, download do atalho sem segredo, criacao de fila, teste, revogacao, reimpressao e conclusao `printed`/`failed`/`uncertain`. Para transporte `web`, `printed` significa que o navegador concluiu a janela de impressao, nao comprovacao fisica do papel. Erros sao limitados e nao podem carregar cookie/token, HMAC CSRF, payload HTTP bruto ou dados fora do recibo. Eventos antigos de download/pareamento do agente permanecem como historico.
