...
- Cashback registra login/falha/logout, criacao/edicao de cliente, compra, resgate, mensagens, atendentes, configuracoes e manutencao em `cashback_audit_events`; vouchers rapidos acrescentam `cashback_rapido_emitido`, `cashback_rapido_usado`, `cashback_rapido_impressao_solicitada` e `cashback_rapido_limite_tentativas`. O comprovante da operacao comum acrescenta `cashback_comprovante_impressao_solicitada` com entidade `compra`, somente para ID autorizado na sessao. Desde 2026-07-25, o XP do novo cliente registra `xp_cliente_cadastrado_lancado`/`falha`, e o uso de saldo registra `xp_cashback_resgate_lancado`/`falha`; no XP, usam respectivamente `cashback_client_creation` por cliente e `cashback_redemption` por resgate. O beneficiario e a conta escolhida, enquanto `created_by` e o ator real da sessao. Os eventos mantem valor/relacoes essenciais na propria tabela transacional e nao afirmam que o papel foi impresso. Desde 2026-05-30 nao espelha mais logs em `wf_logs` e nao possui caminho `mysql2`. Historico antigo de importacoes permanece em `cashback_migration_runs`.
- Eventos antigos de ativacao, fila, teste, revogacao, reimpressao e conclusao da Wimi Impressora permanecem como historico. Desde 2026-07-25, nao surgem novos trabalhos de fila; `cashback_rapido_impressao_solicitada` e `cashback_comprovante_impressao_solicitada` registram somente a abertura do fluxo local e nunca afirmam que o papel saiu.
