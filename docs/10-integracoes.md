...
- A Fase 13 libera tools reais de leitura baixa no Node por ponte PHP interna tokenizada (`/miauw/agent-tools.php`). O Node chama Financeiro, Cashback, Codigos e Cotacao por pontes controladas, com pre-leitura deterministica para pedidos claros e tools disponiveis ao Agents SDK; `buscar_cliente` segue fora por privacidade. Para Codigos e Financeiro, o PHP prefere os endpoints internos dos apps Node com token e usa MySQL apenas como fallback de transicao quando a ponte nao estiver configurada.
