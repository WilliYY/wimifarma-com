...
- A Fase 13 libera tools reais de leitura baixa no Node por ponte PHP interna tokenizada (`/miauw/agent-tools.php`). O Node chama Financeiro, Cashback, Codigos e Cotacao por pontes controladas, com pre-leitura deterministica para pedidos claros e tools disponiveis ao Agents SDK; `buscar_cliente` segue fora por privacidade. Para Financeiro e Cashback, o PHP deve usar os endpoints internos dos apps Node com token; MySQL nesses dominios e apenas rollback manual. Para Codigos, o fallback MySQL ainda pode existir enquanto as flags legadas estiverem ativas.
...
- Pedidos expoe endpoints internos tokenizados para n8n/Miauby WhatsApp: `GET /pedidos/api/internal/arrival-summary` lista pedidos aguardando chegada, e `POST /pedidos/api/internal/confirm-arrival` confirma somente a chegada por titulo ou `order_id`. A rotina n8n `pedidos-chegada-17h` chama o bridge WhatsApp, nao o banco, e o bridge calcula destinatarios pelo card `Pedidos`.
