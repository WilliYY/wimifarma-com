...
- A rotina n8n `financeiro-fechamento-caixa-18h` chama o bridge WhatsApp todo dia as 18h; o bridge consulta `GET /financeiro/api/internal/cash-closing-status` e so envia lembrete para contatos reais com card `Financeiro` quando existir caixa aberto, em conferencia ou sem registro no dia consultado. A mensagem inclui o dia pendente, e o card da rotina no painel Miauby WhatsApp mostra a leitura atual quando o Financeiro responde.
