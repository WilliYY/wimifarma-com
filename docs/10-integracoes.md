...
- O Financeiro oficial em `apps/financeiro` expoe endpoints internos tokenizados (`/financeiro/api/internal/summary`, `/day`, `/checksums`, `/lancamentos`, `/faturamentos` e `/audit/recent`). Miauby e WhatsApp devem usar esses endpoints para `Pix CNPJ`, sangria/lancamentos e faturamento diario quando `FINANCEIRO_INTERNAL_TOKEN` ou `MIAUW_GUARDIAN_TOKEN` estiver configurado.
