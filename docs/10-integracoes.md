...
- A Fase 13 libera tools reais de leitura baixa no Node por ponte PHP interna tokenizada (`/miauw/agent-tools.php`). O Node chama Financeiro, Cashback, Codigos e Cotacao por pontes controladas, com pre-leitura deterministica para pedidos claros e tools disponiveis ao Agents SDK; `buscar_cliente` segue fora por privacidade. Para Financeiro e Cashback, o PHP deve usar os endpoints internos dos apps Node com token; Cashback nao possui caminho MySQL no app atual. Para Codigos, o fallback MySQL ainda pode existir enquanto as flags legadas estiverem ativas.
