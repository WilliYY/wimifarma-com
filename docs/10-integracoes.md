...
- A Fase 7 cria `wimifarma-miauw-agent`, servico Node.js 22 + TypeScript com Agents SDK em `/miauw/agent/`. Ele possui health publico minimo e endpoints internos `status`, `run` e `stream` protegidos por token, mas roda em modo sombra sem escrita real; o chat PHP atual continua sendo o caminho oficial.
...
- A Fase 10 adiciona contrato versionado da personalidade do Miauby (`miauby-persona-2026-05-16`) no PHP e no Node. O status tokenizado `/miauw/agent/status` informa `personality_version`, e `npm run check:persona` valida o prompt Node sem chamar a camada online.
...
- A Fase 13 libera tools reais de leitura baixa no Node por ponte PHP interna tokenizada (`/miauw/agent-tools.php`). O Node chama Financeiro, Cashback, Codigos e Cotacao por pontes controladas, com pre-leitura deterministica para pedidos claros e tools disponiveis ao Agents SDK; `buscar_cliente` segue fora por privacidade. Para Financeiro, Cashback e Codigos, o PHP deve usar os endpoints internos dos apps Node com token; Cashback e Codigos nao possuem caminho MySQL no app atual.
- O Financeiro oficial em `apps/financeiro` expoe endpoints internos tokenizados (`/financeiro/api/internal/summary`, `/day`, `/cash-closing-status`, `/checksums`, `/lancamentos`, `/faturamentos` e `/audit/recent`). Miauby e WhatsApp devem usar esses endpoints para `Pix CNPJ`, sangria/lancamentos, faturamento diario e lembrete de fechamento quando `FINANCEIRO_INTERNAL_TOKEN` ou `MIAUW_GUARDIAN_TOKEN` estiver configurado. Para sangria pelo Miauby Whats, o bridge chama `/financeiro/api/internal/lancamentos` com `categoria=Sangria`, `source=miauby_whatsapp`, `actor_user_id` do usuario vinculado e chave de idempotencia; o Financeiro revalida a permissao do ator antes de gravar. O status de fechamento tambem retorna `open_days_lookback_days`, `open_days_start`, `open_days_end`, `open_days` e `open_days_count` para o Miauby informar quais dias de caixa estao em aberto para finalizar, limitado aos ultimos 10 dias ate a data consultada.
- O parser local de sangria do Miauby Whats aceita observacao depois do valor, responsavel citado como pista auditavel, erros simples (`sagria`, `sangira`) e escrita natural como `tira 10 do caixa para troco`, sem criar registro paralelo. Se uma palavra depois do valor nao for usuario conhecido, ela vira observacao/motivo; o usuario vinculado ao numero continua sendo o responsavel oficial enviado ao Financeiro.
- Pedidos expoe endpoints internos tokenizados para n8n/Miauby WhatsApp: `GET /pedidos/api/internal/arrival-summary` lista pedidos aguardando chegada com valor total do pedido, previsao de chegada e `created_at` persistido do registro, ordenando os mais antigos primeiro; `POST /pedidos/api/internal/confirm-arrival` confirma somente a chegada por titulo ou `order_id`; `POST /pedidos/api/internal/create-order` cria pedido pelo mesmo fluxo da tela a partir de comando WhatsApp autorizado, com `actor_user_id`, auditoria e idempotencia. A rotina n8n `pedidos-chegada-17h` chama o bridge WhatsApp, nao o banco, e o bridge calcula destinatarios pelo card `Pedidos`. Consultas WhatsApp como `miauby em pedidos o que falta chegar` usam a mesma tabela localmente no bridge quando o contato tem card `Pedidos`; comandos como `miauby pedido anb 350` sao parseados localmente pelo bridge, sem Gemini, antes de gravar.
- A rotina n8n `financeiro-fechamento-caixa-18h` chama o bridge WhatsApp todo dia as 18h; o bridge consulta `GET /financeiro/api/internal/cash-closing-status` e so envia lembrete para contatos reais com card `Financeiro` quando existir caixa aberto, em conferencia ou sem registro nos ultimos 10 dias ate o dia consultado. A mensagem inclui o dia pendente, e o card da rotina no painel Miauby WhatsApp mostra a leitura atual quando o Financeiro responde.
...
- `GET /miauby/api/internal/cutover`, em `apps/miauby`, expoe somente para token interno o inventario de corte PHP -> Node/Postgres: fluxos, tabelas, bloqueios, sequencia segura e rollback. Ele e somente leitura e deve continuar com `write_enabled=false`, `route_cutover_enabled=false` e `public_proxy_enabled=false`.
...
- `MIAUW_WHATSAPP_USER_RATE_LIMIT_DEFER_SECONDS`
