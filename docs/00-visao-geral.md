...
- Financeiro deve preservar auditoria e rastreabilidade de fechamentos e divergencias; desde 2026-05-30 o app oficial em Postgres nao possui mais importador, espelho, `mysql2` ou fallback MySQL.
...
- Financeiro foi cortado para `apps/financeiro` com Node.js, TypeScript e Postgres dedicado. A paridade com MySQL foi validada e o caminho legado de `mysql2`/import/espelho foi removido; MySQL `financeiro_*` permanece apenas como referencia historica/backup.
...
- Rollback MySQL do Financeiro agora exige restaurar versao/imagem anterior e backup validado; nao ha mais flag de `.env` para religar o caminho antigo no runtime atual.
