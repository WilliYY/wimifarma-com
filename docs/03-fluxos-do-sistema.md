...
- contas ligadas a Pedidos por `gestao_supplier_orders`, `pedidos_orders` ou `pedidos_confirmed_orders` saem da lista geral e aparecem no bloco visual `Pedidos` na pilha principal da Gestao, com a mesma largura operacional de `Contas abertas`, preservando categoria `Boleto`, pagamentos, totais, status, busca/filtro e auditoria;
- o painel `Mensal` fica na pilha principal junto de `Contas abertas` e `Pedidos` no desktop, tambem com largura operacional grande; em telas menores os paineis empilham sem quebrar o fluxo. Ele mostra as contas da competencia atual com `Repetir mes que vem` ativo em estrutura compacta propria para evitar sobreposicao entre valor e botoes, soma o valor que sera levado para a proxima competencia, permite abrir o extrato completo e salva a ordem manual quando o operador arrasta as linhas com o mouse;
