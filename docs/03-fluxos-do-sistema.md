...
- contas ligadas a Pedidos por `gestao_supplier_orders`, `pedidos_orders` ou `pedidos_confirmed_orders` saem da lista geral e aparecem no bloco visual `Pedidos` na faixa superior da Gestao, preservando categoria `Boleto`, pagamentos, totais, status, busca/filtro e auditoria;
- o painel `Mensal` fica no topo ao lado de `Pedidos` e `Notas` no desktop, com rolagem horizontal controlada em telas menores; ele mostra as contas da competencia atual com `Repetir mes que vem` ativo em estrutura compacta propria para evitar sobreposicao entre valor e botoes, soma o valor que sera levado para a proxima competencia, permite abrir o extrato completo e salva a ordem manual quando o operador arrasta as linhas com o mouse;
