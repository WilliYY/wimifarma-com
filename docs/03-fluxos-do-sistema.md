...
- O card `Miauby` e permanente para toda conta ativa autenticada e abre o assistente interno mesmo quando a conta possui uma lista explicita de outros modulos. Essa liberacao nao inclui `Miauby Whatsapp`, que continua dependendo da permissao individual `miauw_whatsapp`.
...
- Os SVGs ativos desses modulos batem com o mesmo hash da logo oficial nova; a home publica pode usar uma variacao SVG animada propria sem alterar a identidade dos modulos internos.
- Desde 2026-06-08, a variacao animada da Home e `logo-wimifarma-home-animated.svg`, com fundo transparente, marca completa sempre visivel e animacao do sinal `+` girando; ela alimenta as logos do login, rodape e header autenticado, enquanto os SVGs oficiais continuam nos modulos internos.
...
Desde 2026-07-28, quando todas as contas elegiveis ja estao vinculadas, o aviso ocupa uma faixa compacta e a grade de atendentes aproveita toda a largura. Indicadores e downloads CSV usam cards maiores, contraste por grupo e quebra responsiva para evitar colunas estreitas, sem alterar formularios, CSRF, POSTs, filtros, exportacoes, permissoes ou auditoria.

Desde 2026-07-21, `Cashback rapido` aparece antes da consulta do Balcao. O operador informa o valor gasto e seleciona o usuario que imprime; o servidor recalcula o percentual padrao em centavos, gera um codigo aleatorio textual de cinco digitos e cria um voucher de seis meses sem exigir cliente. Codigos historicos de quatro digitos continuam aceitos enquanto validos. O recibo usa 76 mm uteis em rolo de 80 mm, a marca completa `+ | WimiFarma` de `logo-wimifarma-receipt.png`, valor, codigo, data real de vencimento, WhatsApp, endereco e atendente. Existe somente `Imprimir`, que registra a solicitacao e abre o dialogo de impressao local do navegador. Nenhum caminho consegue confirmar fisicamente que o papel saiu.

Desde 2026-07-28, `Dados do cashback rapido` fica aberto por padrao e calcula um resumo de producao por `issued_attendant_id` sobre todas as emissoes diretas. Cada usuario mostra total cadastrado, ativos, usados, vencidos, cancelados, soma historica de cashback emitido e data do ultimo cadastro. O bloco separado `Historico do cashback rapido` inicia recolhido e mostra os 20 registros recentes; ele abre por `quick_history=1` depois de cancelamento ou correcao de XP. Vouchers sucessores, identificados por `parent_voucher_id` ou `source_purchase_id`, continuam na conferencia recente, mas ficam fora do resumo para nao contar como um novo cadastro rapido feito pelo usuario. A consulta e somente leitura e nao altera voucher, XP, impressao ou auditoria.

Desde 2026-07-25, o card, a navegacao e a estacao `Wimi Impressora` foram retirados. Nao existe fila ativa, atalho, agente ou navegador dedicado. Dispositivos e trabalhos antigos continuam no Postgres apenas para auditoria; ativos sao revogados e trabalhos incompletos sao cancelados na inicializacao.

Desde 2026-07-29, o bloco `Codigos cashback rapido` existe em `Novo cliente` e `Gastar/Usar Cashback` e aceita de um a dez codigos na mesma compra. O navegador permite adicionar/remover campos, valida cada codigo e mostra a soma; o servidor continua sendo a fonte de verdade, aceita tambem o campo escalar antigo, rejeita repeticoes e trava todos os vouchers ativos em ordem deterministica com `FOR UPDATE`. A compra minima configurada (4x por padrao) e calculada e arredondada uma unica vez sobre a soma dos beneficios. Cada codigo submetido conta como uma tentativa no limitador persistente. Todos os codigos sao consumidos na mesma transacao, vinculados ao mesmo resgate, e geram uma unica compra, um unico sucessor sobre o valor efetivamente cobrado e um unico comprovante. Se qualquer codigo estiver invalido, usado, vencido ou pertencer a outro cliente, a transacao inteira e revertida. O primeiro conjunto anonimo pode cadastrar o cliente; o sucessor fica vinculado a ele. Voucher rapido nao cria `cashback_credits` comuns nem duplica saldo. Vouchers e creditos comuns novos vencem seis meses corridos apos a data de geracao. Uma combinacao de cinco digitos usada ou cancelada permanece reservada ate `expires_at` e so volta ao sorteio depois do vencimento.

Desde 2026-07-25, o formulario `Novo cliente` preseleciona o usuario logado, mas permite escolher outro atendente elegivel. O POST `save_client` revalida `atendente_id`, confirma cliente e eventual compra em transacao, e somente depois do `COMMIT` tenta lancar +250 XP para o usuario escolhido com origem idempotente `cashback_client_creation`. A acao unica `Cadastrar e imprimir` inclui `print_after_save=1`; quando existe cashback gerado, o redirecionamento autoriza o recibo na sessao e solicita a impressao local uma vez. Sem comprovante, o cadastro termina normalmente sem abrir papel vazio.

Desde 2026-07-28, a identificacao do novo cliente exige `nome` ou `telefone`, e nao mais obrigatoriamente o nome. A validacao combinada existe no navegador e no servidor. Cadastro somente com nome grava telefone nulo; cadastro somente com telefone usa `Cliente <telefone>` como nome operacional para preservar o schema, a busca e os comprovantes.

Consultas e tentativas de resgate compartilham limites persistentes por usuario, inclusive entre abas e processos. `Configuracao e Relatorio` mostra o passivo de codigos ativos e separa cashback comum gerado/usado dos sucessores e usos de voucher rapido.
...
- `/cashback/autoteste.php` confirma o atendente vinculado ao usuario logado, cria compra/credito/resgate e tambem emite dois vouchers rapidos, consome os dois no mesmo resgate, confirma um unico sucessor e desfaz todos os dados temporarios com rollback;
...
- notas ficam em grade responsiva, com topo compacto sem rotulo duplicado, ate cinco lembretes por fileira no desktop e rodape menor para caber mais conteudo;
- cada nota pode ser editada, apagada por exclusao logica e reordenada por arrastar pelo mouse; desde 2026-06-13, o arrasto usa ponteiro com pequena distancia minima antes de pegar a nota, evitando piscar em clique simples, o botao de mover tambem aceita setas quando focado, e `Salvar`/`Apagar` ficam no mesmo rodape compacto do card;
...
- contas ligadas a Pedidos por `gestao_supplier_orders`, `pedidos_orders` ou `pedidos_confirmed_orders` saem da lista geral e aparecem no bloco visual `Pedidos` junto dos demais blocos da Gestao, preservando categoria `Boleto`, pagamentos, totais, status, busca/filtro e auditoria;
- no desktop, `Nova conta`, `Contas abertas`, `Mensal`, `Pedidos`, `Categorias` e `Notas` ficam soltos em uma unica faixa horizontal, um ao lado do outro, sem container interno limitando a largura; se nao couber, a rolagem lateral deve ser a do proprio navegador. Em telas menores, os paineis continuam empilhando sem quebrar o fluxo;
- o bloco visual `Pedidos` dentro da Gestao deve crescer verticalmente no fluxo da pagina, sem rolagem interna ou limite artificial de altura, para nao esconder pedidos; o painel `Categorias` deve mostrar resumo de abertas/fechadas com valores e linhas legiveis por categoria, preservando filtros e acoes em lote;
- o painel `Mensal` mostra as contas da competencia atual com `Repetir mes que vem` ativo em estrutura compacta propria para evitar sobreposicao entre valor e botoes, soma o valor que sera levado para a proxima competencia, permite abrir o extrato completo e salva a ordem manual quando o operador arrasta as linhas com o mouse;
- a busca fica abaixo dos resumos do mes e pesquisa contas/boletos por texto livre combinavel, normalizando espacos, acentos e caixa. Ela considera titulo, fornecedor/distribuidor dos Pedidos, responsaveis do core, categoria, status (`aberto`, `pago`, `vencido`), tipo/origem (`boleto`, `pedido vinculado`, `mensal`), valor exato ou aproximado, saldo, vencimento, datas de lancamento/pagamento/origem, observacao, parcelas/lancamentos e auditoria resumida; ela mostra 10 resultados primeiro, permite `Mostrar mais` e `Limpar`, e o termo vazio volta ao estado normal sem gravar nada;
...
- `Repetir tantos meses` no cadastro manual aceita uma quantidade numerica e mostra uma previa das competencias/datas; no backend, a criacao encadeia a mesma garantia do proximo mes para ate 24 meses futuros dentro da mesma transacao;
...
- desde 2026-06-11, a busca de Pedidos filtra enquanto digita, sem Enter, em `Aguardando chegada`, `Confirmados` e `Historico` do mes selecionado; considera fornecedor, valores/saldo, vencimentos, previsao de chegada, criacao, status, parcelas, observacao, responsaveis e historico/auditoria, normalizando acentos, caixa, espacos, datas `dd/mm` e valores com virgula/ponto. `Limpar` volta a visualizacao normal e a busca nao executa escrita;
- o `Historico` permite corrigir erro humano no card aberto: voltar para `Confirmados`, voltar para `Aguardando chegada` quando o registro veio de um pedido aguardando chegada, ou excluir da tela por arquivamento logico. Essas acoes usam CSRF, transacao e auditoria, e preservam pagamentos, itens, saldos registrados e historico financeiro;
- no painel do lapis de `Aguardando chegada` e `Confirmados`, o operador pode alterar fornecedor, editar cada parcela ativa mesmo quando ja existe pagamento vinculado, retirar parcelas por cancelamento logico quando nao houver pagamento vinculado e adicionar nova parcela com valor, vencimento opcional e nome opcional; edicoes e novas parcelas usam `gestao_account_items`, recalculam total/vencimento/status e preservam auditoria;
- desde 2026-06-13, corrigir uma parcela ja paga nao remove nem altera linhas em `gestao_account_payments`: se o novo total ficar maior que o pago, o saldo reabre e o pedido volta para fluxo aberto quando aplicavel; se ficar menor que o pago, o pagamento ja registrado continua preservado no financeiro/auditoria e o saldo exibido fica zerado;
