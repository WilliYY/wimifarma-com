...
Desde 2026-07-28, o historico recolhivel do `Cashback rapido` calcula um resumo de producao por `issued_attendant_id` sobre todas as emissoes diretas. Cada usuario mostra total cadastrado, ativos, usados, vencidos, cancelados, soma historica de cashback emitido e data do ultimo cadastro. Vouchers sucessores, identificados por `parent_voucher_id` ou `source_purchase_id`, continuam nos 20 registros recentes para conferencia, mas ficam fora do resumo para nao contar como um novo cadastro rapido feito pelo usuario. A consulta e somente leitura e nao altera voucher, XP, impressao ou auditoria.
