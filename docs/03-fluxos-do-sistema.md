...
- cards em `Aguardando chegada` e `Confirmados` ficam minimizados por padrao e podem ser abertos ao clicar no resumo do proprio card, sem botao extra de `+/-`; no modo reduzido, mostram status compacto, saldo e acao principal (`Confirmar chegada` ou `Pago`), enquanto o icone de lapis abre a edicao de fornecedor/valores/vencimentos ativos com auditoria em `gestao_audit_events` e espelho em `wf_logs`; o visual minimizado deve ser denso, com padding/chips/botoes menores para caber mais pedidos por tela;
- os cards-resumo do topo de Pedidos tambem devem usar altura compacta para ampliar a area visivel das colunas operacionais;
