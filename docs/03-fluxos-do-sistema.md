...
Desde 2026-07-21, `Cashback rapido` aparece antes da consulta do Balcao. O operador informa o valor gasto e seleciona o usuario que imprime; o servidor recalcula o percentual padrao em centavos, gera um codigo aleatorio textual de cinco digitos e cria um voucher de seis meses sem exigir cliente. Codigos historicos de quatro digitos continuam aceitos enquanto validos. O recibo usa 76 mm uteis em rolo de 80 mm, a marca completa `+ | WimiFarma` de `logo-wimifarma-receipt.png`, valor, codigo, data real de vencimento, WhatsApp, endereco e atendente. Existe somente `Imprimir`: com a estacao Wimi online, o comprovante entra na fila HTTPS; sem estacao, abre o dialogo local. Nenhum caminho consegue confirmar fisicamente que o papel saiu.

O card `Wimi Impressora` aparece na Home e na navegacao do Cashback apenas para o login mestre `adm`. No Chrome do PC ligado por USB, o ADM ativa uma estacao web e mantem a tela dedicada aberta; nao existe EXE nem servico para instalar. O navegador recebe cookie de dispositivo protegido, consulta a fila por HTTPS e usa Web Locks para que somente uma aba reserve trabalhos. O atalho `.cmd` opcional abre o Chrome com impressao automatica e pode ser colocado em `shell:startup`. Depois disso, qualquer operador autenticado no Cashback pode enviar comprovantes autorizados para a estacao online. O painel mostra fila, falhas e trabalhos incertos; reimpressao e sempre manual para evitar duplicidade depois de queda de energia, fechamento da aba ou reinicio.
