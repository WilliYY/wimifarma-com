...
- contas ligadas a Pedidos por `gestao_supplier_orders`, `pedidos_orders` ou `pedidos_confirmed_orders` saem da lista geral e aparecem no bloco visual `Pedidos` na esteira horizontal da Gestao, preservando categoria `Boleto`, pagamentos, totais, status, busca/filtro e auditoria;
- os paineis operacionais da Gestao ficam em uma esteira horizontal na ordem `Contas abertas`, `Mensal`, `Pedidos`, `Categorias` e `Notas`, com rolagem inferior quando passar da largura da tela; no mobile a esteira continua horizontal para nao empilhar esses paineis. O painel `Mensal` mostra as contas da competencia atual com `Repetir mes que vem` ativo em estrutura compacta propria para evitar sobreposicao entre valor e botoes, soma o valor que sera levado para a proxima competencia, permite abrir o extrato completo e salva a ordem manual quando o operador arrasta as linhas com o mouse;
