...
- A home `/` e os modulos Node publicados por proxy devem manter headers de seguranca basicos nas respostas HTML e APIs: `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `Content-Security-Policy` e `Strict-Transport-Security` quando a requisicao publica for HTTPS.
...
- Manter o proxy Apache de `/cashback/` para `wimifarma-cashback-app:4000`; o Nginx Proxy Manager continua apontando somente para `wimifarma-com-web:80`. `site/cashback` preserva assets e helpers PHP ainda usados pelo Miauby, mas bloqueia PHP direto por HTTP via `.htaccess`; financeiro antigo dentro de Cashback fica arquivado em `site/_legacy-disabled/2026-05-29/cashback-financeiro-php/`.
...
- Manter o proxy Apache de `/tarefa/` para `wimifarma-tarefa-app:3500`; o Nginx Proxy Manager continua apontando somente para `wimifarma-com-web:80`. O PHP legado em `site/tarefa` nao deve voltar a ser fonte oficial sem rollback deliberado e fica bloqueado por `.htaccess` para acesso web direto.
...
- Definir `COTACAO_POSTGRES_PASSWORD` e `COTACAO_SESSION_SECRET` no `.env` de cada ambiente antes de subir a Cotacao V2. `COTACAO_SESSION_COOKIE_SECURE` pode forcar `true`/`false`; vazio usa `auto` em producao e `false` em desenvolvimento para manter proxy HTTPS e testes locais compativeis.
...
- Para Cashback, definir `CASHBACK_POSTGRES_PASSWORD` e `CASHBACK_SESSION_SECRET` no `.env` de cada ambiente. O app usa `core_users` como login unico; nao ha `CASHBACK_AUTH_PROVIDER`, `mysql2`, importador, espelho, logs ou fallback MySQL desde 2026-05-30. `CASHBACK_INTERNAL_TOKEN` pode reutilizar `MIAUW_GUARDIAN_TOKEN` para o Miauby consultar endpoints internos; sem token, `/cashback/internal/*` e `/cashback/api/internal/*` devem recusar. Desde 2026-06-11, Cashback tambem recebe `XP_POSTGRES_HOST`, `XP_POSTGRES_PORT`, `XP_POSTGRES_DB`, `XP_POSTGRES_USER` e `XP_POSTGRES_PASSWORD` para gerar +500 XP idempotente quando o operador usa cashback; se XP estiver indisponivel, a compra/resgate continua e registra aviso/auditoria. Rollback MySQL exige restaurar commit/imagem anterior e backup validado.
- Nao configurar `WIMI_PRINTER_PUBLIC_STATION_URL`, publicar agente, atalho ou estacao web. Desde 2026-07-25, o Cashback imprime apenas pelo dialogo local do navegador; confira o driver e a Bematech no computador que executa a operacao. O historico e a retirada segura estao em `docs/31-wimi-impressora.md`.
...
- Para lembretes Miauby de Tarefa, manter `TAREFA_MIAUW_WHATSAPP_TIMEOUT_MS=25000` ou maior que o tempo normal do anti-flood/provider, e `TAREFA_REMINDER_IN_FLIGHT_GRACE_MINUTES=15` para impedir reprocessamento de tentativa em andamento. O bridge bloqueia repeticao do mesmo `reminder_id`/`dedupe_key`.
- `TAREFA_INTERNAL_TOKEN` libera endpoints internos do Tarefa para Miauby/WhatsApp e outros fluxos tokenizados; pode reaproveitar `MIAUW_GUARDIAN_TOKEN`, `MIAUW_AGENT_INTERNAL_TOKEN` ou `MIAUW_WHATSAPP_INTERNAL_TOKEN` se o ambiente ja usa um desses segredos.
- Para XP, definir `XP_POSTGRES_PASSWORD` e `XP_SESSION_SECRET` no `.env` de cada ambiente. O login usa somente `core_users`. Desde 2026-05-30 nao ha `XP_AUTH_PROVIDER`, flags `XP_LEGACY_MYSQL_*`, importador, espelho, logs, fallback `wf_users` ou dependencia `mysql2`; rollback MySQL exige restaurar versao anterior e backup validado.
...
- Para o Miauby interno sombra, definir `MIAUBY_POSTGRES_PASSWORD` no `.env`, manter `wimifarma-com-db` e `wimifarma-miauby-db` ativos, e rodar `sh scripts/miauby-shadow-migrate.sh migrate` seguido de `sh scripts/miauby-shadow-migrate.sh validate`. O script constroi e executa um `wimifarma-miauby-migrator` descartavel com `--no-deps`, mantendo o runtime `wimifarma-miauby-app` sem credenciais/conexao MySQL. Definir `MIAUBY_INTERNAL_TOKEN` ou garantir `MIAUW_GUARDIAN_TOKEN`/`MIAUW_AGENT_INTERNAL_TOKEN` para `wimifarma-miauby-app` validar `/miauby/api/internal/status`, `/miauby/api/internal/parity`, `/miauby/api/internal/readiness` e `/miauby/api/internal/context`. Esse fluxo so cria/copia dados sanitizados e grava o resultado de paridade em `miauby_migration_runs`; nao altera chat, widget, treino, diagnostico ou engine oficial. Para acelerar a validacao sem expor segredo, rodar `sh scripts/miauby-shadow-smoke.sh 20 3` no VPS apos rebuild.
- Para a Etapa 7A do Miauby, escrita real em Postgres fica restrita a conversa/mensagem e exige dupla trava: `MIAUBY_WRITES_ENABLED=true` no `wimifarma-miauby-app` mais `MIAUBY_WRITE_ADAPTER_REAL_MESSAGE_ENABLED=true` e/ou `MIAUBY_WRITE_ADAPTER_REAL_CONVERSATION_ENABLED=true`. O PHP so chama essa escrita se `MIAUBY_WRITE_ADAPTER_INTERNAL_URL` apontar para `http://wimifarma-miauby-app:4100/miauby/api/internal/write-adapter/commit`; o default seguro continua `/dry-run`. Rollback imediato: `MIAUBY_WRITES_ENABLED=false`, flags especificas `false` e URL de volta para `/dry-run`.
- Para o Miauby WhatsApp, definir `MIAUW_WHATSAPP_POSTGRES_PASSWORD`, `MIAUW_WHATSAPP_WEBHOOK_TOKEN`, `MIAUW_WHATSAPP_ENCRYPTION_KEY`, `MIAUW_WHATSAPP_ALLOWED_SENDERS`, `MIAUW_WHATSAPP_PROVIDER`, `MIAUW_WHATSAPP_DASHBOARD_USER` e `MIAUW_WHATSAPP_DASHBOARD_PASSWORD` antes de usar webhook real. Com `MIAUW_WHATSAPP_PROVIDER=evolution`, preencher `EVOLUTION_API_BASE_URL`, `EVOLUTION_API_KEY` e `EVOLUTION_API_INSTANCE`. Com `MIAUW_WHATSAPP_PROVIDER=meta`, preencher `META_WHATSAPP_ACCESS_TOKEN`, `META_WHATSAPP_PHONE_NUMBER_ID`, `META_WHATSAPP_WEBHOOK_VERIFY_TOKEN`, `META_WHATSAPP_APP_SECRET` e, se necessario, `META_WHATSAPP_GRAPH_API_VERSION`. O default versionado continua `MIAUW_WHATSAPP_ENABLED=false`; no VPS, a ativacao pode ser feita por `.env` e validada em `/miauw/whatsapp/health`, com login em `/miauw/whatsapp/login`, e com token interno em `POST /miauw/whatsapp/internal/evolution-status` quando o provider for Evolution.
- Para n8n no Miauby WhatsApp, preencher `MIAUW_WHATSAPP_N8N_ENABLED=true`, `MIAUW_WHATSAPP_N8N_BASE_URL`, `MIAUW_WHATSAPP_N8N_WEBHOOK_BASE_URL` e `MIAUW_WHATSAPP_N8N_WEBHOOK_SECRET` quando a stack estiver pronta. Na stack `ops/n8n`, manter `N8N_BLOCK_ENV_ACCESS_IN_NODE=false`, porque os workflows versionados leem `$env.WIMIFARMA_INTERNAL_BASE_URL` e `$env.MIAUW_GUARDIAN_TOKEN` para chamar endpoints internos sem gravar segredo no JSON. A rotina `Chegada de pedidos` usa `MIAUW_WHATSAPP_PEDIDOS_INTERNAL_BASE_URL` para consultar Pedidos, e o toggle de ativar/desativar fica no painel `/miauw/whatsapp/`.
...
- O core de autenticacao em Postgres usa `wimifarma-core-db` para `core_users`, `core_audit_logs` e `core_login_rate_limits`, e `wimifarma-core-migrator` sincroniza `wf_users`. Cotacao, Gestao, Pedidos, Tarefa, Cashback e Financeiro usam core sem fallback MySQL; Miauby PHP usa core por padrao, com fallback MySQL apenas como rollback opt-in onde existir.
...
- Backups manuais da tela da Cotacao V2 ficam em `cotacao-data/backups`. O timer `wimifarma-cotacao-backup.timer` gera diariamente um dump Postgres customizado em `cotacao-data/automatic-backups`, valida com `pg_restore --list`, grava SHA-256 e retem 30 dias. Ambos ficam fora do Git; ainda e necessaria uma copia externa/off-host.
...
- A migracao sombra do Miauby interno adiciona `wimifarma-miauby-db`, `wimifarma-miauby-migrator` e `wimifarma-miauby-app`. Deploys dessa fase devem subir o banco, rodar o migrador descartavel, validar contagens/checksums e paridade interna tokenizada. Desde 2026-06-02, o runtime `wimifarma-miauby-app` usa somente Postgres e snapshot do ultimo `validate`; MySQL fica restrito ao migrador manual. Desde 2026-05-31, o corte canonico seguro publica apenas aliases `/miauby/`, `/miauby/agent/` e `/miauby/whatsapp/`; o app sombra `wimifarma-miauby-app` continua sem proxy publico.
...
- Replicar os dumps automaticos de `cotacao-data/automatic-backups` para um destino externo/off-host e testar restauracao periodica fora do banco ativo.
