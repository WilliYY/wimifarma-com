...
- `PEDIDOS_INTERNAL_TOKEN` pode reutilizar `MIAUW_GUARDIAN_TOKEN` para expor `GET /pedidos/api/internal/arrival-summary` e `POST /pedidos/api/internal/confirm-arrival` ao Miauby WhatsApp. Sem token, esses endpoints recusam e a rotina n8n de chegada de pedidos nao executa baixa.
