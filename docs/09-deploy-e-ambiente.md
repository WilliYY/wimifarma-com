...
- Para Wimi Impressora, publicar `WimiImpressoraSetup.exe` em `wimi-printer-release/` no projeto oficial do VPS antes de subir o Cashback. Essa pasta e ignorada pelo Git e montada somente leitura em `/opt/wimi-impressora`. `WIMI_PRINTER_INSTALLER_VERSION` precisa bater com `AppConstants.Version` do agente. Se o arquivo faltar, o painel continua funcional, mas o download fica indisponivel. Build, envio e teste fisico estao em `docs/31-wimi-impressora.md`.
