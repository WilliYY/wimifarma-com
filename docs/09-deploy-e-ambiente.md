...
- Para Cashback, definir `CASHBACK_POSTGRES_PASSWORD` e `CASHBACK_SESSION_SECRET` no `.env` de cada ambiente. `CASHBACK_AUTH_PROVIDER=core` usa `core_users` como login oficial; rollback de autenticacao exige voltar `CASHBACK_AUTH_PROVIDER=mysql` e reintroduzir credenciais MySQL. `CASHBACK_INTERNAL_TOKEN` pode reutilizar `MIAUW_GUARDIAN_TOKEN` para o Miauby consultar endpoints internos; sem token, `/cashback/internal/*` e `/cashback/api/internal/*` devem recusar. `CASHBACK_LEGACY_MYSQL_IMPORT_ENABLED=false`, `CASHBACK_LEGACY_MYSQL_MIRROR_ENABLED=false` e `CASHBACK_LEGACY_MYSQL_LOGS_ENABLED=false` sao o padrao desde 2026-05-29; ligar essas flags e rollback manual, nao deploy normal.
...
- `TAREFA_AUTH_PROVIDER=core` e o default do login oficial de Tarefa em `core_users`; rollback e voltar `TAREFA_AUTH_PROVIDER=mysql` e rebuildar apenas `wimifarma-tarefa-app`. `TAREFA_CORE_AUTH_SHADOW_ENABLED=true` fica para comparacao em ambiente que ainda use MySQL e pode voltar a `false` depois.
...
- `TAREFA_LEGACY_MYSQL_IMPORT_ENABLED=false`, `TAREFA_LEGACY_MYSQL_MIRROR_ENABLED=false` e `TAREFA_LEGACY_MYSQL_LOGS_ENABLED=false` sao o padrao desde 2026-05-30. Com as tres em `false` e `TAREFA_AUTH_PROVIDER=core`, o health do Tarefa nao deve depender de MySQL; rollback MySQL exige religar flags/provedor e reintroduzir credenciais MySQL explicitamente.
...
- O core de autenticacao em Postgres usa `wimifarma-core-db` para `core_users`, `core_audit_logs` e `core_login_rate_limits`, e `wimifarma-core-migrator` sincroniza `wf_users`. Cotacao e Pedidos usam core sem fallback MySQL; Cashback Node, Gestao, Tarefa e Miauby PHP usam core por padrao, com fallback MySQL apenas como rollback opt-in onde existir.
- Cashback roda fora do PHP/WordPress: Apache faz proxy de `/cashback/` para Node, Node usa Postgres para clientes, atendentes, compras, creditos, resgates, mensagens, auditoria e sessoes; MySQL fica desligado por padrao e so volta em rollback manual com `CASHBACK_LEGACY_MYSQL_*`.
...
- Tarefa roda fora do PHP/WordPress: Apache faz proxy de `/tarefa/` para Node, Node usa Postgres para tarefas, auditoria e sessoes, e usa `core_users` como login oficial por padrao. MySQL fica desligado no runtime normal e so volta em rollback manual com flags/provedor/credenciais explicitas.
...
- Apagar `core-data/` remove a autenticacao/auditoria compartilhada em Postgres. Cotacao, Pedidos, Cashback, Miauby PHP e os modulos Node com auth core param de autenticar; Gestao/Tarefa so ficam operaveis se rollback MySQL estiver ligado explicitamente e saudavel.
