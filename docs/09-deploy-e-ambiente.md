...
- Para o Miauby interno sombra, definir `MIAUBY_POSTGRES_PASSWORD` no `.env`, subir `wimifarma-com-db` e `wimifarma-miauby-db`, e rodar `sh scripts/miauby-shadow-migrate.sh migrate` seguido de `sh scripts/miauby-shadow-migrate.sh validate`. O script usa `exec` no `wimifarma-miauby-app` quando possivel e usa `docker compose run --rm --no-deps` como fallback para nao tentar recriar `wimifarma-com-db` em VPS com containers antigos/labels Compose diferentes. Definir `MIAUBY_INTERNAL_TOKEN` ou garantir `MIAUW_GUARDIAN_TOKEN`/`MIAUW_AGENT_INTERNAL_TOKEN` para `wimifarma-miauby-app` validar `/miauby/api/internal/status`, `/miauby/api/internal/parity`, `/miauby/api/internal/readiness` e `/miauby/api/internal/context`. Esse fluxo so cria/copia dados sanitizados e compara paridade em `wimifarma_miauby`; nao altera chat, widget, treino, diagnostico ou engine oficial. Para acelerar a validacao sem expor segredo, rodar `sh scripts/miauby-shadow-smoke.sh 20 3` no VPS apos rebuild.
