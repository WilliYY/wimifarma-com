...
- `PEDIDOS_AUTH_PROVIDER=core` usa `core_users` como login oficial de Pedidos; rollback e voltar `PEDIDOS_AUTH_PROVIDER=mysql` e rebuildar `wimifarma-pedidos-app`. `PEDIDOS_AUTH_MYSQL_FALLBACK_ENABLED=false` e o default; ligar `true` apenas como rollback temporario em `wf_users`. `PEDIDOS_CORE_AUTH_SHADOW_ENABLED=true` fica para comparacao somente quando o provider estiver em `mysql`.
