...
- Para Wimi Impressora Web, nao publicar EXE nem montar `wimi-printer-release/`. O Cashback serve a estacao diretamente em `/cashback/internal/print-station`; `WIMI_PRINTER_PUBLIC_STATION_URL` e opcional e deve apontar para essa URL HTTPS publica quando o dominio mudar. Depois do deploy, o `adm` baixa e abre o atalho `.cmd` no computador da Bematech, ativa a estacao dentro do perfil exclusivo do Chrome aberto por ele e faz teste fisico controlado. Operacao e seguranca estao em `docs/31-wimi-impressora.md`.
