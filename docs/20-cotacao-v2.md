...
- O endpoint interno `GET /cotacao/api/internal/encomendas` lista somente linhas ativas cujo texto atual contem uma variacao segura de encomenda (`encomenda`, `encomendar`, `encomendado`, `enc.`), ordena por data mais antiga por padrao, usa `cotacao_v2_encomenda_reminders.detected_at` como data exata quando existir e informa fallback quando precisa usar `updated_at`/`created_at` da linha. A extracao preserva o texto original para auditoria, normaliza espacos e separa observacao antes/depois da palavra sem inventar dados.
