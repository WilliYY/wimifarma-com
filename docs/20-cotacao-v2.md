...
O `POST /cotacao/api/internal/encomendas` exige token, ator ativo com acesso ao card Cotacao e `request_id`. A linha, o evento e a auditoria em `cotacao_v2_miauby_encomenda_commands` sao gravados na mesma transacao; `(source, request_id)` impede duplicacao por retry ou confirmacao repetida. A linha nova usa somente `produto` e `categoria`: todo contexto util e ordenado em `Encomenda | ...`, inclusive a quantidade do pedido, sem criar colunas ou campos novos. O payload antigo com `quantidade`, `responsavel`, `telefone` e `categoria_extra` continua aceito como compatibilidade e tambem e convertido para essas duas colunas. O cliente informado nunca e substituido pelo usuario autenticado: o primeiro pertence a Encomenda, enquanto o segundo fica como ator da auditoria. Ambos os canais continuam exigindo confirmacao humana antes da escrita.
