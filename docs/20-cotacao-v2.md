...
Desde 2026-08-16, `apps/cotacao/src/falteiro-command.js` e a fonte unica de interpretacao dos comandos do Falteiro usados pelo Miauby Interno e pelo Miauby WhatsApp. O parser e case-insensitive, aceita `miauby`/`miauw` opcional e reconhece intencao clara de falta de estoque ou necessidade de compra em frases como `falta`, `acabou`, `acabando`, `esta faltando`, `estamos sem`, `sem estoque`, `nao temos`, `nao tem mais`, `terminou`, `comprar`, `repor`, `reposicao` e `coloca no falteiro`, inclusive com o produto antes da intencao. Perguntas, negacoes, comandos de chegada e indisponibilidades comuns como internet ou energia nao viram escrita.

Dosagem e apresentacao permanecem no produto. A cada comando, o endpoint monta o catalogo de categorias a partir das categorias nao vazias de linhas ativas que tambem tenham produto e das regras cadastradas para a coluna `categoria`; valores numericos ou vazios sao ignorados. O parser compara a frase inteira com esse catalogo por conjuntos de conceitos, independentemente de posicao, caixa, acento, pontuacao ou ordem natural. Aliases como `urgencia`/`prioridade`, `cotacao`/`cota`/`cotar`, `faltando`/`acabou`/`sem estoque` e `linha popular` ajudam a reconhecer o conceito, mas nunca alteram o rotulo salvo. Categorias compostas usam correspondencia mais especifica primeiro: se `Urgente`, `Popular` e `Urgente Popular` existirem, a frase com os dois termos escolhe o rotulo composto exato. O nome oficial e preservado e categoria nova nunca e inventada.

Intencao, categoria, aliases e conectivos comprovadamente contextuais sao retirados antes de reconstruir o produto com sua dosagem/apresentacao. Um termo pode participar da intencao e de uma categoria composta: em `falta belfaren cp urgente cotar`, a primeira `falta` abre o Falteiro e tambem completa a categoria oficial `Urgente Falta Cotar`, sem transformar a categoria simples `Falta` em padrao. Se restarem termos claros de categoria sem uma correspondencia unica no catalogo, o endpoint responde `category_not_found` ou `category_ambiguous`, lista as categorias reais disponiveis e nao grava produto nem categoria. Assim, `urgente popular` nunca e reduzido silenciosamente para apenas `Urgente`.
