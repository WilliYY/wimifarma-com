...
Desde 2026-08-22, `apps/cotacao/src/falteiro-command.js` e a fonte unica de interpretacao dos comandos do Falteiro usados pelo Miauby Interno e pelo Miauby WhatsApp. O parser e case-insensitive, aceita `miauby`/`miauw` opcional e reconhece intencao clara de falta de estoque, estoque baixo, compra ou reposicao em frases como `falta`, `acabou`, `zerou`, `esta acabando`, `estoque baixo`, `so temos 2 caixas`, `ultima caixa`, `vai acabar`, `sem estoque`, `nao temos`, `comprar`, `repor`, `reposicao` e `coloca no falteiro`, inclusive com o produto antes da intencao. Perguntas, negacao da acao, comandos de chegada e indisponibilidades comuns como internet ou energia nao viram escrita.

O campo `Produto` guarda somente a identidade comercial necessaria: nome, dosagem, apresentacao, quantidade da embalagem, tamanho, modelo e marca quando ela for obrigatoria. O campo `Categoria` recebe o contexto operacional util separado por ` | `: categoria oficial, estado do estoque, quantidade a comprar, prazo, limite de preco, preferencia, demanda comercial e observacoes. Assim, `30cp`, `20ml` e `FPS 60 200ml` continuam no produto, enquanto `comprar 5 caixas`, `amanha`, `ate R$ 8`, `vende muito` e `nao pegar validade curta` ficam na categoria.

A cada comando, o endpoint monta o catalogo de categorias a partir das categorias nao vazias de linhas ativas que tambem tenham produto e das regras cadastradas para a coluna `categoria`; valores numericos ou vazios sao ignorados. O parser compara a frase inteira com esse catalogo por conjuntos de conceitos, independentemente de posicao, caixa, acento, pontuacao ou ordem natural. Categorias compostas usam correspondencia mais especifica primeiro: se `Urgente`, `Popular` e `Urgente Popular` existirem, a frase com os dois termos escolhe e preserva exatamente o rotulo composto real. Contextos complementares nao criam categoria oficial nova: apenas acompanham o rotulo reconhecido no mesmo campo operacional.

Intencao, categoria, contexto e conectivos sao retirados antes de reconstruir o produto, sem apagar apresentacao nem marca obrigatoria. Quantidade de compra so e separada quando ligada a uma acao como `comprar`, `pegar` ou `repor`; numero de embalagem continua no produto. Marca em `somente EMS` fica no produto, enquanto `se tiver EMS melhor`, `qualquer laboratorio` e `nao pegar Medley` ficam na categoria. Demanda geral, como `tres clientes perguntaram por`, entra no Falteiro; reserva para uma pessoa especifica continua sendo Encomenda. Negacao de acao, como `nao coloca` ou `nao acabou`, bloqueia; restricao negativa, como `nao e urgente`, e preservada. Categoria oficial incerta responde `category_not_found` ou `category_ambiguous` e nao grava.
...
Exemplo completo: `Miauby amanha ate 8 reais comprar 5 caixas urgente losartana 50 EMS porque esta acabando e vende muito` grava `Produto = Losartana 50 EMS` e `Categoria = Urgente | Esta acabando | Vende muito | Comprar 5 caixas | Amanha | Ate R$ 8`. A resposta curta vem do mesmo endpoint e confirma exatamente o que foi gravado.

## Encomenda contextual pelo Miauby

Desde 2026-08-22, Miauby Interno e Miauby WhatsApp reutilizam `miauw_skill_cotacao_encomenda_command_from_message()` para interpretar a frase inteira sem depender da ordem ou da caixa. A intencao aceita `encomenda`, `encomendar`, `encomendou`, `cliente pediu`, `cliente quer`, `pedido do cliente`, `pedido para`, `reserva para`, `separa para`, `deixar separado para` e `guardar para`. Produto, dosagem, marca obrigatoria e apresentacao permanecem em `produto`; nome composto, telefone, quantidade do pedido, endereco, bairro, referencia, entrega/retirada, data, horario, prioridade, preferencia de marca e observacao livre sao entidades de contexto. Cliente e telefone sao opcionais, mas produto e obrigatorio. Frase realmente ambigua, como `Miauby encomenda Maria`, pede somente o produto e nao grava.

O `POST /cotacao/api/internal/encomendas` exige token, ator ativo com acesso ao card Cotacao e `request_id`. A primeira linha ativa integralmente vazia e reservada com `FOR UPDATE`; linhas parcialmente preenchidas nunca sao reutilizadas e a falta de espaco retorna conflito sem criar linha no fim. A linha, o evento `cells_batch_updated` e a auditoria em `cotacao_v2_miauby_encomenda_commands` sao gravados na mesma transacao; `(source, request_id)` impede duplicacao por retry ou confirmacao repetida. A gravacao usa somente `produto` e `categoria`: todo contexto util e ordenado em `Encomenda | ...`, inclusive a quantidade do pedido e observacoes como `valor 57`, sem criar colunas ou campos novos. O payload antigo com `quantidade`, `responsavel`, `telefone`, `observacao` e `categoria_extra` continua aceito como compatibilidade e tambem e convertido para essas duas colunas. O cliente informado nunca e substituido pelo usuario autenticado: o primeiro pertence a Encomenda, enquanto o segundo fica como ator da auditoria. Ambos os canais continuam exigindo confirmacao humana antes da escrita.

Exemplos equivalentes: `Miauby encomenda losartana 50 Maria 44 4984894`, `Miauby Maria encomenda losartana 50 44 4984894`, `Miauby Maria pediu losartana 50 e vai buscar amanha` e `Miauby guardar amoxicilina 500mg para Maria`. `30cp` e `36 unidades` quando identificam a apresentacao permanecem no produto; `2 caixas` quando representa a quantidade pedida entra na categoria. `Miauby encomenda Bepantol` tambem e valido e salva somente `Encomenda` na categoria. Consultas como `lista encomendas`, `o que tem de encomenda` e `pedidos encomenda` continuam somente leitura.

Desde 2026-08-22, `apps/cotacao/src/encomenda-query.js` e a camada autoritativa das consultas naturais de Encomenda. O `GET /cotacao/api/internal/encomendas` aceita `q`, interpreta a frase inteira sem depender de caixa ou ordem e filtra por produto parcial, cliente, telefone, categoria/contexto e status, inclusive em combinacoes. Consultas normais retornam somente linhas atuais nao finalizadas; `todas`, `antigas`, `historico`, `canceladas` e equivalentes incluem o historico real preservado em `cotacao_v2_encomenda_reminders`, sem inventar registros nem criar uma tabela paralela. Produto continua sendo a informacao principal; cliente, telefone, endereco, previsao, status, categoria completa e data aparecem somente quando puderem ser extraidos dos dados existentes. Referencias conversacionais usam o `row_id` UUID validado para reler exatamente a linha antes de responder, sem depender da posicao no limite da lista. A rota e estritamente de leitura e nunca confirma, cria, edita, cancela ou muda status.
...
- `GET /cotacao/api/internal/encomendas` para listar encomendas ativas da Cotacao por token, sem alterar linhas nem lembretes
...
- `cotacao_v2_encomenda_reminders`: historico operacional criado quando uma linha contem `encomenda`, com cotacao, linha, texto original, produto/quantidade extraidos, data de deteccao, envio previsto legado, destinatarios mascarados, status (`pendente`, `enviado`, `erro`, `cancelado`, `resolvido`), tentativas e resultado do Miauby Whats. Importacoes Google Sheets e restores de backup recalculam esses registros para manter data/contexto e cancelar pendentes de linhas removidas, sem apagar historico.
- Desde 2026-06-12, o worker individual de encomendas dentro da Cotacao ficou legado desativado por padrao (`COTACAO_ENCOMENDA_LEGACY_REMINDER_ENABLED=false`) e nao envia WhatsApp. O endpoint interno `GET /cotacao/api/internal/encomenda-reminders/status` continua somente leitura para diagnosticar esse historico legado, sem processar lembrete.
- O endpoint interno `GET /cotacao/api/internal/encomendas` usa linhas atuais por padrao e aceita a consulta natural em `q`; somente pedidos explicitos de historico agregam registros reais de `cotacao_v2_encomenda_reminders`. Ele ordena por data mais antiga por padrao, usa `detected_at` quando existir e informa fallback quando precisa usar `updated_at`/`created_at`. A extracao preserva texto e categoria completos para auditoria, normaliza apenas para comparar e nunca inventa campos.
- O Miauby WhatsApp consulta esse endpoint para `miauby encomendas` e variacoes de lista, e tambem para o resumo diario das 17h. A rotina usa permissao pelo card `Cotacao`, respeita allowlist/ferias, nao altera a planilha, nao cancela lembrete e nao marca entrega.
...
- Excecao atual de 2026-06-12: encomenda entra no resumo diario unico do Miauby Whats as 17h em `America/Sao_Paulo`. O resumo inclui linhas ativas com termo de encomenda na categoria ou texto relevante, ordena pelas mais antigas, mostra produto, quantidade, EAN quando houver, observacao relacionada e data/hora de deteccao/criacao. Ele nao muda valores, fornecedor, ganhador, prioridade, cor, status ou posicao da linha.
- A antiga regra individual do dia seguinte as 16h ficou desativada. A tabela `cotacao_v2_encomenda_reminders` segue preservada para historico/data exata de deteccao e para compatibilidade de diagnostico, mas nao deve disparar avisos separados por linha.
...
- Desde 2026-08-28, duas ou mais celulas de `PRODUTO` com nome igual ou quase igual recebem fundo verde-neon claro somente na tela. A comparacao ignora caixa, acentos e pontuacao; tambem aceita um complemento depois do mesmo nome ou um unico erro de digitacao quando os dois nomes normalizados possuem uma unica palavra com pelo menos cinco caracteres. Desde 2026-08-31, a dosagem e analisada separadamente da identidade: `dipirona 500 mg`, `dipirona 500mg`, `dipirona 500` e `dipirona` continuam proximas, e `amoxicilina 0,5 g` equivale a `amoxicilina 500 mg`; quando os dois nomes informam dosagens diferentes, como `sinvastatina 20`/`40`, `desvenlafaxina 50`/`100` ou `Prolopa BD 100/25`/`200/50`, eles nao sao marcados juntos. Numeros de embalagem ligados a `cp`, `comprimidos`, `caixa`, `frasco` e apresentacoes equivalentes sao ignorados como quantidade, e compartilhar somente o numero nunca aproxima identidades diferentes, como `Pregabalina 75` e `Puran 75`.
...
- Quando `PRODUTO` passa de texto nao vazio para vazio, o backend apaga na mesma transacao somente o estilo manual `cell:<rowId>:produto` e devolve as chaves realmente removidas em `clearedStyleKeys`. Texto composto apenas por espacos conta como vazio. Reescrever o produto com outro texto nao remove cor; preencher uma celula antes vazia tambem nao dispara limpeza.
- A limpeza automatica nao apaga estilo manual de linha/coluna, regra condicional, cor de outra celula nem a marcacao visual de similaridade. Resposta HTTP, evento incremental e Socket.IO carregam a mesma lista `clearedStyleKeys`; o historico local guarda o estilo removido para `Ctrl+Z` restaurar nome e cor e para `Ctrl+Y` apagar ambos novamente.
...
- Desde 2026-06-03, `PATCH /cotacao/api/cells` e `/cells/batch` gravam valor e evento/historico na mesma transacao. Se o evento falhar, a alteracao da celula tambem deve voltar; sincronizacao de lembrete de encomenda nao deve transformar uma celula ja persistida em erro para a tela.
- Desde 2026-06-04, o frontend mostra alerta de sincronizacao quando o backend sinaliza `overwroteRemote=true`, mantendo o mesmo contrato de ultimo salvamento vencendo e guardando o valor anterior no historico.
- Desde 2026-06-04, adicionar linhas no fim da cotacao e serializado por transacao travando a propria cotacao antes de calcular `MAX(position)`, evitando posicoes duplicadas quando dois usuarios ou integracoes adicionam linhas ao mesmo tempo.
...
- A concorrencia atual usa contrato de ultimo salvamento vencendo; `expectedValue` fica no payload para auditoria/flag historica e aviso visual, nao para rejeitar o save. Se a equipe quiser bloqueio forte por versao, isso precisa de mudanca separada, teste com duas abas e plano de UX para conflito.
