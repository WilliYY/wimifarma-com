...
Desde 2026-08-16, Miauby Interno e Miauby WhatsApp reutilizam `miauw_skill_cotacao_encomenda_command_from_message()` para interpretar Encomenda sem depender da ordem ou da caixa. A intencao aceita `encomenda`, `encomendar`, `encomendou`, `cliente pediu`, `pediram`, `reserva para` e `separa para`. Produto, dosagem, marca e apresentacao permanecem em `produto`; nome composto, telefone, quantidade, endereco, bairro, referencia, entrega/retirada, data, horario, prioridade e observacao livre sao extraidos separadamente. Cliente e telefone sao opcionais, mas produto e obrigatorio. Frase realmente ambigua, como `Miauby encomenda Maria`, pede somente o produto e nao grava.

O `POST /cotacao/api/internal/encomendas` exige token, ator ativo com acesso ao card Cotacao e `request_id`. A linha, o evento e a auditoria em `cotacao_v2_miauby_encomenda_commands` sao gravados na mesma transacao; `(source, request_id)` impede duplicacao por retry ou confirmacao repetida. `quantidade` vai para a coluna propria; os demais metadados presentes formam a categoria operacional `Encomenda | Cliente: ... | Entrega: ...`, sem inventar campos ausentes. O cliente informado nunca e substituido pelo usuario autenticado: o primeiro pertence a Encomenda, enquanto o segundo fica como ator da auditoria. Ambos os canais continuam exigindo confirmacao humana antes da escrita.

Exemplos equivalentes: `Miauby encomenda losartana 50 Maria 44 4984894`, `Miauby Maria encomenda losartana 50 44 4984894` e `Miauby entrega amoxicilina 500mg para Maria, 2 caixas, amanha as 15h, Rua Minas 2263, urgente`. `30cp` permanece apresentacao do produto, enquanto `2 caixas` preenche a quantidade. `Miauby encomenda Bepantol` tambem e valido e salva somente `Encomenda` na categoria. Consultas como `lista encomendas`, `o que tem de encomenda` e `pedidos encomenda` continuam somente leitura.
