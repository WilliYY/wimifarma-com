...
- Em `/cashback/dashboard.php#busca`, o Balcao lista os ultimos clientes ativos alterados por `updated_at`/`created_at`, mostra 5 por padrao e revela mais em blocos de 5 pelo botao `Mostrar mais`, sem alterar saldo, compra ou resgate. A busca e o autocomplete normalizam telefone por digitos e aceitam ID com pontuacao como `#139`, sem usar telefone vazio quando o texto pesquisado nao tem numeros. Desde 2026-06-07, os cards dessa lista separam ID, telefone, atendente, atualizado, saldo e acoes em uma grade mais aberta, com saldo e botoes centralizados, para evitar texto espremido em nomes e informacoes. Desde 2026-06-16, os cards, o resumo lateral, o cliente selecionado, a busca viva e o seletor de cliente tambem mostram o saldo ativo agrupado por `expires_at`, lendo apenas creditos ativos, nao cancelados, com saldo restante e ainda nao vencidos.
...
- Ainda em 2026-06-23, o layout desktop usa tres colunas principais para cadastro, aguardando chegada e confirmados; o Historico ocupa uma faixa inferior em grade responsiva, e um card aberto do Historico usa a largura completa da faixa. A mudanca e somente visual, amplia tipografia/chips/acoes, separa o contador de Confirmados da ordem por vencimento e preserva busca, mostrar mais, recolhimento, edicao, POSTs, CSRF, pagamentos e auditoria.
...
- `/login-senha/` e `/login-senha/index.php`: cofre com cadastro e lista compacta tipo planilha; clicar na linha abre edicao de nome/login/nova senha, mostrar/ocultar, copiar e arquivar acesso; arquivar envia para `Historico de senhas`, recolhido por padrao, onde acessos arquivados podem ser excluidos individualmente ou limpos em lote sem revelar/copiar senha arquivada; a lista permite reorganizar linhas por arrastar e salva apenas `sort_order`; a auditoria de eventos recentes fica recolhida por padrao e abre no proprio card. Desde 2026-06-08, a tela tem refinamento visual compacto com formulario destacado, linhas-card densas, editor expandido mais claro, foco/acessibilidade visual e responsividade ajustada, sem mudar CSRF, POSTs, endpoints, reorder, criptografia ou auditoria.
...
- Desde 2026-06-24, o filtro por cor e calculado por celula na coluna filtrada e usa a cor final renderizada, com normalizacao entre `hex`/`rgb`, precedencia de regra condicional e estilo manual, cor-base da coluna e destaque de menor preco. `PRODUTO`, `CATEGORIA` e distribuidoras aceitam uma ou mais cores; quando o menu abre com todas marcadas, o primeiro clique direto em uma cor seleciona somente ela e cliques seguintes adicionam/removem cores. Aplicar ou limpar o filtro apenas redesenha a grade local e nao executa POST/PATCH, nao grava no Postgres e nao altera o filtro de outra sessao.
