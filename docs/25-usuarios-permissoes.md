...
- A tela do painel mostra o card fixo `Perfis do sistema`, apenas explicativo, para lembrar a diferenca entre Colaborador, Gerente, Admin e Farmacia. Esse card nao cria permissao nova: Colaborador continua dependendo dos modulos marcados, Gerente so tem efeito nos modulos que ja validam `gerente`, Admin segue a regra administrativa atual, e Farmacia continua sendo canal institucional.
