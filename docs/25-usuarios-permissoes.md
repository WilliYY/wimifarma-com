...
Criar uma base central para logins individuais, controle de acesso por modulo, vinculo com XP, vinculo com WhatsApp do Miauby e historico de mudancas sem separar a experiencia de cada modulo por usuario. As features continuam unicas por modulo; o painel decide quem pode entrar em cada area.
...
- `login_senha_entries` e `login_senha_audit_events`: cofre Login / Senha em banco dedicado, liberado pelo card/permissao `login_senha`.
...
- O campo `Perfil` no painel Usuarios mostra rotulos amigaveis (`Colaborador`, `Gerente`, `Admin`, `Farmacia`), mantendo valores internos controlados em `core_users.role`: `user`, `gerente`, `admin` e `farmacia`. Esse campo nao e apenas cosmetico: altera permissao base em modulos que respeitam role. O usuario mestre `adm` continua com perfil bloqueado para nao perder acesso administrativo.
- Perfil e cards/modulos sao controles separados. O perfil fica em `core_users.role` e define a base do usuario; os cards ficam por usuario em `core_user_module_permissions`. Nao existe configuracao global de cards por perfil, e trocar o perfil de Thiago, Sueli ou qualquer outro usuario nao altera os cards de outros logins do mesmo perfil.
- Regras gerais por perfil so devem ser criadas quando forem pedidas explicitamente. Exemplo: uma regra como "Colaborador nao ve Configuracao no XP" so vira regra geral de `user` depois de implementada no modulo XP e documentada; ate la, nao presumir hierarquia nem permissao nova.
- A tela do painel mostra o card fixo `Perfis do sistema`, apenas explicativo, para lembrar a diferenca entre Colaborador, Gerente, Admin e Farmacia e reforcar que cards sao individuais por usuario. Esse card nao cria permissao nova e deve mostrar apenas diferencas reais implementadas. Se um perfil nao tiver regra propria em determinado ponto, registrar que ele e organizacional/visual naquele ponto.
- O card controla a visibilidade na Home, mas o backend deve validar a permissao real por modulo. Admin/`adm` podem ter regras administrativas explicitas, mas isso nao vira heranca global de cards para todos os usuarios do mesmo perfil.
- O perfil `farmacia` representa um numero institucional, nao uma pessoa operacional. Ele pode ter WhatsApp e cards liberados, mas o painel esconde/desabilita XP e ferias pessoais para esse perfil; na Home, ele nao aparece em `Trocar usuario` e nao autentica como operador logavel. Quando o WhatsApp oficial enviar comando que precisa de responsavel humano, o Miauby deve perguntar quem fez a acao ou usar o nome valido informado no texto; se o nome nao existir, lista usuarios ativos para escolha por numero, ordinal ou nome. Para comprovante Pix CNPJ por foto/PDF lido com sucesso, o perfil `farmacia` tambem escolhe o responsavel humano antes de gravar; se a leitura falhar, recebe apenas o atalho manual.
- A Home principal (`site/home.php`) autentica em `core_users` ativo, usando `username_normalized` em minusculo e `password_hash` bcrypt. Assim `thiago`, `Thiago` e `THIAGO` entram no mesmo login canonico; desde 2026-09-01, `WIMIFARMA_HOME_LOGIN_USER`/`WIMIFARMA_HOME_LOGIN_PASSWORD` e `adm`/`adm` nao autenticam.
...
- O painel Usuarios nao cria tarefa privada. Tarefas privadas devem ser criadas e gerenciadas no modulo `/tarefa/`, que grava `tarefa_tasks.assigned_core_user_id`, revalida o usuario de destino e mantem o fluxo de editar/concluir/cancelar no proprio modulo Tarefa.
- Tarefas privadas nao sao espelhadas em `wf_tarefas`, para nao vazar tarefa individual no legado MySQL.
- O vinculo de WhatsApp pelo painel chama endpoints internos do bridge: `POST /miauw/whatsapp/internal/allowlist/link-user`, `GET /miauw/whatsapp/internal/allowlist/by-user`, `POST /miauw/whatsapp/internal/allowlist/update-user-display-name` e `POST /miauw/whatsapp/internal/allowlist/unlink-user`. O modulo Usuarios nao grava telefone cru; recebe apenas `contact_id`, mascara, nome, status e cards. Quando o operador nao informa `Nome no Miauby`, o painel usa `core_users.display_name` e, se ele estiver vazio, o login tecnico. Se o nome exibido ou login tecnico do usuario muda e ja existe WhatsApp vinculado, o painel sincroniza o nome exibido e o `linked_username_snapshot` no bridge para evitar nome antigo em painel/log; o responsavel operacional continua sendo o `linked_user_id` revalidado no core. Ao abrir o painel e apos vincular um numero, Usuarios reconcilia o snapshot seguro do core com o bridge por usuario: insere/atualiza contatos retornados, marca vinculos ausentes no bridge como `stale_bridge_missing` e audita apenas quando houve mudanca, sem telefone cru e sem envio de mensagem real.
...
- Em acoes financeiras pelo Miauby, sessao logada ou WhatsApp vinculado vencem o responsavel digitado para evitar que um usuario registre sangria/PIX como outra pessoa sem permissao. A excecao e o perfil `farmacia`: por ser WhatsApp institucional, ele nunca vira responsavel humano automatico. Se vier `miauby sangria 10 will`, `miauby pix cnpj 28,90 sueli` ou `miauby pedido anb 350 thiago`, o nome informado e resolvido para um usuario ativo; se vier sem nome ou com nome inexistente, o bridge guarda uma pendencia `selecionar_responsavel_whatsapp`, lista usuarios ativos e aceita numero, ordinal ou nome antes de gravar. Em foto/PDF de Pix CNPJ lido com sucesso, a Farmacia recebe resumo curto do valor/data/destino e escolhe responsavel; os detalhes completos ficam no payload/auditoria e na observacao interna do Financeiro, nao na resposta publica. Para acoes sensiveis como cancelar pedido ou concluir/cancelar tarefa, o fluxo do perfil `farmacia` escolhe o humano primeiro e depois pede a confirmacao normal da acao.
...
- Em comandos de Tarefas pelo Miauby WhatsApp, o responsavel vem do numero vinculado na allowlist. O contato precisa ter card `Tarefas`, estar autorizado e vinculado a `core_users`; o app Tarefa revalida `actor_user_id` e permissao antes de criar, listar, consultar, concluir ou cancelar. Usuario comum cria tarefa privada para si; ADM/admin pode criar para outro usuario ou tarefa geral. O perfil `farmacia` nao cria tarefa pessoal por padrao: `miauby tarefa para thiago conferir pedido` cria privada para Thiago, enquanto `miauby tarefa conferir pedido` abre `selecionar_destino_tarefa_whatsapp` com usuarios e `Geral/equipe`; a opcao geral e aceita apenas pelo endpoint interno com `source=miauby_whatsapp`, sem abrir permissao de admin na UI. Usuario comum nao pode cancelar tarefa criada/delegada por ADM; se a busca por texto encontrar varias tarefas, o Miauby lista opcoes por grupo/usuario, guarda uma pendencia curta vinculada ao numero e aceita numero, ordinal, grupo (`a geral`, `adm 1`, `minha 2`), usuario ou trecho do titulo antes de consultar ou pedir o SIM/NAO final. Pendencias expiradas exigem repetir o comando para evitar acao errada.
...
- Desde 2026-06-03, a Home principal usa `core_user_module_permissions` para decidir quais cards aparecem para o usuario logado. `adm`/role `admin` ve todos os cards como regra especial administrativa; usuarios sem linhas explicitas seguem em compatibilidade legado para modulos comuns; usuarios com permissoes salvas veem apenas os modulos marcados no painel Usuarios. Marcar/desmarcar um card salva somente aquele usuario. Desde 2026-06-05, `login_senha` e excecao sensivel: usuario comum precisa de permissao explicita e novos usuarios nascem com esse card desmarcado.
- Desde 2026-06-06, `calendario` entra como modulo comum no painel Usuarios; usuarios existentes com permissoes salvas recebem a linha inicial `calendario=true`, e depois o ADM pode desmarcar individualmente.
- No logout da Home, as sessoes dos modulos tambem sao expiradas no navegador (`WFXP`, `WFUSUARIOS`, `WFLOGINSENHA`, `WFCASHBACK`, `WFCOTACAOV2`, `WFCODIGOS`, `WFCALENDARIO`, `WFGESTAO`, `WFPEDIDOS`, `WFTAREFA`, `WFFINANCEIRO`, `WFWCASHBACK`) e chaves frontend da Home/Miauby sao limpas. Isso evita que, apos sair de Thiago e entrar como ADM, a Home reaproveite XP, fala, cofre ou contexto antigo.
...
- O backend agora reforca permissao por modulo nos acessos diretos dos modulos ja cortados para core auth: Cashback, Cotacao, Codigos, Calendario, XP, Tarefa, Gestao, Pedidos, Financeiro, Login / Senha e Miauby. A regra de compatibilidade continua para modulos comuns: ausencia total de linhas permite legado; quando ha linhas salvas, modulo ausente/desmarcado nao deve abrir por URL direta. `login_senha` nao usa essa permissao legado e exige linha explicita para usuario comum.
- Em Pedidos, o acesso por `WFHOME_SSO`, login manual e endpoints internos tokenizados deve seguir a permissao individual `core_user_module_permissions` do modulo `pedidos`, nao uma trava fixa por perfil. Assim um colaborador com o card `Pedidos` liberado entra normalmente, enquanto um usuario sem esse card continua bloqueado por URL direta.
...
- Desde 2026-06-03, o card expandido de usuario usa layout frontend em blocos responsivos: cabecalho com contagem separada, badges abaixo do nome, conta/senha/XP em grid proprio, cofre ADM e status compactos, modulos densos e integracoes ajustadas por largura. As integracoes `Ferias` e `WhatsApp do funcionario` usam areas responsivas; o bloco de tarefa privada foi removido para concentrar a criacao em `/tarefa/`. Para perfil `farmacia`, ficam apenas os controles institucionais de WhatsApp, sem XP/ferias pessoais. Essa regra nao altera CSRF, permissao, hash de senha, cofre ADM, WhatsApp ou endpoints.
- Desde 2026-06-15, o painel de Usuarios recebeu ajuste visual de frontend para reduzir compressao e sobreposicao: topo menor e solido, largura operacional maior, card `Novo usuario` mais contido, cards de usuario e resumo de acesso alinhados, chips de modulos mais estaveis e integracoes `Ferias`/`WhatsApp do funcionario` separadas no grid correto. A mudanca preserva os mesmos nomes de campos, CSRF, POSTs, permissoes, cofre ADM, XP, WhatsApp, ferias, auditoria e banco.
...
5. Criar tarefas individuais somente pelo modulo Tarefa, mantendo o painel Usuarios focado em contas, permissoes, XP, ferias e WhatsApp.
