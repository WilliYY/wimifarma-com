...
- O mini-card XP da Home deve sempre pertencer ao login atual da Home. A Home consulta `/usuarios/api/me/xp-card` e `/xp/api/me/xp-card` com `no-store` e descarta qualquer payload cujo `user.username` nao combine com o login atual.
