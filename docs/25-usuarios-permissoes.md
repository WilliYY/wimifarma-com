...
- Em acoes financeiras pelo Miauby, sessao logada ou WhatsApp vinculado vencem o responsavel digitado para evitar que um usuario registre sangria/PIX como outra pessoa sem permissao. A excecao e o perfil `farmacia`: por ser WhatsApp institucional, ele nunca vira responsavel humano automatico. Se vier `miauby sangria 10 will` ou `miauby pix cnpj 28,90 sueli`, o nome informado e resolvido para um usuario ativo; se vier sem nome, o bridge guarda uma pendencia `selecionar_responsavel_whatsapp`, lista usuarios ativos e aceita numero ou nome antes de gravar.
