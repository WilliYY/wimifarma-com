...
- Desde 2026-06-03, `text_command_contracts` tambem entra no pacote canonico para registrar que comandos textuais treinados no Miauby WhatsApp devem virar variacoes textuais do Miauby interno quando fizer sentido. O interno nao exige prefixo `miauby`, nao processa midia e usa apenas texto manual, por exemplo `sangria 10 troco`, `pix cnpj 28,90 compra fornecedor`, `pedido anb 350`, `pedidos`, `o que falta chegar`, `cancelar pedido anb`, `criar tarefa conferir caixa`, `minhas tarefas`, `concluir tarefa conferir caixa`, `cancelar tarefa conferir caixa`, `cotacao dipirona` e `status do calendario`. O pacote tambem expoe `identity_resolution`: `miauby_interno` resolve responsavel pela sessao logada, enquanto `miauby_whatsapp` resolve pelo numero vinculado/allowlist. Desde 2026-06-07, `resumo_calendario` aparece como leitura segura do Calendario: o pacote pode explicar contexto, cores, dias marcados e atualizacoes recentes, mas nao retorna texto completo nem permite escrita direta no Node.
- Para Pedidos, o contrato textual compartilhado usa o app Pedidos como dono da leitura/escrita por endpoints tokenizados: `GET /pedidos/api/internal/arrival-summary`, `GET /pedidos/api/internal/cancel-candidates` e `POST /pedidos/api/internal/cancel-order`. Listar pedidos consulta apenas `Aguardando chegada`. Cancelar pedido tambem fica restrito a `Aguardando chegada`, pede escolha quando houver varios candidatos, exige confirmacao humana final, avisa quando ha financeiro vinculado e preserva pagamentos/historico por cancelamento logico auditado.
