...
- camada deterministica compartilhada em `apps/miauw-agent/src/semantic-command.ts`, exposta por `POST /miauw/agent/interpret`, que classifica a frase inteira antes dos parsers do Interno e do WhatsApp. Ela reorganiza comandos reconhecidos em formato canonico, preserva os formatos antigos por fallback, interrompe a execucao quando duas acoes relevantes permanecem ambiguas e bloqueia negacoes explicitas. A comparacao normaliza Unicode, acentos e caixa, aceita typo conservador de uma letra somente com ativacao explicita e usa cache de frases para manter baixa latencia.
