...
- Em `apps/cashback`, salvar dinheiro em centavos inteiros, percentual em basis points, usar transacao para resgate, manter `legacy_mysql_id`, preservar rotas PHP antigas e nao deixar exportacao CSV sair em centavos crus. Cashback Manual deve substituir, nao somar, o cashback automatico novo; cancelamento de cashback gerado deve ser logico e limitado a credito intacto; XP por resgate deve ser idempotente por `source`/`source_entity_id`.
