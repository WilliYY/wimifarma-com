...
- Na Cotacao V2, importacao Google Sheets e restore de backup sao bloqueados para usuarios sem permissao forte (`adm`, `admin` ou `gerente`), mesmo com sessao e CSRF validos.
...
- `/cashback/` e atendido pelo servico Node `apps/cashback`, usa sessao propria `WFCASHBACK`, autentica somente por `core_users`, valida CSRF antes de criar cliente, compra, resgate, configuracao, atendente ou status de WhatsApp, e grava auditoria em Postgres. Atendente novo exige conta core ativa com acesso ao modulo; o backend ignora nome vindo do formulario e sincroniza `cashback_attendants.name` a partir de `core_users.display_name`/`username`.
- O Cashback rapido usa dinheiro em centavos recalculado no servidor, idempotencia por token de emissao, codigo textual atual de cinco digitos e `FOR UPDATE` no resgate. Codigos historicos de quatro digitos continuam aceitos enquanto validos. A reutilizacao de uma combinacao de cinco digitos e permitida somente quando todas as emissoes anteriores daquele numero possuem `expires_at < CURRENT_DATE`; status `usado` ou `cancelado` nao antecipa a liberacao. A geracao usa advisory lock transacional global, consulta os codigos ainda reservados e so entao escolhe entre as 100.000 combinacoes, impedindo duas emissoes concorrentes iguais. Por decisao operacional, um papel ja vencido pode coincidir com uma emissao futura depois da reciclagem; o sistema mostra a validade da emissao atual ao validar o codigo, e o atendente deve comparar essa data com o comprovante apresentado. Consulta e tentativa de uso exigem sessao, aceitam somente correspondencia exata e usam `cashback_quick_voucher_rate_limits`: ate 30 tentativas por usuario em 10 minutos, com bloqueio de 15 minutos para consulta e 30 minutos para resgate. Recibos por ID so aparecem para os vouchers lembrados na sessao que os gerou. O atendente escolhido para o papel nao substitui o `issued_by` do usuario autenticado. Solicitacao de impressao usa CSRF e nao deve ser registrada como impressao concluida.
- Comprovantes de compra identificada usam `cashback_clients.id` como `Codigo do cliente` e so podem ser carregados ou enviados para impressao quando o `purchase_id` foi lembrado em `cashbackPurchaseReceiptIds` na sessao atual. O cadastro com compra inicial segue a mesma allowlist de sessao usada por `save_redeem`; o parametro `receipt_origin` muda apenas onde o recibo aparece e nao autoriza outro ID. O recibo anonimo de `Cashback rapido` nao recebe `client_code`.
- A Wimi Impressora guarda tickets e tokens de dispositivo somente como SHA-256 no servidor; o token bruto do agente fica cifrado com DPAPI LocalMachine em `ProgramData`. Ticket de download vale 30 minutos e uma utilizacao, e nunca deve aparecer em log. Jobs carregam somente dados necessarios ao recibo e um conjunto fechado de tipos, sem comando arbitrario. Claim usa lock transacional; interrupcao gera `uncertain`, sem repeticao automatica. Atualizacao exige agente autenticado e SHA-256 do artefato, aceita somente o caminho oficial em `Program Files`, usa troca atomica e restaura o backup se o servico nao voltar. Telas autenticadas e APIs sensiveis do Cashback enviam `Cache-Control: private, no-store`, sem cache de dados operacionais. Enquanto nao houver assinatura Authenticode, manter o EXE restrito ao card `adm` e ao uso interno da farmacia.
...
- `/usuarios/` usa sessao propria `WFUSUARIOS` no servico Node, autentica contra `core_users`, restringe o painel a `adm` ou role `admin`, valida CSRF nas acoes e registra auditoria central sem salvar senha ou hash em logs. Na criacao pelo painel, nomes com espaco/acento sao normalizados no backend para login seguro em `core_users.username`/`username_normalized`, sem relaxar autenticacao ou permissao.
...
- O modulo Login / Senha (`/login-senha/`) guarda acessos internos em Postgres dedicado com senha cifrada por AES-256-GCM. A permissao `login_senha` e default fechada para usuarios comuns, mesmo quando nao existem linhas legadas em `core_user_module_permissions`. Revelar/copiar senha ou login exige sessao, permissao e CSRF, e registra auditoria sem salvar a senha. Esse modulo nao deve ser exposto ao Miauby WhatsApp, Gemini, contexto generativo ou endpoint interno de leitura sem decisao nova e guardrails proprios.
...
- Nao versionar `LOGIN_SENHA_POSTGRES_PASSWORD`, `LOGIN_SENHA_SESSION_SECRET`, `LOGIN_SENHA_VAULT_KEY` nem volumes de `login-senha-data/`. Se a chave do cofre vazar, trocar em janela controlada e planejar recriptografia; se for perdida, senhas antigas cifradas nao podem ser recuperadas pela aplicacao.
...
- Nao versionar `MIAUW_OPENAI_API_KEY`, `MIAUW_WHATSAPP_OPENAI_API_KEY` nem modelos/base URLs privados; as rotas de audio e o fallback de OCR Pix usam a chave somente no backend/worker, sem expor segredo ao navegador.
...
- Automacoes n8n do WhatsApp, como chegada de pedidos e fechamento de caixa, devem chamar apenas endpoints internos tokenizados do Miauby. O n8n nao deve acessar banco de negocio nem escolher telefone manualmente; o bridge calcula destinatarios pelos cards liberados (`Pedidos`, `Financeiro`) e ignora LIDs protegidos. No lembrete de encomenda da Cotacao, qualquer `recipients`/`phones` recebido por payload tambem precisa bater com contato `allowed` e card `Cotacao`, servindo apenas como filtro seguro.
...
- Leitura de comprovante Pix por midia deve conferir allowlist e card `Financeiro` antes de baixar/OCR via Gemini, respeitar limite diario e pausar Gemini quando detectar spending cap, cota, billing ou 429. Nesses casos deve orientar o lancamento manual, sem fallback automatico para outro provedor pago. O fallback OpenAI fica restrito a erro transitorio de OCR, como timeout, 5xx ou resposta vazia, quando chave OpenAI existir. A midia deve ficar somente em memoria, ter tamanho limitado e ser descartada apos extracao. O backend pode usar legenda/nome do arquivo e fallback sobre OCR sanitizado, mas nao deve persistir `raw_text` completo, URL temporaria ou payload bruto. Quando a imagem nao parecer comprovante Pix, deve responder `Isso ai é um comprovante pix?` sem tentar lancamento.
...
- Como os logins visuais dos modulos agora voltam para `/`, a producao precisa manter `WIMIFARMA_HOME_SSO_SECRET` ou `WP_AUTH_KEY` forte; sem SSO valido, o login da home nao consegue criar sessoes nos modulos.
