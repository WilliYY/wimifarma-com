...
- O contador da home usa cookie anonimo `WFHOME_VISITOR` com `HttpOnly`/`SameSite=Lax`, nao grava IP, user-agent, telefone, login nem payload bruto, e salva somente totais agregados em `site/wp-content/uploads/wimifarma-runtime/home-counter.json`; o diretorio recebe `.htaccess` gerado para bloquear listagem/acesso direto.
