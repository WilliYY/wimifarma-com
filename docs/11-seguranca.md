...
- A Wimi Impressora Web usa token aleatorio de 32 bytes no cookie `WFWIMIPRINT`; o navegador recebe `HttpOnly`, `SameSite=Strict`, `Secure` em producao e escopo `/cashback`, enquanto o Postgres recebe somente SHA-256. Ativacao/revogacao exige login exato `adm` e CSRF da sessao; heartbeat, claim e conclusao usam HMAC CSRF vinculado ao token, conferencia de mesma origem, CSP com nonce e `Cache-Control: private, no-store`. Jobs carregam somente dados necessarios ao recibo e tipos fechados, sem comando arbitrario. Claim usa `FOR UPDATE SKIP LOCKED`, Web Locks impedem duas abas do mesmo dispositivo e interrupcao gera `uncertain` sem repeticao automatica. O atalho `.cmd` nao contem segredo nem altera Windows; abre somente o Chrome com `--kiosk-printing`. Downloads e atualizacoes EXE foram removidos, eliminando o risco operacional do binario sem Authenticode.
