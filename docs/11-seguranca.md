...
- O Cashback rapido usa dinheiro em centavos recalculado no servidor, idempotencia por token de emissao, codigo textual de quatro digitos com unicidade permanente e `FOR UPDATE` no resgate. Consulta e tentativa de uso exigem sessao, aceitam somente correspondencia exata e usam `cashback_quick_voucher_rate_limits`: ate 30 tentativas por usuario em 10 minutos, com bloqueio de 15 minutos para consulta e 30 minutos para resgate. Recibos por ID so aparecem para os vouchers lembrados na sessao que os gerou. O atendente escolhido para o papel nao substitui o `issued_by` do usuario autenticado. Solicitacao de impressao usa CSRF e nao deve ser registrada como impressao concluida. O limite de quatro digitos e 10.000 emissoes totais; aumentar o tamanho antes de esgotar, sem liberar reutilizacao de codigo antigo.
