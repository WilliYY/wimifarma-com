...
- Cashback (`/cashback/`) usa o servico Node `apps/cashback`, sessao propria `WFCASHBACK` no Postgres `wimifarma_cashback` e autentica somente contra `core_users`; desde 2026-05-30 nao ha fallback MySQL nem `CASHBACK_AUTH_PROVIDER` no app. Criar cliente, registrar compra, usar cashback, emitir voucher rapido, registrar solicitacao de impressao, atualizar configuracoes, editar atendentes e marcar WhatsApp usam CSRF. A consulta exata de codigo rapido exige sessao, limita tentativas por minuto e nao lista codigos. Desde 2026-07-21, criar/editar atendente exige `core_user_id` de conta humana ativa com acesso ao Cashback, validado novamente no backend; o perfil institucional `farmacia`, nomes livres e atendentes legados sem conta nao entram mais na operacao. Na emissao rapida, o atendente selecionado e validado novamente, e o ator real permanece no usuario da sessao. Areas sensiveis como relatorio/exportacao/diagnostico continuam com senha operacional.
- `Wimi Impressora` e uma excecao mais restrita que role: card, painel, ativacao da estacao web, download do atalho, teste, revogacao e reimpressao exigem username normalizado exatamente `adm`. O envio de um comprovante para a fila e permitido a usuarios autenticados do Cashback somente quando o ID foi lembrado na sessao que concluiu a operacao. A estacao nao usa token interno generico nem depende da sessao ADM depois de ativada: autentica por cookie de dispositivo `HttpOnly`/`SameSite=Strict`/`Secure`, cujo valor bruto existe somente no navegador e cujo SHA-256 fica no Postgres. Heartbeat, claim e conclusao tambem exigem HMAC CSRF e mesma origem. APIs Bearer do agente antigo ficam apenas para compatibilidade, sem download ou atualizacao EXE.
