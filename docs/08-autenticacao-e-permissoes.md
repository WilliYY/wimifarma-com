...
- O mini-card XP da Home consulta primeiro o endpoint do Usuarios e depois o do XP, ambos em modo `no-store`, e so renderiza se o payload pertence ao login atual da Home. Se algum endpoint responder uma sessao antiga de outro usuario, a Home ignora a resposta em vez de mostrar XP incorreto.
