...
- `Wimi Impressora` e uma excecao mais restrita que role: card, painel, download, teste, revogacao e reimpressao exigem username normalizado exatamente `adm`. O envio de um comprovante para a fila e permitido a usuarios autenticados do Cashback somente quando o ID foi lembrado na sessao que concluiu a operacao. APIs do agente nao aceitam sessao nem token interno generico: pareamento usa ticket curto de uso unico e as demais chamadas usam Bearer exclusivo do dispositivo.
