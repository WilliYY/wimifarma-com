...
- O painel Usuarios nao recupera senha antiga importada por hash. Quando o administrador cria ou troca uma senha, o login continua validando `core_users.password_hash`, mas uma copia cifrada para consulta interna do ADM e gravada em `core_user_admin_passwords`. Se a senha ainda nao foi redefinida no painel, o ADM precisa definir uma nova para passar a saber qual senha esta valida.
