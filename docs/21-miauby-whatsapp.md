...
Desde 2026-06-03, contatos vinculados a `core_users.role='farmacia'` representam o WhatsApp oficial/institucional da loja. Esse perfil continua autorizado por allowlist/cards, mas nao e tratado como responsavel humano de sangria, Pix CNPJ, pedido ou outro lancamento operacional. Quando o comando ja informa um nome, o bridge resolve esse nome contra usuarios ativos que nao sejam `farmacia`; quando nao informa, grava uma pendencia `tool=selecionar_responsavel_whatsapp`, lista usuarios ativos e aceita resposta por numero ou nome antes de executar. Essa escolha de responsavel e separada de confirmacoes `Sim/Nao` de acao forte.
