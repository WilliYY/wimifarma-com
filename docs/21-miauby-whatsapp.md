...
Desde 2026-06-08, o OCR desse fluxo tem guard de custo: o modelo padrao e `gemini-2.5-flash-lite`, `MIAUW_WHATSAPP_PIX_RECEIPT_OCR_DAILY_LIMIT` limita tentativas em 24h e `MIAUW_WHATSAPP_GEMINI_SPEND_GUARD_PAUSE_MINUTES` pausa Gemini quando detectar spending cap, cota, billing ou 429. Nessas pausas o WhatsApp responde com o formato manual `miauby pix cnpj valor responsavel`, sem fallback automatico para outro provedor pago. Fallback OpenAI fica restrito a falha transitoria de OCR, como timeout, 5xx ou resposta vazia, quando chave OpenAI existir. Desde 2026-06-25, se a primeira leitura Gemini vier incompleta/insegura ou com JSON invalido/vazio, o bridge tenta uma segunda leitura conservadora antes de cair no manual ou no fallback OpenAI. O painel mostra modelo, tentativas 24h, limite diario e status do guard.
