...
Desde 2026-06-05, o bridge tambem responde consultas de encomenda da Cotacao sem passar por Gemini/core. Com card `Cotacao` liberado, `miauby encomendas`, `miauby encomenda`, `miauby lista encomendas`, `miauby o que tem de encomenda`, `miauby pedidos encomenda`, `miauby encomendas antigas` e `miauby encomendas recentes` chamam `GET /cotacao/api/internal/encomendas` com token interno, limitam a 10 itens e retornam blocos curtos com EAN quando houver, produto, quantidade, observacao relacionada e data/hora de criacao/deteccao. O comando e somente leitura: nao altera linhas, nao marca entrega, nao cancela lembrete e nao cria cadastro paralelo. Sem card `Cotacao`, o bridge bloqueia antes da consulta. Desde 2026-06-12, o mesmo endpoint alimenta a automacao `cotacao_encomendas_resumo_17h`: todos os dias as 17h em `America/Sao_Paulo`, o bridge monta um unico resumo `Encomendas da Cotacao - 17h`, ordenado pelas mais antigas, com produto, quantidade, EAN quando houver, observacao antes/depois da palavra de encomenda (`encomenda`, `encomendar`, `encomendado`, `enc.`) e data/hora de deteccao/criacao. O envio vai somente para contatos autorizados com card `Cotacao`, respeita ferias, usa dedupe diario persistido em `miauw_whatsapp_automation_runs` e nao envia nada quando nao houver encomenda ativa. A rotina individual antiga `cotacao_encomenda_16h` fica legado desativado e, se chamada, apenas registra `skipped`.
