...
Desde 2026-08-16, mensagens iniciadas por `falteiro`, `falta`, `faltou` ou `acabou` sao candidatas ao registro de falta na Cotacao, sem diferenciar maiusculas/minusculas. No WhatsApp, a mensagem continua sujeita a prefixo/allowlist conforme a configuracao e exige o card `Cotacao`; no interno, exige que o usuario logado tenha acesso a Cotacao. Os clientes fazem apenas a triagem inicial e enviam a frase completa ao endpoint tokenizado `POST /cotacao/api/internal/falteiro/commands`; produto, dosagem e prioridade sao interpretados uma unica vez pela Cotacao, evitando regras diferentes entre canais.

O WhatsApp usa `whatsapp:<trace_id>` como `request_id`, registra resultado/linha/replay no resumo sanitizado do evento e nao duplica a escrita em retry. O interno usa uma chave unica por mensagem processada e registra `cotacao_falteiro` nos traces. O endpoint ocupa atomicamente a primeira linha cujo produto esteja vazio, mantem `40mg` junto ao produto, grava `Urgente` apenas em categoria e devolve a mesma confirmacao curta aos dois canais. Perguntas como `qual produto faltou ontem?`, comandos de pedido como `falta chegar pedido` e frases sem gatilho no inicio seguem para as rotas normais e nao escrevem na planilha.
