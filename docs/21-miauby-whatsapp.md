...
Os clientes fazem apenas uma triagem conservadora e enviam a frase completa ao endpoint tokenizado `POST /cotacao/api/internal/falteiro/commands`. Produto, dosagem, apresentacao e categoria sao interpretados uma unica vez pela Cotacao. A categoria so e preenchida quando corresponde ao catalogo atual formado pelas linhas/regras da coluna `categoria`; o rotulo cadastrado e preservado e nenhuma categoria nova e inventada.

O WhatsApp usa `whatsapp:<trace_id>` como `request_id`, registra resultado/linha/replay no resumo sanitizado do evento e nao duplica a escrita em retry. O interno usa uma chave unica por mensagem processada e registra `cotacao_falteiro` nos traces. O endpoint ocupa atomicamente a primeira linha cujo produto esteja vazio, preserva as outras celulas, atualiza o tempo real, audita o comando e responde com o mesmo produto/categoria aos dois canais. Perguntas como `qual produto faltou ontem?`, comandos de pedido como `falta chegar pedido` e indisponibilidades obvias sem relacao com estoque seguem para as rotas normais e nao escrevem na planilha.
