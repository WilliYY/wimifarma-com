...
Os clientes fazem apenas uma triagem conservadora e enviam a frase completa ao endpoint tokenizado `POST /cotacao/api/internal/falteiro/commands`. Na entrada do WhatsApp, Meta e Evolution usam `safeInboundText()` desde a extracao do payload ate a fila e tambem antes da memoria conversacional estruturada: nenhum normalizador intermediario pode converter quebra de linha em espaco. Assim, cada linha digitada continua sendo um segmento independente ate o parser autoritativo. A Cotacao analisa primeiro a intencao global e depois segmenta de um a 50 itens por virgula, ponto e virgula, quebra de linha, marcadores, numeracao e conectores naturais. O conector `e` so separa produtos quando ha evidencia suficiente, preservando kits, nomes compostos e observacoes. Cada item mantem `rawText` e indicadores de confianca; produto, dosagem, apresentacao, marca, contexto e categoria sao resolvidos separadamente, e modificadores globais so sao herdados com referencia coletiva explicita. A categoria oficial so e preenchida quando corresponde ao catalogo atual formado pelas linhas/regras da coluna `categoria`; o rotulo cadastrado e preservado e nenhuma categoria nova e inventada. Produtos conhecidos servem apenas como ancora segura de segmentacao/correcao, sem substituir texto ambiguo.

O WhatsApp usa `whatsapp:<trace_id>` como `request_id`, registra quantidade, linhas e replay no resumo sanitizado do evento e nao duplica a escrita em retry. O interno usa uma chave unica por mensagem processada e registra `cotacao_falteiro` nos traces. Para N itens, limitado a 50 por mensagem, o endpoint trava e reserva as N primeiras linhas integralmente vazias: nenhum campo em `values` pode conter dado, inclusive EAN, produto, quantidade, categoria ou fornecedor. Assim, uma linha parcialmente preenchida nunca e reutilizada. O lote e gravado numa unica transacao, publica um unico evento de tempo real e responde com `items`, `detected_count`, `created_count` e uma confirmacao consolidada. Falta de linhas, lote acima do limite ou erro em qualquer item nao grava parte do lote. A auditoria usa `(source, request_id, item_index)` para preservar um registro por segmento e impedir duplicacao em retry. Perguntas como `qual produto faltou ontem?`, comandos de pedido como `falta chegar pedido` e indisponibilidades obvias sem relacao com estoque seguem para as rotas normais e nao escrevem na planilha.
...
Desde 2026-05-27, o bridge tambem pode ler foto, print, imagem encaminhada ou PDF/documento de comprovante Pix de remetente autorizado, validar o destino por CNPJ/chave Pix e preparar um lancamento `Pix CNPJ` no Financeiro com confirmacao `Sim`/`Nao`. A midia e baixada apenas em memoria no worker, enviada ao Gemini para extracao estruturada e descartada; o banco guarda somente metadados sanitizados e o comando/pendencia gerados. Desde 2026-06-01, a validacao ficou mais conservadora: CNPJ de destino diferente do configurado bloqueia o lancamento, CNPJ achado apenas no texto bruto so vale quando aparece em contexto de destino/recebedor, a extracao pode carregar ID Pix/E2E e confianca por campo, e falhas/recusas registram diagnostico sanitizado sem `raw_text` completo. Ainda em 2026-06-01, a etapa de velocidade corta chamadas desnecessarias: antes do OCR, descarta anexos com legenda/nome/extensao claramente nao Pix; quando o transporte fornece hash do arquivo, usa hash salgado para reconhecer repeticao; e depois da extracao usa ID Pix/E2E para evitar nova confirmacao de comprovante ja tratado nos ultimos 90 dias. Quando a imagem nao parece comprovante Pix, o Miauby responde `Isso ai é um comprovante pix?` sem criar pendencia. Desde 2026-06-05, nome correlato da empresa e usado apenas como pista de OCR: se o CNPJ/chave configurado nao aparecer no destino, o Miauby responde `Nao achei nosso CNPJ nesse comprovante. Nao gravei nada.`. Se a leitura falhar ou ficar incompleta, o WhatsApp responde que nao conseguiu ler com seguranca e orienta o formato manual `miauby pix cnpj valor responsavel`, com exemplo curto.

Desde 2026-06-05, campos opcionais de rastreio do comprovante Pix, como `transaction_id`, `end_to_end_id` e chave Pix do destino, nao bloqueiam a leitura quando valor, pagador e destino Wimifarma ja foram confirmados; se esses IDs existirem, continuam sendo salvos no diagnostico sanitizado e usados para dedupe. O resumo diario Pix/OCR tambem nao deve tratar ausencia apenas desses campos opcionais como problema acionavel.

Desde 2026-06-02, o comprovante Pix por midia separa detalhe interno de resposta publica: OCR, destino, pagador completo, instituicao, ID Pix, confiancas e observacao completa continuam no payload sanitizado, na pendencia e no Financeiro, mas o WhatsApp mostra apenas resumo operacional curto. Desde 2026-06-05, esse resumo tambem inclui o pagador extraido e o tipo do destino quando disponiveis, como `PIX CNPJ lancado: R$ 28,90 - Responsavel: Sueli - Feito por: MARCIO - Tipo: foi no CNPJ - 01/06 as 12:16.`. Se faltar responsavel, a resposta deve ser `Comprovante lido. Valor: R$ 28,90 - Feito por: MARCIO - Tipo: foi no CNPJ - Responsavel nao identificado. Confira no Financeiro.`. Desde 2026-06-03, quando a midia vem do perfil institucional `Farmacia` e a leitura passa, o bridge nao abre confirmacao `Sim/Nao` nem usa o pagador como humano: responde resumo com valor, data, destino, `Feito por` e `Tipo`, pergunta `Quem foi o responsavel?`, lista usuarios ativos e so grava apos a escolha.
...
Desde 2026-06-08, o painel `/miauw/whatsapp/` e o alias `/miauby/whatsapp/` ficaram mais compactos: os resumos de secoes mostram contadores, `Sincronia recente` tambem fica recolhida por padrao, e listas que podem gerar muito dado usam rolagem interna limitada apenas quando abertas. `Erros abertos` continua sendo a excecao operacional e abre sozinho quando houver falha acionavel. A mudanca nao altera consultas Postgres, webhook, worker, n8n, outbox, envio real, allowlist, tokens, CSRF ou regras de negocio.
...
Desde 2026-06-03, contatos vinculados a `core_users.role='farmacia'` representam o WhatsApp oficial/institucional da loja. Esse perfil continua autorizado por allowlist/cards, mas nao e tratado como responsavel humano de sangria, Pix CNPJ, pedido ou outro lancamento operacional. Quando o comando ja informa um nome valido, o bridge resolve esse nome contra usuarios ativos que nao sejam `farmacia`; quando nao informa ou informa nome inexistente, grava uma pendencia `tool=selecionar_responsavel_whatsapp`, lista usuarios ativos e aceita resposta por numero, ordinal ou nome antes de executar. Respostas curtas de selecao como `1`, `2`, `Thiago` ou `cancelar` dispensam o prefixo `miauby` somente quando ja existe uma pendencia ativa ou recem-expirada daquele mesmo telefone; fora desse contexto, a regra normal de prefixo/allowlist continua valendo. Desde 2026-06-08, se chegar um comando novo explicito com prefixo `miauby` enquanto uma escolha de responsavel estiver pendente, a pendencia antiga e expirada e o novo comando e interpretado do zero, evitando registrar com responsavel errado ou vazio depois de repeticoes. Para comprovante Pix CNPJ por foto/PDF lido com sucesso, o mesmo perfil recebe primeiro o resumo do valor/data/destino, pagador (`Feito por`) e tipo (`foi no CNPJ`) e escolhe o responsavel humano; se a midia nao for lida com confianca, recebe a orientacao manual guiada e nada e registrado. Essa escolha de responsavel e separada de confirmacoes `Sim/Nao` de acao forte: para cancelamento de pedido ou concluir/cancelar tarefa, primeiro escolhe o humano e depois entra na escolha/confirmacao da acao. Em Tarefas, `miauby tarefa para thiago conferir pedido` cria tarefa privada para Thiago; `miauby tarefa conferir pedido` pergunta `Essa tarefa e para quem?` e lista usuarios mais `Geral/equipe`, usando `tool=selecionar_destino_tarefa_whatsapp`. A opcao `Geral/equipe` cria tarefa geral somente pelo endpoint interno tokenizado com `source=miauby_whatsapp`, sem transformar o perfil `farmacia` em admin na tela do modulo.

Desde 2026-08-16, pedidos de relatorio no WhatsApp usam memoria conversacional explicita em `miauw_whatsapp_conversation_states`. Se faltar o tipo ou a data, o bridge pergunta somente o dado ausente, guarda os campos por hash do telefone por 15 minutos e aceita a resposta seguinte sem repetir `miauby`. Os tipos aceitos sao Financeiro, Cashback, Tarefas, Cotacao, Codigos, Calendario e Geral; datas aceitam `hoje`, `ontem`, `anteontem`, `DD/MM`, `DD/MM/AAAA`, `AAAA-MM-DD` e `dia N`, sempre em `America/Sao_Paulo`. Data impossivel nao consulta dados; `cancelar` encerra somente a conversa pendente daquele telefone. Quando os campos ficam completos, o evento recebe a solicitacao normalizada e auditavel antes de consultar o core. A ponte do agente preserva `data=YYYY-MM-DD` nos contratos `resumo_financeiro`, `resumo_cashback`, `resumo_tarefas` e `resumo_geral`. Relatorio de dia exato nunca deve cair silenciosamente no mes inteiro: Financeiro usa `/api/internal/day`, Cashback e Tarefas recebem intervalo de um dia, e o geral diario combina somente essas fontes temporais. O Geral diario so executa quando o contato possui os tres cards; caso contrario, informa quais acessos faltam. A regra normal de prefixo volta assim que a pendencia e consumida, cancelada ou expira.

Alertas `provider_paused` permanecem no historico, mas recebem `resolved_at` automaticamente depois do primeiro envio real bem-sucedido. Assim uma pausa ja recuperada nao mantem o diagnostico em falha e o incidente continua auditavel.
...
Desde 2026-06-03, o bridge cria Pedidos pelo WhatsApp sem passar pelo Gemini quando a mensagem autorizada vem no padrao `miauby pedido ...` ou em variacoes operacionais como `miauby distribuidora anb 350`. O parser aceita fornecedor antes/depois do valor, `R$`, `valor/deu/ficou/total/boleto`, abreviacoes comuns (`venc`, `pgto`, `prev`), previsao de chegada (`amanha`, `05/06`, `segunda`, `semana que vem`), status inicial (`ja chegou so pagar`, `pago falta chegar`, `pago e recebido`) e parcelas com vencimento (`2 parcelas 200 10/06 e 150 20/06`, `1x/2x`, `dia 10`). Em parcelamento com um unico valor, `350 em 2x` trata 350 como total dividido, enquanto `2 parcelas 175` ou `2 boletos 175` trata 175 como valor de cada parcela. O contato precisa estar na allowlist, ter card `Pedidos` e estar vinculado a um usuario do core; o app Pedidos revalida `actor_user_id` e respeita `core_user_module_permissions`. A mesma mensagem usa `idempotency_key=whatsapp:{trace_id}` em `POST /pedidos/api/internal/create-order`, evitando duplicar conta financeira/pagamento em retry de webhook ou worker. Se faltar fornecedor/valor, houver data passada, permissao faltando ou status contraditorio, o bridge responde curto e nao grava nada; quando a soma das parcelas divergir do total informado, ele cria uma pendencia `Sim/Nao` vinculada ao ultimo comando daquele numero e so grava se a pessoa confirmar. Desde 2026-06-08, o bridge tambem valida o responsavel imediatamente antes de chamar Pedidos e a resposta publica de sucesso mostra o nome resolvido do responsavel, inclusive quando ele vem do numero vinculado comum. Ainda em 2026-06-08, falha temporaria na chamada interna ao app Pedidos nao deixa o WhatsApp mudo: consulta/lista/cancelamento respondem que nada foi alterado, e criacao responde para conferir `/pedidos/` antes de reenviar, evitando duplicidade quando nao der para confirmar a gravacao. A resposta publica do WhatsApp continua curta e operacional, por exemplo `Pedido criado: ANB - R$ 350,00 - Willian.` ou `Pedido parcelado criado: ANB - 2 parcelas - R$ 350,00 - Thiago.`, sem despejar detalhes tecnicos.

Ainda em 2026-06-03, Pedidos ganhou comandos de consulta e cancelamento compartilhados entre WhatsApp e Miauby interno. No WhatsApp autorizado, `miauby pedido`, `miauby pedidos`, `miauby ver pedidos`, `miauby pedidos abertos`, `miauby o que tem para chegar`, `miauby o que falta chegar` e `miauby pedidos aguardando chegada` listam somente pedidos em `Aguardando chegada`, um por linha, com fornecedor, valor, data do pedido, previsao ou `sem previsao` e status `aguardando chegada`/`ja pago, falta chegar`. Se nao houver nada, responde `Nenhum pedido aguardando chegada. Milagre logistico detectado 😼`. Desde 2026-08-24, a lista manual guarda por poucos minutos exatamente os dez itens numerados exibidos. A resposta curta `chegou` pergunta qual pedido chegou; `3`, `3 chegou`, `chegou 3` ou `o terceiro chegou` consomem uma unica vez a opcao correspondente e chamam `POST /pedidos/api/internal/confirm-arrival` pelo `order_id`, depois de revalidar o card `Pedidos`. A escolha vencida exige listar novamente, `nao`/`deixa` cancelam sem escrita e um novo comando explicito substitui a selecao. Para cancelar, aceita `miauby cancelar pedido anb`, `miauby cancelar pedido da nissei`, `miauby cancelar pedido 350`, `miauby remover pedido anb`, `miauby excluir pedido da santa cruz` e `miauby nao precisa mais do pedido da anb`. O bridge chama `GET /pedidos/api/internal/cancel-candidates` com `actor_user_id`; o app Pedidos revalida permissao do usuario no core e busca somente pedidos ainda aguardando chegada. Com um candidato claro, o bridge pede `Confirma cancelar: ANB - R$ 350,00?`. Se houver pagamento ou conta financeira vinculada, a pergunta vira `Esse pedido tem financeiro vinculado. Confirma cancelar mesmo assim: ANB - R$ 350,00?`. Com varios candidatos, ele lista opcoes numeradas e guarda pendencia em `miauw_whatsapp_confirmations` com `tool=selecionar_pedido_whatsapp`; respostas como `1`, `a segunda`, `a de 350`, `anb 350` ou `cancela a primeira` escolhem o pedido, e depois ainda vem a confirmacao final. A execucao chama `POST /pedidos/api/internal/cancel-order` com `idempotency_key=whatsapp-confirm:{confirmation_id}`, `source=miauby_whatsapp` e auditoria; o app Pedidos arquiva/cancela logicamente sem apagar pagamentos, parcelas ou historico financeiro. Pedido ja finalizado/historico nao entra nesse comando.

Desde 2026-08-22, o bridge responde consultas de Encomenda da Cotacao sem passar por Gemini/core e preserva a frase inteira ao chamar `GET /cotacao/api/internal/encomendas?q=...`. Com card `Cotacao` liberado, entende formas como `quais encomendas tem`, `quais encomenda tem?`, `o que tem de encomenda e tals?`, `quais pedidos estao encomendados`, `mostra encomendas`, perguntas por produto, cliente, telefone, contexto ou status e combinacoes desses filtros. Desde 2026-08-24, essa leitura deterministica ocorre antes do interpretador semantico e antes da ajuda de prefixo: somente a consulta de leitura pode listar sem `Miauby` digitado, sempre depois de validar contato permitido e card `Cotacao`; qualquer criacao, alteracao ou outro comando digitado continua exigindo o prefixo. `e tal` e `e tals` sao descartados como vicios de fala e nao filtram a lista. Um `sim` ou `nao` sem confirmacao forte realmente pendente deixa de ser capturado pelo executor legado e continua na memoria conversacional; isso nao amplia o prazo nem a permissao de qualquer escrita. A Cotacao aplica a regra compartilhada e devolve somente dados reais: produto primeiro e, quando existentes, EAN, quantidade, cliente, telefone, endereco, previsao, status, categoria completa e data. Por padrao entram somente encomendas atuais nao finalizadas; historico, antigas e canceladas exigem pedido explicito. A resposta mostra ate 10 e informa o total. A consulta nao altera linha, status, lembrete nem cadastro e continua bloqueada sem acesso ao card. O mesmo endpoint sem `q` alimenta o resumo diario das 17h, preservando allowlist, ferias, dedupe e ausencia de envio quando nao houver encomenda ativa.

Desde 2026-08-16, frases com produto claro, como `miauby encomenda losartana 50 Maria 44 4984894` ou `miauby cliente Joao pediu amoxicilina 500mg 44998489494`, entram no fluxo de escrita contextual. O bridge usa o mesmo parser PHP do Miauby Interno, exige contato vinculado a usuario ativo com card `Cotacao`, prepara `criar_encomenda_cotacao` e somente grava depois da confirmacao vigente. A tool deve estar em `MIAUW_WHATSAPP_CONFIRMED_ACTIONS_ALLOWLIST`; o repositorio continua deixando a execucao forte desligada por padrao. Cliente e telefone nao sao obrigatorios e nunca sao substituidos pelo ator do WhatsApp. Depois da confirmacao, a Cotacao reserva somente a primeira linha integralmente vazia, grava `Produto` e `Categoria`, inicia a categoria com `Encomenda` e preserva nela as observacoes separadas; linha parcialmente preenchida nao e sobrescrita e nenhuma linha nova e criada no fim. Quando apenas o produto estiver faltando, os demais dados ficam preservados em uma pendencia curta vinculada ao contato; a resposta seguinte pode conter somente o produto, `cancelar` encerra a pendencia e outro comando Miauby substitui o contexto anterior. Consultas explicitas continuam no fluxo de leitura acima.
...
- `miauw_whatsapp_conversation_states`: estados isolados e expiraveis. `structured_conversation` e `persistent_user_context` usam 24 horas renovaveis; a projecao persistente guarda somente topico, intencao, filtros e referencias permitidas, uma linha por usuario/canal, sem mensagem integral nem confirmacao/selecao/pergunta pendente.
...
- `POST /miauw/whatsapp/internal/allowlist/update-user-display-name`: endpoint interno por token de cabecalho para atualizar o nome exibido e, quando enviado, o `linked_username_snapshot` dos contatos ja vinculados a um `core_users.id`, sem receber telefone cru. O painel Usuarios usa esse endpoint quando `core_users.display_name` ou `core_users.username` muda e o usuario ja tem WhatsApp vinculado.
...
- `POST /miauw/whatsapp/internal/watchdog`: endpoint interno tokenizado para n8n/monitoramento. Verifica fila travada, outbox `pending/sending`, falhas recentes, respostas lentas, envios `sent` sem id do provedor, pausa do transporte e conversas que receberam `sent` mas ficaram com nova mensagem sem resposta. Desde 2026-06-05, nao envia WhatsApp mesmo se receber `notify=problems|always`; registra somente log interno em `miauw_whatsapp_automation_runs` para analise posterior.
- `POST /miauw/whatsapp/internal/evolution-baileys-alert`: endpoint interno tokenizado para n8n chamar a cada 30 min. Verifica sinais seguros da Evolution/Baileys pelo bridge, como conexao nao aberta, provedor pausado e outbox `failed/dead` recente no provider `evolution`; aceita `notify=never|problems|always`, `dry_run=true` e `lookback_minutes`, mas desde 2026-06-05 fica log-only no WhatsApp: registra em `miauw_whatsapp_automation_runs` e nao envia alerta para usuarios. Esse endpoint nao executa shell nem le Docker logs; o script `ops/evolution/check-baileys-init-timeouts.sh` continua como runbook de host.
...
- `POST /miauw/whatsapp/internal/cotacao-encomenda-reminder`: endpoint legado interno/tokenizado da antiga rotina individual de encomenda. Desde 2026-06-12, se chamado, registra `skipped` e nao envia WhatsApp; a rotina ativa e `POST /miauw/whatsapp/internal/cotacao-encomendas-daily-summary`.
...
- Prefixo `miauby` fica exigido por padrao no repositorio. Em ambiente operacional com allowlist revisada, ele pode ser desligado por `MIAUW_WHATSAPP_REQUIRE_PREFIX=false`; nesse modo, conversa solta sem comando vai para Gemini com personalidade/instrucoes seguras. Se `MIAUW_WHATSAPP_ALLOW_COMMANDS_WITHOUT_PREFIX=true`, comandos operacionais detectados, como `sangria 10 Will`, tambem acionam o core Miauby/API com tools e confirmacao. Com prefixo exigido, mensagens iniciadas com `miauby` acionam o core; com prefixo desligado, `miauby` em qualquer posicao tambem aciona o core. Desde 2026-06-04, quando o prefixo segue exigido, `gemini ...`, `barato ...` e `simples ...` sao prefixos explicitos permitidos para conversa/teste do Gemini. Desde 2026-06-05, texto solto de remetente autorizado sem `miauby` entra na fila apenas como `prefix_missing_help_only`, nao chama parsers operacionais, Gemini, core/tools nem cria pendencia; desde 2026-06-11, responde somente `😺 Para acessar o Miauby é só digitar Miauby e seu comando! ✨`. O registry central `apps/miauw-whatsapp/src/command-help.ts` continua alimentando `miauby menu` e o catalogo oficial do Gemini, agrupado por card real do WhatsApp, mas lista somente fluxos Whats fechados e validados, como Financeiro, Pedidos, Cotacao, Tarefas e Miauby/n8n. Cards que ainda dependem de conversa generica/core/Gemini, como Cashback, Gestao, XP e Codigos, nao entram nesse catalogo ate terem comando direto validado. A secao `N8n / avisos enviados` mostra apenas automacoes que podem mandar WhatsApp para usuarios autorizados, com horario, card e destinatarios resolvidos por nome de usuario/display name, sem telefone nem status tecnico; rotinas internas ficam no painel/log.
...
- Sem `miauby`, conversa simples usa Gemini e nao chama API interna somente quando `MIAUW_WHATSAPP_REQUIRE_PREFIX=false`; com prefixo exigido, texto solto autorizado sem `miauby` responde somente a orientacao curta para digitar `miauby` e o comando. Comandos operacionais sem prefixo so chamam o core quando `MIAUW_WHATSAPP_ALLOW_COMMANDS_WITHOUT_PREFIX=true`, o remetente esta em allowlist, o card esta liberado e a acao ainda exige pendencia/confirmacao.
...
- O worker do bridge tambem expira pendencias vencidas a cada ciclo de fila, evitando que contexto antigo fique marcado como `pending` quando ninguem envia uma nova mensagem.
- Dados sensiveis continuam bloqueados localmente antes de chamar Gemini/core.
- Saudacoes simples como `oi`, `ola`, `teste`, `status` e `ajuda` respondem localmente, de forma super curta, sem chamar Gemini/core para reduzir latencia. Pedidos claramente fora do Miauby, como receita/bolo, filme, piada ou assunto amplo sem relacao operacional, tambem podem ser bloqueados localmente com resposta curta antes de gastar Gemini. O Gemini deve tratar `miauby menu` como catalogo oficial de capacidades do Miauby WhatsApp; quando alguem perguntar o que ele faz, nao deve inventar comando fora da tabela e pode orientar `miauby menu` ou `miauby n8n`. O comando `miauby n8n` tambem responde localmente com as automacoes previstas, horario/card e destinatarios por usuario conforme os cards liberados para aquele numero, sem expor telefone nem status tecnico.
- Audio fica desligado por padrao no Git. Quando `MIAUW_WHATSAPP_AUDIO_INPUT_ENABLED=true`, audio individual de remetente autorizado e baixado do transporte apenas no worker, limitado por `MIAUW_WHATSAPP_AUDIO_MAX_BYTES`, transcrito pelo Gemini e descartado. A transcricao passa por validacao antes do roteador: texto vazio, glossario/seed inventado ou texto implausivel para a duracao do audio e bloqueado com resposta curta, sem consultar modulo e sem executar comando. Todo audio transcrito valido ja funciona como ativacao do Miauby e nao precisa conter o nome, mesmo quando `MIAUW_WHATSAPP_REQUIRE_PREFIX=true`; se a abertura for reconhecida como `Miauby`, `Miau bi`, `Miau bis` ou `Miauw`, inclusive repetida, ela e removida antes do comando. Texto digitado continua exigindo `miauby` exatamente. Transcricao valida segue o mesmo roteador, cards, permissoes, idempotencia e confirmacoes existentes.
...
- Com `MIAUW_WHATSAPP_PIX_RECEIPT_IMAGE_ENABLED=true`, foto, print, imagem encaminhada ou PDF/documento individual de remetente autorizado pode ser tratado como comprovante Pix. O destino precisa bater com o CNPJ/chave Pix `MIAUW_WHATSAPP_PIX_RECEIPT_CNPJ`; nomes correlatos em `MIAUW_WHATSAPP_PIX_RECEIPT_DESTINATION_ALIASES` ficam apenas como pista de OCR e nao liberam lancamento sem o CNPJ/chave configurado. Antes do OCR, o bridge aplica um filtro leve em legenda/nome/extensao: imagem/PDF sem pista negativa continua indo para leitura, mas arquivo claramente fora de formato ou legenda como nota fiscal, receita, catalogo ou planilha e respondido com `Isso ai é um comprovante pix?` sem gastar Gemini. Se o OCR extrair um CNPJ de destino diferente do configurado, ou se nao achar o CNPJ/chave configurado no destino, o bridge bloqueia o lancamento em vez de tentar aceitar por nome. CNPJ encontrado apenas no `raw_text` compacto so vale quando estiver em contexto de destino, recebedor, favorecido ou chave Pix. O contato precisa ter card `Financeiro` liberado e a gravacao continua exigindo pendencia `Sim`/`Nao`. Se faltar valor ou pagador, o bridge nao grava e pede os dados corrigidos por texto; se faltar o CNPJ/chave configurado, responde `Nao achei nosso CNPJ nesse comprovante. Nao gravei nada.`. Data/hora extraidas entram no lancamento; na correcao manual sem data/hora, o sistema usa o momento atual.
...
1. Entrada: o bridge guarda somente referencia sanitizada da midia recebida. No processamento da fila, ele baixa a midia via Evolution `/chat/getBase64FromMediaMessage/{instance}` ou via Media API da Meta, envia para `MIAUW_WHATSAPP_AUDIO_TRANSCRIBE_MODEL` e usa apenas a transcricao validada para seguir no roteador. O proprio audio valido ativa o Miauby; `parseAudioActivationTranscript()` preserva a frase inteira quando nao ha nome e remove `Miauby`, `Miau bi`, `Miau bis` ou `Miauw` quando reconhecidos no inicio. O evento registra `audio_activation_mode=audio_message`, `exact_prefix` ou `phonetic_prefix`; texto digitado nao recebe essa tolerancia. Se a transcricao parecer chute/glossario interno, o evento recebe metadado sanitizado de falha e o usuario recebe `Nao consegui entender bem esse audio. Me manda em texto ou grava de novo falando uma frase clara.`.
...
- outbox atual com problema, ignorando `dead/stale_pending_expired` e `dead/codex_test_wrong_instance_resolved` como falha ativa;
...
O painel tambem mostra `n8n automacoes`: Chegada de pedidos, Fechamento de caixa, Encomendas da Cotacao, Pedidos e boletos, Financeiro, Deploy/checks e Miauby + n8n. Desde 2026-05-29, essa area separa visualmente status da stack, fluxo seguro (`n8n agenda -> backend valida -> WhatsApp avisa`) e cards das rotinas com Quando, Card, Destino, o que o n8n chama, o que o Miauby faz, exemplo de mensagem/estilo, Limite e Controle. Desde 2026-06-01, o card `Fechamento de caixa` mostra tambem `Status agora`, lendo o Financeiro quando possivel para indicar se existe caixa aberto nos ultimos 10 dias e qual dia esta pendente. Desde 2026-06-12, o card `Encomendas da Cotacao` representa a rotina diaria das 17h, enquanto a leitura de `GET /cotacao/api/internal/encomenda-reminders/status` fica apenas como diagnostico do historico legado da Cotacao, sem disparar envio individual. O destino de cada rotina e calculado pelos cards liberados para contatos autorizados reais; LIDs da Evolution protegidos por alias nao entram como destinatarios. n8n apenas orquestra/agenda, enquanto permissao, dados, escrita forte e auditoria continuam no backend Wimifarma. As rotinas `Chegada de pedidos`, `Fechamento de caixa` e `Encomendas da Cotacao` tem box de controle `Ligado/Desligado` no painel, com botao `Desativar`/`Ativar`; n8n pode continuar agendado, mas o bridge ignora a execucao quando elas estiverem pausadas. Para operacao atual, n8n deve chamar os endpoints internos `smoke-check`, `watchdog`, `evolution-baileys-alert`, `pix-ocr-daily-summary`, `pedidos-arrival-check`, `financeiro-cash-closing-reminder` e `cotacao-encomendas-daily-summary`; o endpoint antigo `cotacao-encomenda-reminder` ficou legado desativado e nao envia WhatsApp. O bridge calcula destinatarios apenas das rotinas que avisam usuarios, registra cada execucao em `miauw_whatsapp_automation_runs`, registra em `miauw_whatsapp_error_logs` apenas falhas acionaveis e aplica cooldown/dedupe para nao floodar a equipe. Desde 2026-06-05, `watchdog` e `evolution-baileys-alert` ficam apenas em painel/log interno mesmo se o payload trouxer `notify=problems|always`. Perguntas como `miauby em pedidos o que falta chegar` sao respondidas localmente pelo bridge com a mesma tabela da rotina, sem depender de Gemini/core, desde que o contato tenha card `Pedidos`. O worker tambem recupera outbox `pending` recente em lote pequeno e expira pendencias antigas por `MIAUW_WHATSAPP_OUTBOX_RECOVERY_MAX_AGE_MINUTES`, evitando mensagens velhas fora de contexto depois de queda/redeploy.

Desde 2026-05-29, o smoke-check executa os checks de health/proxy/core/Evolution em paralelo para nao somar todos os timeouts em uma chamada do n8n. O watchdog so considera `queued`/`pending` como travado quando `next_attempt_at` ja venceu; mensagens aguardando backoff normal nao devem gerar alerta. Desde 2026-06-05, esses achados do watchdog ficam silenciosos no WhatsApp e entram apenas como historico interno em `miauw_whatsapp_automation_runs`, com mensagem, severidade, fingerprint e tipos dos problemas. Se o transporte estiver em pausa por erro temporario, o worker falha rapido o envio e agenda retry/backoff, em vez de ficar bloqueado em `processing`/`sending` ate a recuperacao reprocessar a mesma mensagem.
...
- o destino bate por CNPJ extraido ou chave Pix extraida; nomes correlatos configurados em `MIAUW_WHATSAPP_PIX_RECEIPT_DESTINATION_ALIASES` servem apenas como pista de OCR e, se o CNPJ/chave configurado nao aparecer no destino, a leitura e recusada com `Nao achei nosso CNPJ nesse comprovante. Nao gravei nada.`;
...
Quando passa, o bridge transforma a extracao em comando interno `pix cnpj R$ valor - pagador - obs ...`, chama `site/miauw/agent-actions.php` para preparar a acao e envia uma confirmacao `Sim`/`Nao` para numeros de funcionario comum. O comando e a pendencia mantem os detalhes completos sanitizados, mas a mensagem de confirmacao ao contato usa resumo curto em linhas (`PIX CNPJ identificado ✅`, valor, pagador, responsavel quando houver e data quando extraida). Para o perfil `Farmacia`, o fluxo e diferente: a extracao aceita monta um comando `pix_cnpj` sem responsavel publico, guarda data/destino/instituicao/ID Pix em auditoria e abre `selecionar_responsavel_whatsapp`; a resposta final apos escolher usuario fica curta (`PIX CNPJ lançado ✅`, valor e pagador/responsavel quando houver). Com Meta Cloud API, botoes interativos podem ser usados. Com Evolution API, a confirmacao e sempre texto simples (`Responda SIM para gravar ou NAO para cancelar`) porque `sendButtons` pode retornar sucesso sem renderizar no WhatsApp normal/linked device. `Sim` grava no Financeiro Node/Postgres como lancamento `Pix CNPJ` por endpoint interno tokenizado e responde curto; `Nao` cancela e orienta escrever os dados corrigidos como `miauby pix cnpj 65 Bruno`. O typo `pix cpnj` tambem e aceito. Texto manual que ja vem nesse formato e parseado localmente e grava pelo endpoint interno do Financeiro sem passar pelo Gemini, desde que card, responsavel e permissao estejam OK. Textos internos como `Miauby criou a categoria ... por comando interno` nao devem ser despejados no WhatsApp; quando aparecerem em resposta publica, sao resumidos para `Miauby - Categoria criada: ...`.
