...
Quando passa, o bridge transforma a extracao em comando interno `pix cnpj R$ valor - pagador - obs ...`, chama `site/miauw/agent-actions.php` para preparar a acao e envia uma confirmacao `Sim`/`Nao`. Com Meta Cloud API, botoes interativos podem ser usados. Com Evolution API, o padrao e texto simples (`Responda SIM para gravar ou NAO para cancelar`) porque `sendButtons` pode retornar sucesso sem renderizar no WhatsApp normal/linked device; botoes na Evolution so devem ser ligados com `MIAUW_WHATSAPP_EVOLUTION_INTERACTIVE_CONFIRMATIONS=true` apos teste real. `Sim` grava no Financeiro como lancamento `Pix CNPJ`; `Nao` cancela e orienta escrever os dados corrigidos no formato curto `pix cnpj 50,00 - Nome - obs opcional`. O typo `pix cpnj` tambem e aceito.
