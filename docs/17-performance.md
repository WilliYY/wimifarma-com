...
- Em 2026-06-08, essa variacao animada da Home foi trocada para `logo-wimifarma-home-animated.svg`; o asset versionado reaproveita os keyframes originais do SVG fornecido em loop infinito, remove a camada branca externa, usa filtro SVG para tirar o branco do PNG embutido e manter a marca clara/transparente, e usa cache-bust em `site/home.php`. A Home nao deve aplicar `mix-blend-mode` nessa logo, pois isso escurece a marca em fundos coloridos; a validacao deve conferir login central, rodape e header autenticado em desktop/mobile.
