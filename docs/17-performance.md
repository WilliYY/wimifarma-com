...
- Em 2026-06-01, a animacao pesada de troca de cor do footer liquido do login foi desativada em `site/home.php` para evitar piscada/repaint perceptivel no Chrome. A animacao liquida original do footer foi restaurada com 128 bolhas e filtro SVG, com camada isolada por `contain: layout`/`translateZ(0)`; o `paint containment` nao deve voltar nessa camada porque cria um recorte horizontal visivel no efeito, e a camada das bolhas precisa ficar mais alta que a faixa visivel para o limite reto nao aparecer. A variacao de cores voltou depois como um ciclo lento de `background-color` aplicado ao fundo e as bolhas, sem circulos decorativos extras nem alteracao de geometria; em `prefers-reduced-motion`, as bolhas ficam estaticas em vez de sumirem e deixarem uma faixa reta.
...
- Em 2026-06-01, a tela de login passou a reutilizar a mesma variacao animada da home autenticada, sem JS extra; o SVG oficial continua nos modulos internos.
- Em 2026-06-09, a Home passou a usar `logo-wimifarma-home-animated.svg` como variacao animada em loop, mantendo fundo transparente, proporcao `1024/400`, preload `image/svg+xml`, cache-bust em `site/home.php` e sem `mix-blend-mode`. A versao oficial do arquivo preserva a animacao enviada pelo usuario e aplica filtro SVG interno para transformar o preto dos PNGs recortados em branco e o fundo branco em transparencia; nao reintroduzir retangulo branco, filtro CSS global ou fundo recortado nessa logo. Ainda em 2026-06-09, o SVG foi otimizado sem perda visual: o mesmo PNG interno fica uma unica vez em `logo-raster-source`, e as tres partes animadas usam `<use>` com os mesmos recortes/filtro, reduzindo peso sem trocar arte, detalhes ou loop. A validacao deve conferir login central, rodape e header autenticado em desktop/mobile.
- Em 2026-06-10, o botao flutuante do widget global do Miauby passou de avatar circular para video MP4 em loop (`site/miauw/miauby-widget-loop.mp4`) dentro de caixa quadrada arredondada, com cerca de 1,27 MB, sem recompressao para preservar detalhe. O video usa `autoplay muted loop playsinline`, `poster` do avatar estatico e cache-bust `20260610-miauby-video`; se houver lag futuro, otimizar primeiro uma copia medindo tamanho/dimensao antes/depois e validar que o video continua cabendo sem corte.
- Em 2026-06-01, o footer do login ganhou contador anonimo de visitantes em PHP puro, sem JS, banco novo ou chamada externa. Ele usa arquivo JSON pequeno em `site/wp-content/uploads/wimifarma-runtime/home-counter.json`, trava escrita com `flock`, ignora user-agents de bots/checks comuns para nao inflar validacoes e mostra separadamente visitantes unicos e visitas registradas por janela de 30 minutos. Assim, redirecionamentos de login/logout e recarregamentos imediatos no mesmo navegador nao viram novas visitas. Se o arquivo estiver zerado e o navegador ja tiver cookie antigo, a primeira visualizacao humana inicializa 1 visitante para evitar o card preso em zero.
- Em 2026-06-02, o conteudo do footer do login foi compactado e reagrupado em tres areas responsivas: marca/WhatsApp, navegacao com atendimento e contador. A mudanca reduziu altura e espacamentos, mas preservou a camada liquida de bolhas e o filtro SVG sem alterar a geometria da animacao.
- Em 2026-06-02, a tela de login ganhou ajuste responsivo para celulares e tablet pequeno: `100dvh`/`100svh`, safe-area, campos/botao com altura confortavel para toque, onda liquida afastada do formulario, video promocional ainda oculto em telas estreitas e elementos flutuantes pesados ocultos em celular. Foi validado visualmente em 360x740, 390x844, 430x932, 768x1024 e 1366x768, sem rolagem horizontal no documento e sem alterar autenticacao, CSRF, sessao ou SSO.
