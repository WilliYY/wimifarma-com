...
Em 2026-05-29, o historico local da Cotacao passou a incluir tambem estilos manuais: pintar ou apagar cor de celula, linha, coluna ou selecao entra no mesmo `Ctrl+Z`/`Ctrl+Y` das edicoes de celula, colagens, filtros e colunas. A digitacao ativa dentro de uma celula continua usando o undo nativo do textarea ate o valor ser salvo.

Desde 2026-08-28, apagar totalmente um nome antes preenchido em `PRODUTO` remove somente a cor manual daquela celula dentro do mesmo `BEGIN`/`COMMIT` do valor. Os eventos `cell_updated` e `cells_batch_updated`, as respostas HTTP e os canais `cell:update`/`cells:update` incluem `clearedStyleKeys`, permitindo que todas as abas removam o estilo sem snapshot completo. Reescrever o nome preserva a cor, e o historico local restaura nome e cor juntos no `Ctrl+Z`.
