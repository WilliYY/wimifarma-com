(function () {
  ...
  function initLoginRunners() {
    ...
    if (reducedMotion || nodes.length === 0 || !window.requestAnimationFrame) {
      return;
    }
    ...
    var states = nodes.map(function (node, index) {
      return {
        node: node,
        x: clamp(window.innerWidth * (0.14 + (index * 0.16)), 18, Math.max(18, window.innerWidth - 160)),
    ...
    function tick(now) {
      ...
      states.forEach(function (state, index) {
        ...
        var width = rect.width || 130;
        var height = rect.height || 130;
