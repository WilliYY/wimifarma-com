...
type RenderSupplierOrder = SupplierOrderRow & {
  items: ItemRow[];
  payments: PaymentRow[];
};
...
function redirectHome(res: Response, month = '', view = ''): void {
  const cleanView = view === 'pedidos' ? 'pedidos' : '';
...
function currentView(req: Request): '' | 'pedidos' {
  if (req.path === `${BASE_PATH}/pedidos` || req.path === `${BASE_PATH}/pedidos/`) return 'pedidos';
  if (String(req.query.view || req.body?.view || '') === 'pedidos') return 'pedidos';
  return '';
}
...
function orderDueStatus(order: SupplierOrderRow): { key: string; label: string; days: number | null } {
...
function arrivalStatus(order: SupplierOrderRow): { key: string; label: string } {
...
async function createSupplierOrder(req: Request): Promise<void> {
  ...
  try {
    ...
    const accountResult = await client.query<{ id: string }>(
      `INSERT INTO gestao_accounts
       ...
       RETURNING id`,
      [
        `Pedido - ${supplier}`.slice(0, 180),
        paidNow ? 'pago' : 'pendente',
        totalCents,
        month,
        note || null,
        dueAt,
    ...
    const orderResult = await client.query<{ id: string }>(
      `INSERT INTO gestao_supplier_orders
        (account_id, supplier_name, status, expected_arrival_at, created_by)
       VALUES ($1, $2, 'pedido', $3::date, $4)
       RETURNING id`,
      [accountId, supplier, expectedArrivalAt, userId],
    );
    orderId = Number(orderResult.rows[0].id);

    await auditPg(client, accountId, userId, 'gestao_pedido_criado', `Pedido criado para ${supplier} / ${formatMoney(totalCents)}`);
    if (paidNow) {
      await auditPg(client, accountId, userId, 'gestao_pedido_pago_criacao', 'Pedido criado ja com pagamento registrado.');
    }
    await client.query('COMMIT');
  } catch (error) {
    await client.query('ROLLBACK');
  ...
  await logMysql(userId, 'gestao_pedido_criado', 'gestao_pedido', orderId, `Pedido criado: ${supplier} / ${formatMoney(totalCents)}`);
}

async function confirmSupplierOrderArrival(req: Request): Promise<void> {
  ...
  try {
    await client.query('BEGIN');
    const orderResult = await client.query<{ account_id: string; status: SupplierOrderStatus; supplier_name: string }>(
      'SELECT account_id, status, supplier_name FROM gestao_supplier_orders WHERE id = $1 FOR UPDATE',
      [id],
    );
    const order = orderResult.rows[0];
    if (!order) throw new Error('Pedido nao encontrado.');
    if (order.status !== 'pedido') throw new Error('Esse pedido ja teve chegada confirmada.');
    ...
    await client.query(
      `UPDATE gestao_supplier_orders
       SET status = $1,
           confirmed_at = now(),
           confirmed_by = $2,
           finished_at = CASE WHEN $1 = 'historico' THEN now() ELSE finished_at END,
           finished_by = CASE WHEN $1 = 'historico' THEN $2 ELSE finished_by END
       WHERE id = $3`,
      [movedToHistory ? 'historico' : 'confirmado', userId, id],
    );
    await auditPg(client, accountId, userId, 'gestao_pedido_chegada_confirmada', `Chegada confirmada: ${order.supplier_name}`);
    if (movedToHistory) {
      await auditPg(client, accountId, userId, 'gestao_pedido_finalizado', 'Pedido ja estava pago; movido para historico apos chegada.');
    }
    await client.query('COMMIT');
  } catch (error) {
    await client.query('ROLLBACK');
  ...
  await logMysql(userId, 'gestao_pedido_chegada_confirmada', 'gestao_pedido', id, movedToHistory ? 'Pedido recebido e finalizado.' : 'Pedido recebido e aguardando pagamento.');
}
...
async function listSupplierOrders(month: string): Promise<RenderSupplierOrder[]> {
  const bounds = monthBounds(month);
  const ordersResult = await pgPool.query<SupplierOrderRow>(
    `SELECT o.*,
     ...
     WHERE a.archived_at IS NULL
       ...
       AND (
        o.status IN ('pedido', 'confirmado')
        OR a.competence_month = $1
        OR (o.finished_at >= $2::timestamptz AND o.finished_at < $3::timestamptz)
        OR EXISTS (
          SELECT 1 FROM gestao_account_payments gp
          WHERE gp.account_id = a.id
            AND gp.paid_at >= $2::timestamptz
     ...
     ORDER BY
       CASE o.status WHEN 'pedido' THEN 0 WHEN 'confirmado' THEN 1 ELSE 2 END ASC,
       CASE WHEN o.status = 'pedido' AND o.expected_arrival_at IS NOT NULL THEN 0 ELSE 1 END ASC,
       CASE WHEN o.status = 'pedido' THEN o.expected_arrival_at END ASC NULLS LAST,
       CASE WHEN o.status = 'confirmado' AND a.due_at IS NOT NULL THEN 0 ELSE 1 END ASC,
       CASE WHEN o.status = 'confirmado' THEN a.due_at END ASC NULLS LAST,
       COALESCE(o.finished_at, o.confirmed_at, o.created_at) DESC,
       o.id DESC
     LIMIT 500`,
    [month, bounds.start, bounds.end],
  );
...
async function supplierOrdersBadge(): Promise<number> {
  const result = await pgPool.query<{ count: string }>(
    `SELECT COUNT(*)::bigint AS count
     FROM gestao_supplier_orders o
     JOIN gestao_accounts a ON a.id = o.account_id
     WHERE o.status = 'pedido'
       AND a.archived_at IS NULL
...
function renderSupplierOrderCard(req: Request, order: RenderSupplierOrder, selectedMonth: string): string {
  const orderId = Number(order.id);
  ...
  const remainingCents = Math.max(0, totalCents - paidCents);
  const progress = totalCents > 0 ? Math.min(100, Math.max(0, (paidCents / totalCents) * 100)) : 0;
  const due = orderDueStatus(order);
...
function renderOrdersView(req: Request, selectedMonth: string, orders: RenderSupplierOrder[], arrivingToday: number): string {
  ...
  return `<section class="gestao-orders-workspace">
    <div class="gestao-orders-summary">
      ...
      <div><span>No historico</span><strong>${e(historyOrders.length)}</strong></div>
    </div>
...
async function renderApp(req: Request): Promise<string> {
  ...
  const allAccounts = selectedView === 'pedidos' ? [] : await listAccounts(selectedMonth);
  const notes = selectedView === 'pedidos' ? [] : await listNotepadNotes();
  const orders = selectedView === 'pedidos' ? await listSupplierOrders(selectedMonth) : [];
  const arrivingToday = selectedView === 'pedidos' ? await supplierOrdersBadge() : 0;
  ...
  const contentHtml = selectedView === 'pedidos'
    ? renderOrdersView(req, selectedMonth, orders, arrivingToday)
  ...
  const viewTabs = `<nav class="gestao-view-tabs" aria-label="Areas da Gestao">
    <a class="${selectedView === '' ? 'is-active' : ''}" href="${BASE_PATH}/?mes=${encodeURIComponent(selectedMonth)}">Contas</a>
    <a class="${selectedView === 'pedidos' ? 'is-active' : ''}" href="${BASE_PATH}/pedidos?mes=${encodeURIComponent(selectedMonth)}">Pedidos${arrivingToday > 0 ? `<span>${e(arrivingToday)}</span>` : ''}</a>
  </nav>`;
  ...
  <main class="gestao-page" data-miauby-screen-object="modulo gestao" data-miauby-screen-label="Modulo Gestao: ${e(formatMoney(summary.paidCents))} pago no mes, ${e(formatMoney(summary.pendingCents))} pendente">
    <section class="gestao-hero">
      <div>
        ...
        <h1>${selectedView === 'pedidos' ? 'Pedidos' : 'Gestao'}</h1>
        <p>${selectedView === 'pedidos' ? 'Recebimento de fornecedores, vencimentos de boletos e pagamentos entrando na categoria Boleto.' : 'Contas manuais, categorias livres, pagamentos parciais e saldo conferido por mes.'}</p>
      </div>
...
app.get(`${BASE_PATH}/api/orders/badge`, asyncRoute(async (_req, res) => {
  res.json({ ok: true, arriving_today: await supplierOrdersBadge() });
}));
...
app.get(`${BASE_PATH}/pedidos`, requireAuth, asyncRoute(async (req, res) => {
  res.type('html').send(await renderApp(req));
}));
app.get(`${BASE_PATH}/pedidos/`, requireAuth, asyncRoute(async (req, res) => {
  res.redirect(`${BASE_PATH}/pedidos?mes=${encodeURIComponent(monthValue(req.query.mes))}`);
}));
...
app.post(`${BASE_PATH}/pedidos`, requireAuth, verifyCsrf, asyncRoute(handleGestaoPost));
