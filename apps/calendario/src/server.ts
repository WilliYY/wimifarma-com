  ...
  try {
    ...
    const result = await pgPool.query<NoteRow>(
      `INSERT INTO calendario_day_notes (calendar_id, month, day, note_text, color_id, updated_by, updated_at)
       ...
       ON CONFLICT (calendar_id, month, day) DO UPDATE SET
    ...
    await pgPool.query('UPDATE calendario_calendars SET updated_at = NOW() WHERE id = $1', [calendarId]);
...
app.post(`${BASE_PATH}/api/colors/:id/archive`, async (req, res, next) => {
  try {
    ...
    await pgPool.query('UPDATE calendario_day_notes SET color_id = NULL, updated_at = NOW(), updated_by = $1 WHERE color_id = $2', [user.id, colorId]);
    await auditEvent(user, 'calendario_cor_arquivada', calendarId, `Cor ${result.rows[0].label} arquivada.`, { color_id: colorId });
    res.json({ ok: true });
  } catch (error) {
    next(error);
  }
