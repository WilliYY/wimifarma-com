...
const LEGACY_MYSQL_IMPORT_ENABLED = parseBool(env.CASHBACK_LEGACY_MYSQL_IMPORT_ENABLED ?? 'true');
const LEGACY_MYSQL_MIRROR_ENABLED = parseBool(env.CASHBACK_LEGACY_MYSQL_MIRROR_ENABLED ?? 'true');
const LEGACY_MYSQL_LOGS_ENABLED = parseBool(env.CASHBACK_LEGACY_MYSQL_LOGS_ENABLED ?? 'true');
...
router.get(['/', '/index.php'], (req: Request, res: Response) => {
  res.redirect(req.session.user ? `${BASE_PATH}/dashboard.php#busca` : `${BASE_PATH}/login.php`);
});

router.get('/login.php', (req: Request, res: Response) => {
  if (req.session.user) {
...
async function ensureSchema(): Promise<void> {
  await pgPool.query(`
    ...
    CREATE TABLE IF NOT EXISTS cashback_whatsapp_messages (
      ...
      status TEXT NOT NULL DEFAULT 'pendente' CHECK (status IN ('pendente', 'aberta', 'copiada', 'enviada', 'cancelada')),
      due_date DATE,
...
async function generateRecompraMessages(req: Request, returnDays: number, today: string): Promise<DbRow[]> {
  ...
  for (const row of result.rows as DbRow[]) {
    ...
    const saved = await saveWhatsappMessage({
      campaign: 'recompra',
      dedupeKey: `recompra:${num(row.id)}:${today}`,
      clientId: num(row.id),
      purchaseId: null,
      creditId: null,
      clientName: String(row.name || ''),
...
function statusLabel(value: unknown): string {
  const labels: Record<string, string> = { pendente: 'Pendente', aberta: 'Aberta', copiada: 'Copiada', enviada: 'Enviada', cancelada: 'Cancelada' };
  return labels[String(value || '')] || String(value || '');
}
