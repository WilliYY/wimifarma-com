...
test('new quick voucher codes use five digits and rotate through 100000 values', () => {
  assert.equal(CURRENT_QUICK_VOUCHER_CODE_DIGITS, 5);
  assert.equal(QUICK_VOUCHER_CODE_SPACE, 100_000);
  assert.equal(quickVoucherCodeAt(930, 0), '00930');
  assert.equal(quickVoucherCodeAt(99_999, 1), '00000');
});
