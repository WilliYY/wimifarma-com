...
const toolContracts: ToolContract[] = [
  ...
  },
  {
    ...
    openaiTool: true,
    localAction: true,
    description: 'Cria item de encomenda na Cotacao Geral apenas apos confirmacao humana, preservando contexto operacional em qualquer ordem.',
    parameters: objectSchema(
      {
        produto: { type: 'string', minLength: 2, maxLength: 160 },
        quantidade: { type: 'string', maxLength: 80 },
        responsavel: { type: 'string', minLength: 2, maxLength: 100 },
        telefone: { type: 'string', maxLength: 32 },
        endereco: { type: 'string', maxLength: 220 },
        tipo_entrega: { type: 'string', maxLength: 20 },
        data_encomenda: { type: 'string', maxLength: 60 },
        horario: { type: 'string', maxLength: 60 },
        prioridade: { type: 'string', maxLength: 40 },
        referencia: { type: 'string', maxLength: 260 },
        observacao_livre: { type: 'string', maxLength: 320 },
        categoria_extra: { type: 'string', maxLength: 600 },
        observacao: { type: 'string', maxLength: 700 },
      },
      ['produto'],
    ),
    required: ['produto'],
